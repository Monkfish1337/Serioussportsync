...
function createApp() {
  ...
  // "Check it works" was removed in 0.95.1.
  //
  // It ran a settled fixture from the user's catalogs through every configured
  // pipeline and reported what each one did. The idea was sound; the execution
  // never was. It has to guess which fixture has a release, and there is no
  // reliable way to guess that — it picked by recency, which is uncorrelated
  // with availability, so on working installs it chose things like a Friday
  // practice session, found nothing, and told the user their setup was broken.
  // Two rounds of fixing the guess did not fix the guessing.
  //
  // A check that cries wolf on a healthy install is worse than no check: it
  // sends people to debug configuration that was never wrong. Stremio and Nuvio
  // already answer the same question honestly, against whatever the user
  // actually wants to watch, so that is where verification belongs.

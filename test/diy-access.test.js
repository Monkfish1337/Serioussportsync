...
test('stored DIY settings cannot enable playback after demotion and do not affect fast providers', () => {
  const user = {role:'admin',config:{diyUsenetEnabled:true,nativeNntpEnabled:true,diyNativeSearchEnabled:true,uuEnabled:true,easynewsEnabled:true,torboxApiKey:'fixture'}};
  ...
  assert.equal(config.uuEnabled,true);

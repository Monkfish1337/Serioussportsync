...
function start(options) {
  ...
  const cfg = settings.getAvailabilityWarm();
  if (!available) {
    log('recent-event warm-up disabled because SQLite is unavailable');
    return status();
  }
  if (!cfg.enabled) {
    log('recent-event warm-up disabled in Database settings');
    return status();
  }
  log('scheduling recent-event warm-up every ' + cfg.intervalHours + 'h (last '

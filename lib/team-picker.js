...
const CHOOSERS = Object.freeze([
  ...
  { key: 'nhl', label: 'NHL team', provider: 'espn', league: 'nhl', describe: (team) => team.name, hint: 'Regular season and playoffs.' },
]);
...
function prepareRoster(key, teams) {
  const custom = require('./custom-promotions');
  const users = require('./users');
  const old = custom.list();
  const legacySelected = old.filter((spec) => spec.enabled !== false
    && spec.autoTeam !== true && String(spec.id || '').startsWith(key + '-')
    && (spec.teamId || (spec.teamFilter && spec.teamFilter.id)))
    .map((spec) => spec.id);
  // The previous shared switch affected every account. Preserve its visible
  // teams once per account before turning the roster into shared inventory.
  if (legacySelected.length) {
    for (const user of users.listUsers()) {
      const prior = Array.isArray(user.config && user.config.teamPromotions)
        ? user.config.teamPromotions : [];
      users.updateUserConfig(user.id, {
        teamPromotions: Array.from(new Set(prior.concat(legacySelected))),
      });
    }
  }
  const changed = custom.ensureTeamRoster(teams.map((team) => specFor(key, team)));
  if (changed) require('./promotions').reload();
}
...
async function teamsFor(key, opts) {
  ...
  try {
    if (entry.provider === 'football-data') {
      ...
      if (!apiKey) {
        // "Admin → Sources" named a page that does not exist: the nav item is
        // Server, and there has never been a Sources page. A new operator with
        // no key — which is every new operator — was sent somewhere they could
        // not find. Name the page as the sidebar labels it.
        return { ok: false, teams: [], error: 'Add a football-data.org API key on the Server page to choose a club.' };
      }
...
// Does this saved promotion belong to this chooser and this team?
//
// One function rather than three copies, because the three places that ask —
// drawing a team as picked, deciding whether a click creates or removes, and
// the remove route finding what to disable — MUST agree. They did not: only
// the remove route had the rule, the endpoint feeding the page never reported
// a selection at all, and so the wizard drew every team as unpicked and turned
// every click into a create.
//
// The id prefix is part of the test because a team id is only unique within its
// provider: ESPN numbers NFL, NBA and MLB teams separately, so team 12 exists
// three times over.
function matchesTeam(spec, key, teamId) {
  if (!spec) return false;
  const wanted = String(teamId || '');
  if (!wanted) return false;
  const filter = spec.teamFilter || {};
  const sameTeam = String(filter.id || '') === wanted || String(spec.teamId || '') === wanted;
  return sameTeam && String(spec.id || '').indexOf(String(key || '') + '-') === 0;
}

// The ids of the teams in `teams` that currently have a SERVED catalog.
//
// Enabled only, and that is the point rather than an oversight: deselecting a
// team keeps its promotion and its stored fixtures and merely stops serving
// them, so a kept-but-off team must draw as unpicked — otherwise clicking it to
// get it back would be read as a request to remove it.
function selectedTeamIds(key, specs, teams) {
  const list = Array.isArray(specs) ? specs : [];
  const out = [];
  for (const team of (Array.isArray(teams) ? teams : [])) {
    const id = String((team && team.id) != null ? team.id : '');
    if (!id) continue;
    const found = list.find((spec) => matchesTeam(spec, key, id));
    if (found && found.enabled !== false) out.push(id);
  }
  return out;
}
...
module.exports = {
  CHOOSERS, chooser, teamsFor, prepareRoster, specFor, promotionIdFor, matchesTeam, selectedTeamIds,
  clearCache, CACHE_MS,
};

...
function csrf(req, res, next) {
  ...
  // Sandboxed/private browser contexts can serialize a legitimate form origin
  // as `null`. Sec-Fetch-Site is a browser-controlled header, so only accept
  // that value when the browser explicitly identifies the request as same-origin.
  if (origin === 'null' && site === 'same-origin') return next();
  let originHost;

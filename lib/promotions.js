...
const aew = {
  ...
  includeEvent(ev) {
    const n = (ev.name || '').trim();
    if (/^(?:AEW\s+)?(Dynamite|Collision|Rampage|Battle\s+of\s+the\s+Belts)\b/i.test(n)) return false;    // Non-broadcast fan events. AEW's own schedule lists these alongside the
...
function correctWeeklyShowDate(name, date, raw) {
  ...
  // An episode TheSportsDB has not scheduled yet carries no local time and a
  // 00:00:00 placeholder, and its dateEvent IS the local air date — the local
  // date field repeats it. Shifting that midnight to Eastern put every future
  // Raw on a Sunday and every future SmackDown on a Thursday. Aired episodes
  // always have strTimeLocal set, and their dateEventLocal differs.
  if (!String(raw.strTimeLocal || '').trim() && raw.dateEventLocal === utcDate
      && (!raw.strTime || raw.strTime === '00:00:00')) {
    return utcDate;
  }
...
function weeklyWrestlingShow({ id, name, brand, show, leagueId, pattern, includePattern, parent, aewSchedule }) {
  ...
  // AEW's weekly episodes are also topped up from AEW's own schedule, because
  // TheSportsDB's free key never reaches the future ones. See
  // aew.weeklyEpisodes and the supplement step in scripts/refresh.js.
  if (aewSchedule) promotion.aewScheduleShow = show;
...
const wrestlingWeeklyShows = [
  ...
  weeklyWrestlingShow({ id: 'aew-dynamite', name: 'AEW Dynamite', brand: 'AEW', show: 'Dynamite',
    leagueId: '4563', pattern: /^(?:AEW\s+)?Dynamite\b/i, parent: aew, aewSchedule: true }),
  weeklyWrestlingShow({ id: 'aew-collision', name: 'AEW Collision', brand: 'AEW', show: 'Collision',
    leagueId: '4563', pattern: /^(?:AEW\s+)?Collision\b/i, parent: aew, aewSchedule: true }),
];
...
function f1TitleSession(title) {
  ...
  // Support programmes name the weekend but no session, so a race event read
  // them as unlabelled race rips: "Azerbaijan.Paddock.Uncut" and
  // "Azerbaijan.Weekend.Warm-Up" both passed as the Grand Prix (2026-09-25).
  // Madrid, the same day: Pre-Qualifying.Show, Post-Qualifying.Show and
  // Teds.Qualifying.Notebook read as qualifying, and Pre-Race.Show as the race.
  if (/paddock|uncut|warm[\s._-]*up|build[\s._-]*up|(?:pre|post)[\s._-]*(?:race|quali|sprint)|\bshow\b|notebook|press[\s._-]*conference|highlights|analysis|debrief|chequered[\s._-]*flag/.test(t)) {
    return 'other';
  }
  ...
  // "Practice2" (no separator) is a real form: Formula1.S2026E81.Round15.
  // Azerbaijan.Practice2.F1TV..., seen rejected as unlabelled on 2026-09-25.
  if (/\bpractice(?:[1-3])?\b|free[\s.\-_]*practice|\bfp[1-3]\b|практик/.test(t)) return 'practice';
  if (/\brace\b|гонка/.test(t)) return 'race';
  return 'unlabelled';
}

// Which practice session (1-3) a name or release title means, or 0 when it
// does not say. f1Session files all three under one 'practice' kind, which is
// right for the catalog and wrong for matching: a Practice 3 event accepted
// Practice 1 and 2 releases (Discussions #42). Releases write the number
// several ways: "Practice 1", "Practice.One", "Free.Practice.2", "FP3",
// "Практика 1", "Третья практика".
const F1_PRACTICE_WORDS = { one: 1, two: 2, three: 3, первая: 1, вторая: 2, третья: 3 };
function f1PracticeNumber(text) {
  const t = String(text || '').toLowerCase();
  const digit = t.match(/\bfp[\s._-]*([1-3])\b|practice[\s._-]*([1-3])\b|практик[аи]?[\s._-]*([1-3])\b/);
  if (digit) return Number(digit[1] || digit[2] || digit[3]);
  const word = t.match(/practice[\s._-]*(one|two|three)\b|(первая|вторая|третья)[\s._-]*практик/);
  return word ? F1_PRACTICE_WORDS[word[1] || word[2]] : 0;
}
...
const f1 = {
  ...
  searchTitles(event) {
    ...
    if (session === 'race') {
      ...
      // Practice asks for its own number first ("Practice 3", "FP3"), so the
      // searches a bounded provider reaches are for the right session.
      const practice = session === 'practice' ? f1PracticeNumber(name) : 0;
      const label = practice ? sessionLabel + ' ' + practice : sessionLabel;
      if (year) {
        out.add(('Formula 1 ' + year + ' ' + loc + ' GP ' + label).trim());
        out.add(('F1 ' + year + ' ' + loc + ' GP ' + label).trim());
        // F1 scene rips also use the compact location token without GP suffix
        out.add(('Formula 1 ' + year + ' ' + loc + ' ' + label).trim());
        if (practice) {
          out.add(('F1 ' + year + ' ' + loc + ' FP' + practice).trim());
          // The unnumbered forms stay last: some groups bundle all practices.
  ...
  isRelevantStreamTitle(title, event) {
    ...
    } else if (want === 'practice') {
      // Both numbered and different: the wrong practice. A release that names
      // no number (a bundle of all practices) is still accepted.
      const wantNumber = f1PracticeNumber(event.name);
      const gotNumber = f1PracticeNumber(title);
      if (wantNumber && gotNumber && wantNumber !== gotNumber) {
        return { ok: false, reason: 'practice(' + gotNumber + '≠' + wantNumber + ')' };
      }
    }
    return { ok: true };
  },
...
// for.
const NICKNAME_IS_THE_RELEASE_NAME = new Set(['nfl', 'nba', 'mlb', 'nhl']);
...
function createGenericPromotion(spec) {
  ...
  return {
    ...
    // A team catalog ("epl-mun") serves fixtures its league ("epl") also
    // serves. Review aliases shows its query measurements on the league's page
    // and applies the league's query rules to it (lib/query-review.js).
    reviewParent: spec.autoTeam === true && /^[a-z0-9]+-/.test(id) ? id.split('-')[0] : null,
    source: sourceObj,
    posterShape,
    ...
    searchTitles(event) {
      ...
      if (plainSplit && date && teamAliasLookup) {
        ...
        //
        // Only when the stored date can be that UTC day. The MLB schedule stores
        // the LOCAL official date, so for MLB the day before is always wrong —
        // and it sat at index 1, the slot a bounded provider actually reaches,
        // spending half of every Prowlarr MLB search on yesterday. A timestamp
        // tells the two apart: if its UTC day is the stored day and the clock is
        // before noon UTC, it is an American evening game stored by UTC date.
        const stamp = String(event && event.timestamp || '');
        const storedAsUtcNight = !/^\d{4}-\d{2}-\d{2}T\d{2}/.test(stamp)
          || (stamp.slice(0, 10) === date && Number(stamp.slice(11, 13)) < 12);
        const datesToTry = NICKNAME_IS_THE_RELEASE_NAME.has(String(presetName || '').toLowerCase())
          ? (storedAsUtcNight
            ...
            : [dotted, toDmy(date)]).filter(Boolean)
          : [dotted];
...
// NHL, on the same ESPN adapter and "Away at Home" matchup shape as the NBA.
// Asked for in Discussions #43; TheSportsDB's free key (league 4380) returns
// only a slice of an 82-game-per-team season.
const nhl = createGenericPromotion({
  id: 'nhl',
  name: 'NHL',
  idPrefix: 'nhl',
  source: 'espn',
  league: 'nhl',
  // Square: ESPN supplies team logos, not event photography (see NFL above).
  posterShape: 'square',
  promotionAliases: ['NHL', 'National Hockey League'],
  teamAliasPreset: 'nhl',
  searchTitleTemplates: [
    '{promotion} {date_dotted} {name}',
    '{name} {date_dotted}',
  ...
  relevanceKeywords: ['nhl', 'national hockey league'],
  exclusionKeywords: ['nhl network', 'nhl tonight', 'highlights', 'condensed',
    'all access', 'road to the'],
  requireDateInTitle: true,
});
nhl.isCustom = false;
nhl.uuMaxQueries = 6;
...
const discoveredPromotions = Object.keys(releaseIngest.SPORTS).map((sport) => {
  ...
  const promotion = createGenericPromotion({
    ...
    sport,
    posterShape: 'landscape',
    // One bundled mark per sport, drawn by scripts/make-discovered-art.py.
    // All seven used to fall back to the SSS banner, which put seven visually
    // identical rows on the home screen with nothing to tell them apart.
    //
    // 0.90.6 — through brandedPoster, not as a bare '/assets/...' path. A
    // Stremio meta is fetched by the client, not by a browser sitting on this
    // page, so a root-relative URL resolves against the client's own origin
    // and 404s. Every tile rendered empty. brandedPoster prefixes PUBLIC_URL
    // and checks the file is actually shipped, which is why the rest of the
    // bundled art never had this problem.
    poster: brandedPoster('discovered-' + sport + '.png', ''),
    fanart: brandedPoster('discovered-' + sport + '.png', ''),
  ...
  // Built-in Usenet joined later and was missed: the Client check (2026-09-25)
  // showed it spending the full 19s budget on every Discovered event for no
  // rows, since no indexer names a release the way the site's title does.
  promotion.disabledPipelines = ['uu', 'easynews', 'diy-usenet'];
...
const HARDCODED = [ufc, one, wwe, aew, ...wrestlingWeeklyShows, f1, boxing, motogp, matchOfTheDay, championsLeague, mlb, nfl, nba, nhl]
  .concat(footballLeagues).concat(discoveredPromotions)
  .filter((promotion) => !releaseIngest.RETIRED_PROMOTIONS.has(promotion.id));
...
function loadCustomPromotions() {
  ...
  for (const s of specs) {
    if (releaseIngest.RETIRED_PROMOTIONS.has(s.id)) continue;
...
function applyPromotionMatchingOverrides(items) {
  for (const promotion of items) {
    ...
    let saved = promotionOverrides.find(promotion.id);
    if (!saved) continue;
    if (promotion.weeklyShow) saved = promotionOverrides.sanitizeWeekly(saved);
    ...
    promotion.isRelevantStreamTitle = function overriddenRelevance(title, event) {
      // A learned generic rule must not turn one dated weekly episode into
      // every episode, or accept a different show with the same air date.
      // The show-specific matcher is the identity gate for these catalogs.
...
function reload() {
  ...
  for (const p of custom) {
    ...
    if (p.id === 'ucl') {
      ...
      delete p.torrentSearchTitles;   // see the note on mlb.searchTitles
  ...
  const startDates = require('./settings').getPromotionMetadataStartDates();
  for (const promotion of all) {
    promotion.metadataStartDate = startDates[promotion.id]
      || promotion.defaultMetadataStartDate || null;
  }
  rebuildIndexes();
}

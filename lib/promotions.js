...
function dateMatchesEvent(title, event, promotionId) {
  ...
  // MLB series play on consecutive days. A neighbouring date identifies
  // another game, not a timezone variant of this fixture's official date.
  const tolerance = promotionId === 'mlb' || /^mlb:/.test(String(event.id || '')) || event.source?.type === 'mlb' ? 0 : 1;

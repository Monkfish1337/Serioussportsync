...
function teamPresent(title, forms, contextTokens) {
  const normalise = (value) => teamIdentities.foldAscii(String(value || ''))
    .toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim().replace(/\s+/g, ' ');
  const condense = (value) => value.replace(TEAM_CONNECTOR_RE, ' ').replace(/\s+/g, ' ').trim();
  ...
  return {
    ...
    isRelevantStreamTitle(title, event) {
      ...
      if (suppliedHome.length && suppliedAway.length) {
        ...
        if (!teamPresent(title, homes, contextTokens)) return { ok: false, reason: 'no-home-team-alias' };
        if (!teamPresent(title, aways, contextTokens)) return { ok: false, reason: 'no-away-team-alias' };
        ...
        if (split) {
          ...
          const homeHit = teamPresent(title, homes, contextTokens) || homes.some((h) => aliasBoundaryMatch(t, h));
          const awayHit = teamPresent(title, aways, contextTokens) || aways.some((a) => aliasBoundaryMatch(t, a));

  ...
  return {
    ...
    isRelevantStreamTitle(title, event) {
      ...
      if (suppliedHome.length && suppliedAway.length) {
        ...
        if (!homes.some((h) => aliasBoundaryMatch(t, h))) return { ok: false, reason: 'no-home-team-alias' };
        if (!aways.some((a) => aliasBoundaryMatch(t, a))) return { ok: false, reason: 'no-away-team-alias' };
...
const HARDCODED = [ufc, one, wwe, aew, f1, boxing, motogp, matchOfTheDay, manUnited, championsLeague, mlb, nfl, nba];

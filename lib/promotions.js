...
//   * and the title's year or season span must still match the event, so last
...
function weekMatchesEvent(title, event) {
  ...
  let found = false, m;
  while ((m = TITLE_WEEK_RE.exec(text)) !== null) {
    if (parseInt(m[1], 10) === week) { found = true; break; }
  }
  if (!found) return false;
  return yearMatchesEvent(text, event);
}
...
const ufc = {
  ...
  // A numbered PPV is the most-uploaded thing UFC produces; a Fight Night
  // prelim card is among the least. The check has picked the latter twice now
  // and reported the resulting miss as a problem with the user's setup.
  checkWorthiness(event) {
    const kind = this.classify((event && event.name) || '');
    if (kind === 'contender-series') return -1;
    if (kind === 'ppv') return 2;
    if (kind === 'fight-night' || kind === 'ufc-on-network') return 0;
    return 0;
  },
...
const aew = {
  ...
  idPrefix: 'aew',
  enabled: true,
  // AEW's upcoming cards are unreachable through TheSportsDB's list endpoints
  // on a free key, so they are fetched by name instead, keyed off this league
  // id. AEW is the only shipped league that needs this — see the measurements
  // in lib/tsdb-known-events.js.
...
const f1 = {
  ...
  // Which of these are worth running "Check it works" against.
  //
  // The check reported "no streams" on a healthy install because it spent an
  // attempt on Italian Grand Prix Practice 3 — a Friday session almost nobody
  // uploads. A race is the thing that gets released; testing and practice are
  // the things that do not, and a miss on one of those says nothing about the
  // user's setup, which is the only question the check exists to answer.
  checkWorthiness(event) {
    const session = f1Session(event && event.name);
    if (session === 'practice' || session === 'testing') return -1;
    if (session === 'race') return 2;
    if (session === 'sprint') return 1;
    return 0;
  },
...
const motogp = {
  ...
  // Same reasoning as Formula 1 above: the race is what gets released.
  checkWorthiness(event) {
    const session = motogpSession(event && event.name);
    if (session === 'practice' || session === 'testing') return -1;
    if (session === 'race') return 2;
    if (session === 'sprint') return 1;
    return 0;
    ...
    for (const needle of variants) {
      ...
      for (const hay of haystacks) {
        ...
        for (let i = 0; i + needleTokens.length <= tokens.length; i += 1) {
          ...
          // opponent.
          if (/^w(?:k|eek)?\d{1,2}$/.test(before) || before === 'week') return true;
    ...
    searchTitles(event) {
      ...
      if (plainSplit && date && teamAliasLookup) {
        ...
        if (WEEK_NUMBERED_RELEASES.has(String(presetName || '').toLowerCase())
          ...
          if (span) weekQueries.push(leaguePrefix + ' ' + span + ' ' + padded + ' ' + pair);
          weekQueries.push(leaguePrefix + ' ' + padded + ' ' + pair);
          // Unpadded, because "W4" and "W04" are both in use and one extra
          // query is cheaper than missing half the catalogue.
          if (weekNumber < 10) weekQueries.push(leaguePrefix + ' W' + weekNumber + ' ' + pair);
      ...
      return Array.from(new Set([].concat(Array.from(priority), Array.from(out)))).slice(0, 60);
    },
    isRelevantStreamTitle(title, event) {
      if (!title) return { ok: false, reason: 'no-title' };
      const t = title.toLowerCase();
...
const HARDCODED = [ufc, one, wwe, aew, f1, boxing, motogp, matchOfTheDay, championsLeague, mlb, nfl, nba, wnba, collegeFootball]

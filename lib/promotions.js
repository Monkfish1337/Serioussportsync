...
// Champions League ships ready to use with API-Football's stable competition
// ID. Existing installations retain all current promotions and assignments;
// this is a new catalog whose source can still be changed from Metadata.
...
championsLeague.uuMaxQueries = 8;
...
championsLeague.searchTitles = function championsLeagueSearchTitles(event) {
  ...
  // Provider full names first: this is the most deterministic query and fixes
  // the previous shortName-only behaviour. Curated UCL aliases follow via the
  // generic generator for PSG, Bayern Munich, Inter Milan, etc.
  ...
  return Array.from(new Set(precise.concat(championsLeagueBaseSearchTitles(event)))).slice(0, 60);
};
championsLeague.torrentSearchTitles = function championsLeagueTorrentSearchTitles(event) {
  ...
  return queries.length ? [queries[0].replace(/\s+vs\s+/i, ' Vs ')] : [];
};
championsLeague.genres = function championsLeagueGenres() {
  return ['Sports', 'Football', 'UEFA Champions League'];
};

const HARDCODED = [ufc, one, wwe, aew, f1, boxing, motogp, matchOfTheDay, manUnited, championsLeague];
...
function reload() {
  ...
  for (const p of HARDCODED) {
    if (p.id === 'ucl' && customIds.has('ucl')) continue;
    all.push(p);
  }

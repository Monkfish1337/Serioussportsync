...
const boxing = {
  ...
  // 0.25.1: posterShape 'square' to match the league badge's actual shape.
  // TSDB's Boxing badge is a square BOXING-wordmark logo — stretching it
  // into a landscape tile center-crops to just "VG"/"NG" (the tail of the
  // word). A square tile renders the whole badge cleanly. Other promotions
  // stay landscape because they have branded landscape posters in public/.
  // When the user drops a custom boxing-upcoming.jpg shaped for landscape,
  // they can flip this back to 'landscape'.
  posterShape: 'square',
  // Per-event TSDB posters/thumbs are fighter portraits at varied aspect
  // ratios — they tile badly. Force the promotion's branded default for
  // every event (same knob F1 uses).
  useDefaultArt: true,
  defaults: {

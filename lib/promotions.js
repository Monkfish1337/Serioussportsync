  ...
  return {
    ...
    isRelevantStreamTitle(title, event) {
      ...
      if (suppliedHome.length && suppliedAway.length) {
        ...
        const sideHit = (forms) => forms.some((form) =>
          aliasBoundaryMatch(t, form) || plainTeamMatch(title, form));
        if (!sideHit(homes)) return { ok: false, reason: 'no-home-team-alias' };
        if (!sideHit(aways)) return { ok: false, reason: 'no-away-team-alias' };
        ...
        if (split) {
          ...
          const homeHit = homes.some((h) => aliasBoundaryMatch(t, h) || plainTeamMatch(title, h));
          const awayHit = aways.some((a) => aliasBoundaryMatch(t, a) || plainTeamMatch(title, a));

    ...
    searchTitles(event) {
      ...
      if (plainSplit) {
        ...
        if (homeCodes.length && awayCodes.length) {
          ...
          const leaguePrefix = (leagueAliasList[0] || name || '').trim();
          for (const home of homeCodes) {
            for (const away of awayCodes) {
              const pair = home + '-' + away;
              if (leaguePrefix) out.add((leaguePrefix + ' ' + pair).trim());
              if (leaguePrefix && dmyShort) out.add((leaguePrefix + ' ' + pair + ' ' + dmyShort).trim());
              if (date) out.add((pair + ' ' + date).trim());
              out.add(home + ' vs ' + away);
            }
          }
        }
      }

...
const boxing = {
  ...
  posterShape: 'landscape',
  // Boxing per-event TSDB posters/thumbs are fighter portraits at varied
  // aspect ratios — they tile badly. Force the promotion's branded default
  // for every event (same knob F1 uses). 0.25.0: the *fallback* URL is now
  // the square league badge instead of the wide banner — banners are
  // panoramic strips that crop badly in landscape tiles, badges letterbox
  // cleanly. Drop a public/boxing-upcoming.jpg later for a custom branded
  // card; brandedPoster() picks it up automatically when present.
  useDefaultArt: true,
  defaults: {

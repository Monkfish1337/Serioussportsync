...
function mergeCollections(existing, incoming, mode) {
  ...
  // Move only collections actually added or updated by this operation.
  const pinned = ours.filter((item) => item && item.pinToTop === true &&
    (mode !== 'add' || !current.some((old) => old && old.id === item.id)));
  for (let i = pinned.length - 1; i >= 0; i -= 1) {
    const at = next.findIndex((item) => item && item.id === pinned[i].id);
...
function panel() {
  return ''
    ...
    // A <select>, not radios. The radios this replaced were `.sw` labels
    // without the `<i>` element that class styles: `.sw input` is
    // position:absolute;opacity:0, so the control was invisible AND
    // unclickable, and every push silently used the checked default. Merge was
    // the only mode reachable. A native select cannot fail that way.
    + '<label class="f"><span>How to apply it</span><select class="t" id="nuvio-mode">'
    + MODES.map((mode) => '<option value="' + escapeHtml(mode.id) + '"'
      + (mode.id === 'merge' ? ' selected' : '') + '>' + escapeHtml(mode.label) + '</option>').join('')
    + '</select></label>'
    + '</div>'
    + '<p class="row-sub" id="nuvio-mode-detail" style="margin:8px 0 0">'
    + escapeHtml(MODES[0].detail) + '</p>'
    ...
    + '<label class="sw" style="margin-top:6px"><input type="checkbox" id="nuvio-pin-top"'
    + (require('./nuvio-collection-settings').load().collection.pinToTop ? ' checked' : '') + '><i></i>'
    + '<b>Pin to top<small>Place this collection at the top of the profile’s collection list.</small></b></label>'
...
function clientScript(collectionsUrl) {
  return '(' + String(function (api, anonKey, exportUrl, modeDetail) {
    var token = null;
    var profiles = [];
    ...
    var MODE_DETAIL = modeDetail;
    el('nuvio-mode').addEventListener('change', function () {
      el('nuvio-mode-detail').textContent = MODE_DETAIL[this.value] || '';
    });

    // Enter had nowhere to go. These inputs sit inside the Configure form, so
    // pressing it either did nothing or submitted the whole five-step form and
    // navigated away from a half-finished sign-in. Every other login on the
    // internet submits on Enter; this one now does too.
    ['nuvio-email', 'nuvio-password'].forEach(function (id) {
      el(id).addEventListener('keydown', function (event) {
        if (event.key !== 'Enter' && event.keyCode !== 13) return;
        event.preventDefault();
        event.stopPropagation();
        el('nuvio-signin-btn').click();
      });
    });
    ...
    el('nuvio-push-btn').addEventListener('click', function () {
      var mode = el('nuvio-mode').value || 'merge';
      ...
      var pinToTop = document.getElementById('nuvio-pin-top').checked;
      ...
      Promise.all([
        fetch(exportUrl, { headers: { Accept: 'application/json' } }).then(function (r) { return r.json(); })
          .then(function (items) { return items.map(function (item) {
            return Object.assign({}, item, { pinToTop: pinToTop });
          }); }),
    ...
    + JSON.stringify(MODES.reduce((map, mode) => Object.assign(map, { [mode.id]: mode.detail }), {}))
    + ');';
}

...
async function discoverUsenetCandidates({ promo, event, titles, uuConfig, log }) {
  if (!uuConfig) return [];
  const searchOut = await uu.search(titles, uuConfig, {
    log, maxQueries: promo && promo.uuMaxQueries,
  });
  if (!searchOut || !searchOut.results || searchOut.results.length === 0) return [];
...
async function pipelineNzbdav({ event, config, uuConfig, getUsenetCandidates, urlCtx, log }) {
  if (!nzbdavPlayback.isConfigured(config)) {
    log('nzbdav: DIY provider disabled or incomplete - skipping');
    return [];
  }
  if (!uuConfig) {
    log('nzbdav: UU manifest is currently required for title discovery');
    return [];
  }
  const rows = [];
...
async function handleStream(params) {
  ...
  let usenetCandidatesPromise = null;
  const getUsenetCandidates = () => {
    if (!usenetCandidatesPromise) {
      usenetCandidatesPromise = discoverUsenetCandidates({ promo, event, titles, uuConfig, log });
    }
    return usenetCandidatesPromise;
  };
  const [torboxRows, uuRows, nzbdavRows, easynewsRows] = await Promise.all([
    runOrSkip('torbox',   () => pipelineTorrentTorbox({ promo, event, titles: torrentTitles, torboxKey, urlCtx, log })),
    runOrSkip('uu',       () => pipelineUsenetUltimate({ event, uuConfig, getUsenetCandidates, log })),
    runOrSkip('diy-usenet', () => pipelineNzbdav({ event, config: nzbdavConfig, uuConfig, getUsenetCandidates, urlCtx, log })),
  ...
  const seen = new Set();
  const merged = [];
  for (const set of [torboxRows, uuRows, nzbdavRows, easynewsRows]) {
...
async function resolveNzbdav({ eventId, infoHash, creds, userId, log }) {
  ...
  if (!pending) {
    pending = (async () => {
      const playable = await nzbdavPlayback.resolveCandidate(config, found.candidate.payload);

...
async function discoverTorrentCandidates({ promo, event, titles, log, discoveryBudgetMs }) {
  ...
  if (!companionCfg.url && !(prowlarrCfg.url && prowlarrCfg.apiKey) && !bitmagnetCfg.url) {
    if (!sportVideoEnabled) log('torrent discovery: no companion, direct Prowlarr or Bitmagnet configured; Sport-Video disabled');
    return direct;
  }

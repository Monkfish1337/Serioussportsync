...
async function discoverTorrentCandidates({ promo, event, titles, log, discoveryBudgetMs, onlySources }) {
  const companionCfg = settings.getCompanion();
  const prowlarrCfg = settings.getProwlarr();
  const bitmagnetCfg = settings.getBitmagnet();
  ...
  const out = await cachedProviderSearch({
    ...
    producer: async () => {
      ...
      // running and its answer is discarded — there is nothing left to give it
      // to, and the next request re-asks anyway.
      ...
      if (useProwlarr) {
        ...
        // promotion can make.
        const prowlarrTitles = require('./discovery-plan').selectTorrentQueries(titles, event, promo,
          Math.max(1, settings.getDiscoveryTiming().prowlarrMaxQueries));
        log('prowlarr: searching ' + prowlarrTitles.length + ' of ' + titles.length
          + ' title variant(s)');
        tasks.push(withinBudget('prowlarr',
          prowlarr.multiSearch(prowlarrTitles, {
            log, detailed: true, deadlineMs: discoveryBudgetMs,
            timeoutMs: settings.getDiscoveryTiming().prowlarrQueryTimeoutMs,
          }).catch((err) => {
      ...
      if (useBitmagnet) {
        tasks.push(withinBudget('bitmagnet',
          bitmagnet.multiSearch(titles, { log, detailed: true, deadlineMs: discoveryBudgetMs }).catch((err) => {
...
async function pipelineTorrentTorbox({ promo, event, titles, torboxKey, urlCtx, log, discoveryBudgetMs, allowConfirmed = true, onlySources, fastResponse = false }) {
  ...
  if (fastResponse && confirmed.length) {
    ...
    if (ready.length) {
      pipelineTorrentTorbox({promo, event, titles, torboxKey, urlCtx, log,
        discoveryBudgetMs, allowConfirmed, onlySources, fastResponse: false})
  ...
  try {
    discovered = await discoverTorrentCandidates({
      promo, event, titles, log, discoveryBudgetMs, onlySources });
  } catch (error) {
...
async function handleStream(params) {
  ...
  const [torboxRows, uuRows, nzbdavRows, nntpRows, easynewsRows] = await require('./discovery-plan').collectPipelineRows([
    runOrSkip('torbox',   () => pipelineTorrentTorbox({ promo, event, titles: torrentTitles, torboxKey, urlCtx, log, discoveryBudgetMs, fastResponse: timing.fastResponseGraceMs > 0 })),

...
//
//   Pipeline B (Built-in Usenet): event titles -> user's native indexer
...
//   - At least one of `torboxApiKey`, a configured Built-in Usenet pipeline,
//     or `easynewsUsername`+`easynewsPassword` on /account.
//
// Admin requirements:
...
async function handleStream(params) {
  ...
  // an array of pipeline names ('torbox' | 'diy-usenet' | 'easynews') that
  // should be skipped for events from this promotion.
  ...
  const [torboxRows, nntpRows, easynewsRows] = await require('./discovery-plan').collectPipelineRows([
  ...
  const merged = mergePipelineRows([
    ['torbox', torboxRows], ['nntp', nntpRows], ['easynews', easynewsRows],
  ]);

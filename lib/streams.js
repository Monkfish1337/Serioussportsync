...
async function cachedProviderSearch({ event, promo, provider, scope, queries, producer, log, index: suppliedIndex }) {
  ...
  if (index) {
    try {
      ...
      if (cached.hit) {
        log(provider + ': availability-index hit (' + cached.results.length + ' candidate(s))');
...
async function discoverTorrentCandidates({ promo, event, titles, log, discoveryBudgetMs }) {
  ...
  const scope = index ? index.scopeFingerprint('torrent', {
  ...
  const out = await cachedProviderSearch({
    ...
    producer: async () => {
      ...
      return { ok: true, results: (await Promise.all(tasks)).flat() };
    },
  });
  const lists = [out.results || []];
...
async function pipelineTorrentTorbox({ promo, event, titles, torboxKey, urlCtx, log, discoveryBudgetMs }) {
  if (!torboxKey) { log('torbox: no user key — skipping pipeline A'); return []; }

  // 1. Search the configured torrent sources. Both are read-only.
  const candidates = await discoverTorrentCandidates({ promo, event, titles, log, discoveryBudgetMs });
  ...
  if (unknown.length) {
    ...
    for (const candidate of unknown) {
      ...
      if (index) index.observe({
        ...
        state: isCached ? 'cached' : 'unavailable', candidate,
      });
    }
  } else {
...
function finalizeUsenetCandidates(label, candidates, promo, event, log) {
  ...
  return relevant.slice(0, MAX_ROWS);
}
...
async function pipelineNzbdav({ event, config, getUsenetCandidates, urlCtx, log }) {
  if (!nzbdavPlayback.isConfigured(config)) {
    log('nzbdav: DIY provider disabled or incomplete - skipping');
    return [];
  }
  const rows = [];
  for (const candidate of await getUsenetCandidates()) {
...
async function pipelineNativeNntp({ event, config, getUsenetCandidates, urlCtx, log }) {
...
// Background-only discovery. It runs the same provider searches and TorBox
// cache checks as /stream, but never creates playback candidates or resolves
// content. Search TTLs and credential-scope fingerprints are shared with the
// interactive path, so a later click receives an immediate index hit.
async function prefetchAvailability(params) {
  ...
  const discoveryBudgetMs = Math.max(1000,
    parseInt(process.env.AVAILABILITY_WARM_PROVIDER_TIMEOUT_MS || '15000', 10) || 15000);
  const tasks = [];

  if (!disabled.has('torbox')) {
  ...
  if (uuConfig && (!disabled.has('uu') || (!disabled.has('diy-usenet') && diyUsesUu))) {
    const index = availabilityIndex(log);
    const scope = index ? index.scopeFingerprint('uu', uuConfig) : 'uncached';
    tasks.push({ provider: 'uu', promise: cachedProviderSearch({
      event, promo, provider: 'uu', scope, queries: titles, log,
      producer: () => uu.search(titles, uuConfig, {
  ...
  if (!disabled.has('diy-usenet') && usenetIndexer.isConfigured(nativeConfig)) {
    const index = availabilityIndex(log);
    const scope = index ? index.scopeFingerprint('native-indexer', nativeConfig) : 'uncached';
    tasks.push({ provider: 'native-indexer', promise: cachedProviderSearch({
      event, promo, provider: 'native-indexer', scope, queries: titles, log,
      producer: () => usenetIndexer.search(titles, nativeConfig, {
        log, maxQueries: promo.uuMaxQueries, timeoutMs: discoveryBudgetMs,
      }),
    }) });
  }
  ...
  const easynewsCreds = userConfig.easynewsUsername && userConfig.easynewsPassword
    ...
    : null;
  if (!disabled.has('easynews') && easynewsCreds && easynewsCreds.username) {
    const index = availabilityIndex(log);
    const scope = index ? index.scopeFingerprint('easynews', easynewsCreds) : 'uncached';
    tasks.push({ provider: 'easynews', promise: cachedProviderSearch({
      event, promo, provider: 'easynews', scope, queries: titles, log,
      producer: () => easynews.multiSearch(titles, Object.assign({ log }, easynewsCreds)),
    }) });
  }
  ...
  settled.forEach((result, index) => {
    if (result.status === 'rejected') errors.push(tasks[index].provider + ': ' + result.reason.message);
    else if (result.value && result.value.ok === false) {
      errors.push(tasks[index].provider + ': ' + String(result.value.error || 'search-failed'));
    }
  });
  return { ok: errors.length === 0, errors, providers: tasks.map((task) => task.provider) };
}
...
async function handleStream(params) {
    ...
    if (!diyCandidatesPromise) diyCandidatesPromise = (async () => {
      ...
      if (usenetIndexer.isConfigured(nativeUsenetConfig)) {
        ...
        searches.push(cachedProviderSearch({
          event, promo, provider: 'native-indexer', scope, queries: titles, log,
          producer: () => usenetIndexer.search(titles, nativeUsenetConfig, {
            log, maxQueries: promo && promo.uuMaxQueries,
  ...
  const [torboxRows, uuRows, nzbdavRows, nntpRows, easynewsRows] = await Promise.all([
    ...
    runOrSkip('diy-usenet', () => pipelineNzbdav({ event, config: nzbdavConfig, getUsenetCandidates: getDiyCandidates, urlCtx, log })),
    runOrSkip('native-nntp', () => pipelineNativeNntp({ event, config: nntpConfig, getUsenetCandidates: getDiyCandidates, urlCtx, log })),
  ...
  const seen = new Set();
  const merged = [];
  for (const set of [torboxRows, uuRows, nzbdavRows, nntpRows, easynewsRows]) {
    for (const row of set) {
      const scope = row && row._sssDedupeScope ? row._sssDedupeScope + ':' : '';
      const key = row && row.title ? scope + row.title.split('\n')[0] : null;
...
async function resolveNzbdav({ eventId, infoHash, creds, userId, log }) {
  ...
  const index = availabilityIndex(log);
  const scope = index ? index.scopeFingerprint('nzbdav', {
...
async function resolveNativeNntp({ eventId, infoHash, creds, userId, log }) {
  ...
  if (!found.ok) return { ok: false, error: 'candidate-' + found.reason };
  const index = availabilityIndex(log);
  const scope = index ? index.scopeFingerprint('nntp', {

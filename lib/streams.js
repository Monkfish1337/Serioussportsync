...
async function handleStream(params) {
  ...
  // OPTIMISTIC: advertise one row per provider per candidate WITHOUT touching
  // any debrid. We don't know yet which provider has it cached — resolution
  // (and the cached/not-cached truth) happens on click. This is the fix for
  // search-time account pollution + cross-provider stalls.
  //
  // RD-only pre-filter (0.22.1): skip the RD row for candidates whose title
  // matches a known RD-blocked keyword OR whose hash is in the persistent 451
  // denylist (populated by realdebrid.js when RD returns 451 at resolve time).
  // TB/PM rows are unaffected — RD's keyword filter is RD-specific.
  ...
  for (const r of shown) {
    ...
    for (const p of providers) {
      const isRd = (p.code || '').toUpperCase() === 'RD';
      if (isRd && rdKwHit) { rdSkippedKw++; continue; }
      if (isRd && rdDenyHit) { rdSkippedDeny++; continue; }
      streams.push({

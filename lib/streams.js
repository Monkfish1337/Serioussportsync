...
async function handleStream(params) {
  ...
  let rdSkippedKw = 0, rdSkippedDeny = 0, rdPosCached = 0;
  let tbSkippedVerified = 0, tbSkippedDeny = 0, tbVerifiedCached = 0, tbPosCached = 0;
  let pmSkippedVerified = 0, pmSkippedDeny = 0, pmVerifiedCached = 0, pmPosCached = 0;
  ...
  for (const r of shown) {
    ...
    for (const p of providers) {
      ...
      // 0.24.0: positive cache check is the strongest signal — a previous
      // successful resolve for this (hash, provider) trumps everything else
      // (denylist, warmer "not cached" — those signals can be stale).
      const positive = hashLc && posCached.has(hashLc + ':' + codeLc);

      // ---- RD ----
      if (code === 'RD') {
        if (positive)   { rdPosCached++; /* advertise below */ }
        else if (rdKwHit)   { rdSkippedKw++;   continue; }
        else if (rdDenyHit) { rdSkippedDeny++; continue; }
      }
      // ---- TB: positive → warmer-verified → denylist fallback ----
      else if (code === 'TB') {
        if (positive) { tbPosCached++; /* advertise below */ }
        else if (verif && verif.tb === false) { tbSkippedVerified++; continue; }
        else if (verif && verif.tb === true)  { tbVerifiedCached++; /* advertise below */ }
        else if (hashLc && tbDenied.has(hashLc)) { tbSkippedDeny++; continue; }
      }
      // ---- PM: positive → warmer-verified → denylist fallback ----
      else if (code === 'PM') {
        if (positive) { pmPosCached++; /* advertise below */ }
        else if (verif && verif.pm === false) { pmSkippedVerified++; continue; }
        else if (verif && verif.pm === true)  { pmVerifiedCached++; /* advertise below */ }
        else if (hashLc && pmDenied.has(hashLc)) { pmSkippedDeny++; continue; }
      }
      ...
      streams.push({
        ...
        title: (r.title || r.infoHash)
          ...
          + (sizeStr ? ' | 💾 ' + sizeStr : '')
          + (r.indexer ? '\n[' + r.indexer + ']' : ''),
        url: resolveBase + (p.code || '').toLowerCase()
          + '/' + encodeURIComponent(id)
          + '/' + encodeURIComponent(r.infoHash),
        behaviorHints: {
          bingeGroup: 'sport-' + (p.code || '').toLowerCase(),
  ...
  if (rdPosCached || rdSkippedKw || rdSkippedDeny) {
    log('  RD filter: ' + rdPosCached + ' positive-cached'
      + ', skipped ' + (rdSkippedKw + rdSkippedDeny)
      + ' (' + rdSkippedKw + ' keyword, ' + rdSkippedDeny + ' denylist)');
  }
  if (tbPosCached || tbSkippedVerified || tbSkippedDeny || tbVerifiedCached) {
    log('  TB filter: ' + tbPosCached + ' positive-cached, '
      + tbVerifiedCached + ' verified-cached'
      + ', skipped ' + (tbSkippedVerified + tbSkippedDeny)
      + ' (' + tbSkippedVerified + ' verified-not-cached, ' + tbSkippedDeny + ' denylist)');
  }
  if (pmPosCached || pmSkippedVerified || pmSkippedDeny || pmVerifiedCached) {
    log('  PM filter: ' + pmPosCached + ' positive-cached, '
      + pmVerifiedCached + ' verified-cached'
      + ', skipped ' + (pmSkippedVerified + pmSkippedDeny)
      + ' (' + pmSkippedVerified + ' verified-not-cached, ' + pmSkippedDeny + ' denylist)');
  }
  log('  advertising ' + streams.length + ' row(s) from ' + shown.length
    + ' release(s) × ' + providers.length + ' provider(s) — resolve on play');

...
function selectProviderQueries(titles, event, limit) {
  ...
  return Array.from(new Set((titles || []).map((value) => String(value || '').trim()).filter(Boolean)))
    .map((title, index) => {
      const lower = title.toLowerCase();
      let score = dateForms.some((form) => form && lower.includes(form.toLowerCase())) ? 100 : 0;
...
async function prefetchAvailability(params) {
  ...
  const torrentTitles = typeof promo.torrentSearchTitles === 'function'
    ...
    : titles;
  const providerTitles = selectProviderQueries(titles, event, promo.uuMaxQueries || 6);
...
module.exports = {
  ...
  warmTorbox,
  _test: {
    cachedProviderSearch, pipelineTorrentTorbox, pipelineEasynews,

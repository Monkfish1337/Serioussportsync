...
function torrentDiscoveryScope(index) {
  ...
  // Likewise for Bitmagnet: only widen the fingerprint once it is configured,
  // so upgrading does not invalidate every cached search. It MUST be part of
  ...
  const out = await cachedProviderSearch({
    ...
    producer: async () => {
      ...
      if (useProwlarr) {
        ...
        // promotion can make.
        const prowlarrTitles = titles.slice(0,
      ...
      if (useBitmagnet) {
        tasks.push(withinBudget('bitmagnet',
          bitmagnet.multiSearch(titles, { log, detailed: true }).catch((err) => {
      ...
      if (partial) {
        log('torrent discovery: partial result (' + failed.concat(lateSources).join(', ')
          + ' did not contribute) — using it for this request but not caching it');
      }
      return {
...
async function pipelineTorrentTorbox({ promo, event, titles, torboxKey, urlCtx, log, discoveryBudgetMs, allowConfirmed = true, onlySources }) {

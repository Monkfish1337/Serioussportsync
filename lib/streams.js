...
async function discoverTorrentCandidates({ promo, event, titles, log, discoveryBudgetMs }) {
  ...
  const useCompanion = Boolean(companionCfg.url) && companionCfg.enabled !== false;
  const useProwlarr = Boolean(prowlarrCfg.url && prowlarrCfg.apiKey) && prowlarrCfg.enabled !== false;
  const useBitmagnet = Boolean(bitmagnetCfg.url) && bitmagnetCfg.enabled !== false;

...
async function handleStream(params) {
  ...
  let rdSkippedKw = 0, rdSkippedDeny = 0;
  let tbSkippedVerified = 0, tbSkippedDeny = 0, tbVerifiedCached = 0;
  let pmSkippedVerified = 0, pmSkippedDeny = 0, pmVerifiedCached = 0;
  ...
  for (const r of shown) {
    ...
    for (const p of providers) {
      ...
      // ---- RD ----
      if (code === 'RD') {
        if (rdKwHit)   { rdSkippedKw++;   continue; }
        if (rdDenyHit) { rdSkippedDeny++; continue; }
      }
      // ---- TB: warmer-verified first, denylist fallback ----
      else if (code === 'TB') {
        if (verif && verif.tb === false) { tbSkippedVerified++; continue; }
        if (verif && verif.tb === true)  { tbVerifiedCached++; /* advertise below */ }
        else if (hashLc && tbDenied.has(hashLc)) { tbSkippedDeny++; continue; }
      }
      // ---- PM: warmer-verified first, denylist fallback ----
      else if (code === 'PM') {
        if (verif && verif.pm === false) { pmSkippedVerified++; continue; }
        if (verif && verif.pm === true)  { pmVerifiedCached++; /* advertise below */ }
  ...
  if (rdSkippedKw || rdSkippedDeny) {
    log('  RD pre-filter: skipped ' + (rdSkippedKw + rdSkippedDeny)
      + ' row(s) (' + rdSkippedKw + ' keyword, ' + rdSkippedDeny + ' denylist)');
  }
  if (tbSkippedVerified || tbSkippedDeny || tbVerifiedCached) {
    log('  TB filter: ' + tbVerifiedCached + ' verified-cached'
      + ', skipped ' + (tbSkippedVerified + tbSkippedDeny)
      + ' (' + tbSkippedVerified + ' verified-not-cached, ' + tbSkippedDeny + ' denylist)');
  }
  if (pmSkippedVerified || pmSkippedDeny || pmVerifiedCached) {
    log('  PM filter: ' + pmVerifiedCached + ' verified-cached'

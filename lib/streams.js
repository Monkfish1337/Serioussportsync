...
function torrentDiscoveryScope(index) {
  ...
  return index ? index.scopeFingerprint('torrent', {
...
async function pipelineTorrentTorbox({ promo, event, titles, torboxKey, urlCtx, log, discoveryBudgetMs }) {
  ...
  // 1. Serve fresh, event-bound confirmed knowledge first. Discovery remains
  // the fallback, while play-click still revalidates the hash with TorBox.
  const index = availabilityIndex(log);
  const torboxScope = index
    ? index.scopeFingerprint('torbox', { apiKey: torboxKey }) : 'uncached';
  const confirmed = confirmedCandidates(index, {
    eventId: event.id,
    sourceProvider: 'torrent', sourceScope: torrentDiscoveryScope(index),
    availabilityProvider: 'torbox', availabilityScope: torboxScope,
    states: ['cached', 'verified'], limit: MAX_ROWS,
  }, log, 'torbox');
  const discovered = confirmed.length ? []
    : await discoverTorrentCandidates({ promo, event, titles, log, discoveryBudgetMs });
...
module.exports = {
  ...
  resolvePlay,
  _test: { cachedProviderSearch, pipelineTorrentTorbox, pipelineEasynews, mergeCandidates },
};

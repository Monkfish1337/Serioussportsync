...
async function handleStream(params) {
  ...
  // 0.26.1: per-provider opt-in auto-warm. For each provider the user has
  // ticked "Auto-warm on <provider>" for, count how many of this event's
  // candidates are known-cached on that provider (positive cache or warmer-
  // verified true). If zero, queue the top candidate on that provider — even
  // if OTHER providers have cached rows. Fits the mental model: "I ticked TB,
  // TB has nothing cached for this event, warm TB even though PM does".
  // Other providers with cached rows aren't touched.

...
async function handleStream(params) {
  ...
  const getUuSearch = () => {
    if (!uuSearchPromise) {
      uuSearchPromise = uuConfig
        ? uu.search(titles, uuConfig, { log, maxQueries: promo && promo.uuMaxQueries })
        : Promise.resolve({ ok: false, error: 'not-configured', results: [] });
    }
  ...
  const getDiyCandidates = () => {
    if (!diyCandidatesPromise) diyCandidatesPromise = (async () => {
      ...
      if (usenetIndexer.isConfigured(nativeUsenetConfig)) {
        searches.push(usenetIndexer.search(titles, nativeUsenetConfig, {

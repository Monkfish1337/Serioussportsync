...
function filterCandidates(label, results, log, promo, event) {
  ...
  if (!detailedRejections) {
    for (const [reason, count] of Object.entries(rejectReasons)) {
      ...
      if (hidden > 0) log(label + ' REJECT [' + reason + '] +' + hidden
        + ' more hidden (enable detailed rejection logging in Logs to show all)');
    }
  }
...
async function pipelineTorrentTorbox({ promo, event, titles, torboxKey, urlCtx, log, discoveryBudgetMs, allowConfirmed = true }) {
  ...
  let usedConfirmedShortcut = false;
  const recordOutcome = (matchedCount, readyCount) => {
    // A confirmed-result shortcut only contains the already-ready subset; it
    // must not overwrite the funnel recorded by the full discovery result.
    if (!index || usedConfirmedShortcut) return;
    try {
  ...
  usedConfirmedShortcut = confirmed.length > 0;

...
async function handleStream(params) {
  ...
  // 0.25.1: build rows in a small helper so we can run it twice — strict
  // first (warmer verdict respected), then relaxed (verified-not-cached
  // treated as 'unknown' → fall through to optimistic display). The relaxed
  // pass only happens if the strict one yields zero rows, so we never water
  // down quality when verified-cached options exist.
  function buildRows(relaxed) {
    const out = [];
    const c = {
      rdSkippedKw: 0, rdSkippedDeny: 0, rdPosCached: 0,
      tbSkippedVerified: 0, tbSkippedDeny: 0, tbVerifiedCached: 0, tbPosCached: 0,
      pmSkippedVerified: 0, pmSkippedDeny: 0, pmVerifiedCached: 0, pmPosCached: 0,
    };
    for (const r of shown) {
      ...
      for (const p of providers) {
        ...
        // ---- TB: positive → warmer-verified → denylist fallback ----
        else if (code === 'TB') {
          if (positive)                          { c.tbPosCached++; /* advertise below */ }
          else if (verif && verif.tb === false)  {
            if (!relaxed) { c.tbSkippedVerified++; continue; }
            // relaxed pass: ignore the warmer verdict, advertise optimistically
          }
        ...
        // ---- PM: positive → warmer-verified → denylist fallback ----
        else if (code === 'PM') {
          if (positive)                          { c.pmPosCached++; /* advertise below */ }
          else if (verif && verif.pm === false)  {
            if (!relaxed) { c.pmSkippedVerified++; continue; }
          }
  ...
  // Strict pass first — honour warmer verdicts.
  let { rows: streams, c: cc } = buildRows(false);
  let usedRelaxed = false;
  if (streams.length === 0 && shown.length > 0) {
    // Strict pass yielded nothing. Retry without the verified-not-cached
    // skip so the user gets optimistic rows to try (denylist + keyword
    // filter still apply). Click-time failures will be picked up by the
    // denylists as a safety net.
    log('  no rows survived strict warmer-verified filter — retrying relaxed (optimistic fallback)');
    const r2 = buildRows(true);
    streams = r2.rows;
    cc = r2.c;
    usedRelaxed = true;
  }

  // 0.26.0: opt-in auto-warm-on-miss. When the strict pass produced zero
  // verified-cached rows (i.e. nothing in this event is known-cached on any
  // provider) AND the user has ticked at least one "Auto-warm on <provider>"
  // checkbox on their account page, queue the top candidate on those
  // providers in the background and surface a 🔥 Warming pseudo-row so the
  // user knows to come back. The per-user checkbox is the ONLY gate — no
  // server-wide env var required, no surprise additions for users who
  // haven't opted in.
  const userAutoCache = (creds && creds.autoCache) || {};
  const warmProviders = providers.filter((p) => {
    const code = (p.code || '').toLowerCase();
    return !!p.warmCache && !!userAutoCache[code];
  });
  if (usedRelaxed && warmProviders.length > 0 && shown.length > 0) {
    const topResult = shown[0]; // already sorted size-desc
    const warmCodes = warmProviders.map((p) => p.code).join(', ');
    log('  auto-warm: 0 verified-cached, queuing ' + topResult.infoHash
      + ' on ' + warmCodes + ' (per-user opt-in)');
    ...
    Promise.allSettled(warmProviders.map((p) =>
      Promise.resolve()
    ...
    // Surface a placeholder row at the top so the user sees what's happening.
    const sizeStr = humanSize(topResult.size);
    streams.unshift({
      name: '🔥 Warming on ' + warmCodes,
      title: 'Added the top candidate to your ' + warmCodes + ' library.\n'
        + 'Try again in 30-60 seconds — verification picks it up on the next /stream.\n'
  ...
  if (cc.tbPosCached || cc.tbSkippedVerified || cc.tbSkippedDeny || cc.tbVerifiedCached) {
    log('  TB filter: ' + cc.tbPosCached + ' positive-cached, '
      + cc.tbVerifiedCached + ' verified-cached'
      + ', skipped ' + (cc.tbSkippedVerified + cc.tbSkippedDeny)
      + ' (' + cc.tbSkippedVerified + ' verified-not-cached, ' + cc.tbSkippedDeny + ' denylist)');
  }
  if (cc.pmPosCached || cc.pmSkippedVerified || cc.pmSkippedDeny || cc.pmVerifiedCached) {
    log('  PM filter: ' + cc.pmPosCached + ' positive-cached, '
      + cc.pmVerifiedCached + ' verified-cached'
      + ', skipped ' + (cc.pmSkippedVerified + cc.pmSkippedDeny)
      + ' (' + cc.pmSkippedVerified + ' verified-not-cached, ' + cc.pmSkippedDeny + ' denylist)');
  }
  log('  advertising ' + streams.length + ' row(s) from ' + shown.length
    + ' release(s) × ' + providers.length + ' provider(s)'
    + (usedRelaxed ? ' (relaxed fallback — warmer found nothing cached)' : '')
    + ' — resolve on play');

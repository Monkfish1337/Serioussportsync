...
//     -> noise + relevance filter -> UU playback rows.
//
//   Pipeline B2 (Built-in Usenet): event titles -> user's native indexer
//     (Newznab/NZBHydra/Prowlarr) -> noise + relevance filter -> native NNTP
//     playback rows. One search source, one playback backend — no helper
//     container.
//
...
//   - At least one of `torboxApiKey`, `uuManifestUrl`, a configured
//     Built-in Usenet pipeline, or `easynewsUsername`+`easynewsPassword`
//     on /account.
//
// Admin requirements:
...
// Refresh after a database serve.
//
// A viewer served from stored results without a live search gets one on
// request: the Nuvio/Stremio Refresh button re-sends the same stream request,
// and a repeat for the same account and event inside the window is read as
// that press. One press buys one live search; the mark is consumed, so the
// automatic re-request a client makes on reopening the page much later does
// not trigger one. LIVE_REFRESH_WINDOW_SECONDS=0 turns this off.
const STORED_SERVES = new Map();
function liveRefreshWindowMs() {
  const seconds = Number(process.env.LIVE_REFRESH_WINDOW_SECONDS);
  return (Number.isFinite(seconds) && seconds >= 0 ? seconds : 30) * 1000;
}
function noteStoredServe(key, at = Date.now()) {
  if (!key || !liveRefreshWindowMs()) return;
  STORED_SERVES.delete(key);
  STORED_SERVES.set(key, at);
  // Insertion-ordered: drop the oldest once the map is larger than any
  // plausible number of open stream pages.
  while (STORED_SERVES.size > 5000) STORED_SERVES.delete(STORED_SERVES.keys().next().value);
}
function takeRefreshPress(key, now = Date.now()) {
  const at = key && STORED_SERVES.get(key);
  if (!at) return false;
  STORED_SERVES.delete(key);
  return now - at <= liveRefreshWindowMs();
}
...
// gets about 7.5s. A Newznab/NZBHydra/Prowlarr instance that fans out to
...
function torrentDiscoveryScope(index, onlySources) {
  ...
  if (prowlarrCfg.liveSearchEnabled === false) values.prowlarrLiveSearchEnabled = false;
...
async function cachedProviderSearch({ event, promo, provider, scope, queries, producer, log, index: suppliedIndex, validateCachedResults, bypassCache = false }) {
  ...
  // bypassCache: a viewer's Refresh asked for a live answer, so a cached one
  // (possibly produced while live Prowlarr was skipped) must not stand in.
  if (index && !bypassCache) {
    try {
...
//
// Nuvio already identifies the Built-in Usenet pipeline, Easynews and
...
// practice Easynews rows vanished the moment the native indexer caught up: a real
...
// it directly, native NNTP streams straight from the provider, TorBox
// resolves a torrent — and they fail differently, perform differently, and
// are labelled differently in the client. Dropping one removes a working
// fallback for a release the user can already see.
//
// Within a pipeline, deduping is still right: one provider returning the same
// release twice is a duplicate. nntp already sets its own scope for exactly
// this reason, and it still wins where present — this generalises what it
// was doing rather than inventing a new rule.
...
function selectProviderQueries(titles, event, limit, promotion) {
  ...
  // queries that reached the native indexer for an EPL fixture were
...
function buildNntpRow(candidate, deferredUrl, lanOnly = false) {
  ...
  return {
    name: '\u{26A1} Native NNTP' + (lanOnly ? ' (LAN)' : '') + '\n' + qualityLine,
    title: candidate.title + (metaLine ? '\n' + metaLine : ''),
    url: deferredUrl,
...
async function discoverTorrentCandidates({ promo, event, titles, log, discoveryBudgetMs, onlySources, liveProwlarr = false, onLateProwlarr, forceLive = false, onServedWithoutLive, liveStatus }) {
  const companionCfg = settings.getCompanion();
  const prowlarrCfg = settings.getProwlarr();
  const bitmagnetCfg = settings.getBitmagnet();
  const sportVideoEnabled = settings.getSportVideo().enabled;
  let direct = [];
  if (sportVideoEnabled && (!onlySources || !onlySources.size || onlySources.has('sport-video'))) {
    try {
  ...
  const queuedProwlarr = require('./prowlarr-discovery').usesQueue(promo);
  // The discovery database can also contain an administrator-confirmed manual
  // match for a promotion outside the measured queue. Always check it before
  // deciding whether a live source search is needed.
  if (!allowed) {
    try {
      const saved = require('./prowlarr-discovery').getDefault().candidates(event);
      direct = direct.concat(saved);
      if (saved.length) log('prowlarr: recovered '+saved.length+' locally stored candidate(s)');
    } catch (error) {log('prowlarr: local lookup failed: '+error.message);}
  }
  // Database first. Bitmagnet and the Prowlarr queue run in the background so
  // that a viewer does not wait on a live tracker fan-out; a live Prowlarr
  // search for an event the database already has releases for spent ~20s and
  // a batch of torrent downloads through Prowlarr to find what was stored.
  // When the stored evidence holds a relevant release with a hash, serve it and
  // leave the slow live sources (Prowlarr, the companion) out. Bitmagnet stays:
  // it is a local index answering in milliseconds.
  // Viewer requests only: background builds exist to refresh this evidence.
  let storedLinks = 0;
  if (!allowed && liveProwlarr) {
    try {
      const index = availabilityIndex(log);
      const stored = index && typeof index.storedForEvent === 'function'
        ? index.storedForEvent({ eventId: event.id, provider: 'torrent' }) : [];
      direct = mergeCandidates(index, direct, stored);
      storedLinks = filterCandidates('torrent', direct.filter((candidate) =>
        /^[a-f0-9]{40}$/i.test(String(candidate && candidate.infoHash || ''))), () => {}, promo, event).length;
    } catch (error) { log('torrent: stored-release lookup failed: ' + error.message); }
  }
  const permitted = (name) => !allowed || allowed.has(name);
  // forceLive is a viewer's Refresh shortly after being served without a live
  // search (see takeRefreshPress). It overrides both the stored-first rule and
  // the background-queue rule, but never the operator's live-search switch.
  const liveSwitch = prowlarrCfg.liveSearchEnabled !== false;
  const liveAllowed = liveSwitch && (forceLive || (!queuedProwlarr && storedLinks === 0));
  if (forceLive && liveSwitch) {
    log('torrent discovery: refresh requested — running live search despite stored results');
  } else if (storedLinks && !queuedProwlarr && liveSwitch && prowlarrCfg.enabled !== false) {
    log('torrent discovery: ' + storedLinks + ' stored release(s) for this event — skipping live Prowlarr');
  }
  if (liveProwlarr && !allowed && liveSwitch && !liveAllowed && typeof onServedWithoutLive === 'function') {
    onServedWithoutLive();
  }
  ...
  const out = await cachedProviderSearch({
    event, promo, provider: 'torrent', scope, queries: titles, log, bypassCache: forceLive && liveAllowed,
    ...
    producer: async () => {
      ...
      if (useProwlarr) {
        ...
        let waiting = true;
        // A Refresh-forced search is the viewer asking for new releases, so
        // whatever it finds after the response has gone is still kept, for
        // every promotion — not only the priority leagues.
        const captureLate = priority || forceLive;
        promise.then(async outcome => {
          if (!waiting && captureLate && onLateProwlarr) {
        ...
        tasks.push(withinBudget('prowlarr', promise, priority ? Math.min(8000, discoveryBudgetMs) : discoveryBudgetMs).then(outcome => {
          ...
          if (liveStatus) {
            liveStatus.searched = true;
            if (outcome.timedOut) liveStatus.pending = true;
            else if (!outcome.ok) liveStatus.failed = true;
          }
  ...
  if (out && out.ok === false) {
    if (!direct.length) throw new Error(out.error || 'torrent discovery failed');
  }
...
async function pipelineTorrentTorbox({ promo, event, titles, torboxKey, urlCtx, log, discoveryBudgetMs, allowConfirmed = true, onlySources, fastResponse = false, liveProwlarr = false, backgroundRefresh = false }) {
  if (!torboxKey) { log('torbox: no user key — skipping pipeline A'); return []; }

  // A viewer's own request (not the background refresh it spawns below) may be
  // a Refresh press following a stored serve; if so it gets a live search and
  // waits for it rather than taking the fast confirmed path.
  const viewerKey = liveProwlarr && !backgroundRefresh && !onlySources && urlCtx && urlCtx.userId
    ? String(urlCtx.userId) + '|' + event.id : null;
  const forceLive = Boolean(viewerKey) && takeRefreshPress(viewerKey);
  if (forceLive) {
    log('torrent: refresh within ' + Math.round(liveRefreshWindowMs() / 1000)
      + 's of a stored serve — searching live');
    fastResponse = false;
  }
  ...
  if (fastResponse && confirmed.length) {
    ...
    if (ready.length) {
      // Served from confirmed results with no live search in this response.
      if (viewerKey) noteStoredServe(viewerKey);
      pipelineTorrentTorbox({promo, event, titles, torboxKey, urlCtx, log,
        discoveryBudgetMs, allowConfirmed, onlySources, fastResponse: false, liveProwlarr, backgroundRefresh: true})
  ...
  // On a Refresh press, remember what was already known so the response can
  // say what the live search added. A search that finds only what the database
  // held otherwise looks exactly like one that never ran.
  const liveStatus = forceLive ? { searched: false, pending: false, failed: false } : null;
  const knownBefore = new Set();
  if (forceLive) {
    try {
      const stored = index && typeof index.storedForEvent === 'function'
        ? index.storedForEvent({ eventId: event.id, provider: 'torrent', limit: 100 }) : [];
      for (const candidate of stored.concat(confirmed, learned)) {
        const hash = String(candidate && candidate.infoHash || '').toLowerCase();
        if (hash) knownBefore.add(hash);
      }
    } catch (error) { log('torrent: stored-release snapshot failed: ' + error.message); }
  }
  let discovered = [];
  try {
    discovered = await discoverTorrentCandidates({
      promo, event, titles, log, discoveryBudgetMs, onlySources, liveProwlarr, forceLive, liveStatus,
      onServedWithoutLive: () => { if (viewerKey) noteStoredServe(viewerKey); },
  ...
  const rowHashes = cached.map((c) => c.infoHash);
  ...
  if (showWarm && uncached.length > 0) {
    ...
    // Warming asks TorBox to download the torrent; with nobody seeding it
    // that never finishes. Unknown seed counts are still offered.
    const warmTop = uncached.filter((c) => c.seeders === undefined || c.seeders === null || Number(c.seeders) > 0)
      .slice(0, WARM_MAX_ROWS);
    for (const c of warmTop) {
      ...
      rowHashes.push(c.infoHash);
    }
    log('torbox: surfaced ' + warmTop.length + ' warm-to-cache row(s)');
  }
  if (liveStatus) annotateRefresh(rows, rowHashes, relevant, knownBefore, liveStatus, log);
  return rows;
}

// What a Refresh-forced live search changed, shown in the client. New rows
// are flagged, and the first row carries a one-line summary, so "searched and
// found nothing new" is distinguishable from "did not search".
function annotateRefresh(rows, rowHashes, relevant, knownBefore, status, log) {
  const isNew = (hash) => !knownBefore.has(String(hash || '').toLowerCase());
  const added = relevant.filter((candidate) => isNew(candidate.infoHash)).length;
  rows.forEach((row, i) => {
    if (isNew(rowHashes[i])) row.name = '\u{1F195} ' + row.name;
  });
  let summary;
  if (!status.searched) summary = 'Live search unavailable — showing saved results';
  else if (status.failed) summary = 'Live search failed — showing saved results';
  else summary = 'Live search: ' + (added ? added + ' new release' + (added === 1 ? '' : 's') : 'no new releases')
    + (status.pending ? ' so far — still searching, refresh again shortly' : '');
  if (rows[0]) rows[0].title = '\u{1F504} ' + summary + '\n' + rows[0].title;
  log('torrent: refresh result — ' + summary);
}
...
async function pipelineNativeNntp({ event, config, getUsenetCandidates, getConfirmedCandidates, urlCtx, log }) {
  ...
  for (const candidate of candidates) {
    ...
    rows.push(buildNntpRow(candidate, buildNntpDeferredUrl({
      origin: urlCtx.nntpOrigin || urlCtx.origin, userId: urlCtx.userId, apiToken: urlCtx.apiToken,
      eventId: event.id, candidateId: stored.id,
    }), Boolean(urlCtx.nntpOrigin)));
  }
...
async function prefetchAvailability(params) {
  ...
  // budget. A real fan-out indexer measured 14.8s on a good run and lost to
  // the old ceiling by 600ms on a worse one — so the backfill built to rescue
  // slow sources was itself timing out against the very sources it exists for.
  ...
  //
  // Preparation searches Bitmagnet only. Other sources have their own worker
  // or fast live search; legacy all-sources overrides cannot widen this worker.
  const warmTorrentSources = new Set(['bitmagnet']);
  const warmBitmagnetReady = Boolean(bitmagnetCfg.url) && bitmagnetCfg.enabled !== false;
  const torrentConfigured = warmBitmagnetReady;
  if (prepare.prepareTorrent !== false && !torrentConfigured
      && !disabled.has('torbox') && !skipProviders.has('torrent')) {
    log('torrent: automatic preparation needs an enabled Bitmagnet endpoint');
  }
  if (prepare.prepareTorrent !== false && torrentConfigured
      && !disabled.has('torbox') && !skipProviders.has('torrent')) {
  ...
  if (prepare.prepareUsenet === true && uuConfig && !skipProviders.has('uu') && !disabled.has('uu')) {
    const index = availabilityIndex(log);
...
async function handleStream(params) {
  ...
  const urlCtx = {
    ...
    nntpOrigin: settings.getDiyPlaybackOrigin(),
  ...
  //
  // 0.93.2 — 9500ms, up from 8000. A fan-out indexer searches across several
  ...
  const discoveryBudgetMs = Math.max(1000, Math.min(
    ...
    budgetMs - 1000));
  // Per-pipeline outcome for this request. Returned only with params.debug
  // (the admin Client check), so ordinary clients get the usual response.
  ...
  function budgeted(name, promise) {
    return new Promise((resolve) => {
      ...
      const timer = setTimeout(() => {
        ...
        pipelineTimings[name] = { status: 'timeout', durationMs: budgetMs, rows: 0 };
        resolve([]);
      }, budgetMs);
      Promise.resolve(promise)
        .then((rows) => {
          ...
          pipelineTimings[name] = { status: 'ok', durationMs: Date.now() - startedAt,
            rows: Array.isArray(rows) ? rows.length : 0 };
          resolve(rows);
        })
        .catch((err) => {
          ...
          pipelineTimings[name] = { status: 'failed', durationMs: Date.now() - startedAt, rows: 0,
  ...
  // an array of pipeline names ('torbox' | 'uu' | 'diy-usenet' | 'easynews')
  // that should be skipped for events from this promotion. Lets an operator
  ...
  function runOrSkip(name, factory) {
    if (disabled.has(name)) {
      pipelineTimings[name] = { status: 'skipped', durationMs: 0, rows: 0 };
  ...
  const getDiyCandidates = () => {
    if (!diyCandidatesPromise) diyCandidatesPromise = (async () => {
      if (!usenetIndexer.isConfigured(nativeUsenetConfig)) {
        log('diy-usenet: no native search source configured');
        return [];
      }
      const index = availabilityIndex(log);
      const scope = index
      ...
      if (isBudgetFailure(out)) scheduleUsenetBackfill({ event, userConfig, username: params.username, log });
      return finalizeUsenetCandidates('diy-usenet', out && out.results, promo, event, log);
    })();
  ...
  const getDiyConfirmed = (playbackProvider) => {
    ...
    const availabilityScope = nntpAvailabilityScope(index, nntpConfig);
    let recovered = [];
    if (usenetIndexer.isConfigured(nativeUsenetConfig)) {
  ...
  const [torboxRows, uuRows, nntpRows, easynewsRows] = await require('./discovery-plan').collectPipelineRows([
    runOrSkip('torbox',   () => pipelineTorrentTorbox({ promo, event, titles: torrentTitles, torboxKey, urlCtx, log, discoveryBudgetMs, liveProwlarr: true, fastResponse: timing.fastResponseGraceMs > 0 })),
    runOrSkip('uu',       () => pipelineUsenetUltimate({ event, uuConfig, getUsenetCandidates: getUuCandidates, log })),
    runOrSkip('diy-usenet', () => pipelineNativeNntp({ event, config: nntpConfig, getUsenetCandidates: getDiyCandidates, getConfirmedCandidates: getDiyConfirmed, urlCtx, log })),
    runOrSkip('easynews', () => pipelineEasynews({
      promo, event, titles: providerTitles, easynewsCreds, urlCtx, log, budgetMs,
    })),
  ], timing.fastResponseGraceMs);

  const merged = mergePipelineRows([
    ['torbox', torboxRows], ['uu', uuRows], ['nntp', nntpRows], ['easynews', easynewsRows],
  ]);
  ...
  require('./diagnosis-journal').recordOpen({
    eventId: id, user: params.username, ms: Date.now() - requestStartedAt, rows: merged.length,
    pipelines: { torbox: torboxRows.length, usenetUltimate: uuRows.length, nativeNntp: nntpRows.length, easynews: easynewsRows.length },
  });
  ...
  return {
    ...
    ...(params.debug ? { diagnostics: {
      requestId: log.requestId, durationMs: Date.now() - requestStartedAt, pipelines: pipelineTimings,
    } } : {}),
  };
}
...
module.exports = {
  ...
  _test: {
    ...
    pipelineUsenetUltimate, pipelineNativeNntp,
    ...
    noteStoredServe, takeRefreshPress, STORED_SERVES,
  },
};

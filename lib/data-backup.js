...
async function createArchive(dataDir,options={}) {
  ...
  try {
    ...
    check(options.signal);
    // Relative paths from inside the staging folder: GNU tar reads an absolute
    // Windows path such as C:\...\backup.tar.gz as host "C" and fails, while
    // relative names work with GNU tar and Windows' bsdtar alike.
    await run('tar',['-czf','backup.tar.gz','-C','data','.'],{cwd:root,signal:options.signal,timeout:120000,maxBuffer:1024*1024});
    await run('tar',['-tzf','backup.tar.gz'],{cwd:root,signal:options.signal,timeout:60000,maxBuffer:8*1024*1024});

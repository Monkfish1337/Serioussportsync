...
function renderBody(input) {
  ...
  const rows = filtered.map((event) => {
    ...
    // Catalog metadata prefers dateLocal. Older TSDB records can retain the
    // UTC source day in date while already showing the local day in NuVio.
    const effectiveDate = patch.date || event.dateLocal || event.date || '';
    ...
    return '<div class="panel">'
      ...
      // The chip says whether this event has been touched; the field below says
      // what the source gave. Both used to read "Source date", so an untouched
      // event printed the same two words twice above its own date.
      + (changed ? '<span class="chip" data-tone="warn">Overridden</span>' : '<span class="chip plain" data-tone="off">Unchanged</span>')

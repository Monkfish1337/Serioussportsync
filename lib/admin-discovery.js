...
function render(data) {
  ...
  if(tab==='overview' && data.coverage) {
    ...
    body=`<div class="row row-cards my-3">${[['Past seven days',c.total],['Torrent identities',c.matched],['Missing torrent identities',c.missing]].map(([label,value])=>`<div class="col-md-4"><div class="card"><div class="card-body"><div class="text-secondary">${label}</div><div class="h1">${value}</div></div></div></div>`).join('')}</div>${data.coverageError?`<div class="alert alert-warning">Coverage is incomplete: ${esc(data.coverageError)}. Events without recorded matches may have database matches that could not be read.</div>`:''}<p>Rolling window: ${esc(time(c.from))} to ${esc(time(c.to))}. Unique past scheduled fixture events; disabled promotions, account-specific My Teams catalogs and Sport-Video release-derived Discovered catalogs are excluded. See Sport-Video for their discovered, matched and prepared release counts. Future, cancelled and postponed events are excluded. Events without a start time are counted after their dated day ends in UTC.</p><p>Matched means an event has a relevant saved torrent hash from Prowlarr, Bitmagnet or Sport-Video. Live Usenet Ultimate, native Usenet and Easynews results do not contribute to Discovery coverage. Torrent availability is still checked per account; title-only matches do not count. This dashboard reads local evidence and makes no indexer searches.</p><h3>Coverage by promotion</h3><div class="table-responsive"><table class="table"><thead><tr><th>Promotion</th><th>Total</th><th>Torrent identity</th><th>Missing</th><th>Recorded gap reasons / Prowlarr status</th></tr></thead><tbody>${c.promotions.map(p=>`<tr><td>${esc(names.get(p.id) || p.id)}</td><td>${p.total}</td><td>${p.matched}</td><td><a href="/admin/discovery?tab=overview&promotion=${encodeURIComponent(p.id)}">${p.missing}</a></td><td>${Object.entries(p.reasons).map(([reason,count])=>`${count} · ${esc(reason)}`).join('<br>') || 'All events have saved torrent matches'}</td></tr>`).join('') || '<tr><td colspan="5">No past events in this window.</td></tr>'}</tbody></table></div><h3>Missing events</h3><form method="GET" action="/admin/discovery"><input type="hidden" name="tab" value="overview"><label class="form-label" for="coverage-promotion">Promotion</label><select class="form-select" name="promotion" id="coverage-promotion"><option value="">All promotions</option>${c.promotions.map(p=>`<option value="${esc(p.id)}" ${data.promotion===p.id?'selected':''}>${esc(names.get(p.id) || p.id)}</option>`).join('')}</select><button class="btn btn-outline-secondary my-2">Apply filter</button></form><p>${missing.length} events without a recorded usable torrent identity (${c.titleOnly} title-only torrent matches across all promotions). Reasons show saved evidence and source settings; enabled sources may have no recorded attempt or result.</p><div class="table-responsive"><table class="table"><thead><tr><th>Event</th><th>Date</th><th>Recorded reason / Prowlarr</th><th>Bitmagnet</th><th>Sport-Video</th><th>Next fixture retry</th></tr></thead><tbody>${missing.map(e=>`<tr><td>${esc(e.name)}<div><a class="small" href="/admin/discovery/manual?eventId=${encodeURIComponent(e.id)}">Match resources</a></div></td><td>${esc(e.date)}</td><td>${esc(e.reason)}</td><td>${esc(e.bitmagnet)}</td><td>${esc(e.sportVideo)}</td><td>${e.nextAt?esc(time(e.nextAt)):'—'}</td></tr>`).join('') || '<tr><td colspan="6">No missing events in this selection.</td></tr>'}</tbody></table></div>`;
  }
  if(data.selection) {

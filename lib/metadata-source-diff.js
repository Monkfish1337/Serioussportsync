...
async function compare(promotion, definition, existingEvents, opts) {
  ...
  // nothing. Named TheSportsDB lookups are one rate-limited request per name
  // and would spend the preview's whole 60s deadline on their own.
  const raw = await fetchPromotion(candidate, opts.log || (() => {}), { skipNamedLookups: true });

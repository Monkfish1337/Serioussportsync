...
function selectTorrentQueries(titles, event, promotion, maxQueries) {
  ...
  if (!['mlb', 'nba', 'nfl', 'nhl'].includes(String(promotion && (promotion.teamAliasPreset || promotion.id)))) return unique.slice(0, cap);
  ...
  if (promotion.id === 'nfl' && Number.isInteger(week) && week > 0 && /^\d{4}-\d{4}$/.test(span)) {
    diverse.push('NFL ' + span + (event.seasonPhase === 'preseason' ? ' PS' : '') + ' W' + String(week).padStart(2, '0') + ' ' + pair.join(' '));
  }

...
function sanitizeWeekly(input) {
  ...
  const reusable = (value) => String(value || '')
    .replace(/\bS\d{1,3}E\d{1,3}\b/gi, ' ')
    .replace(/#\d{2,5}\b/g, ' ')
    .replace(/\s+/g, ' ').trim();
  const aliases = lines(values(input.promotionAliases).map(reusable), 20, false)
    .filter((value) => value.replace(/[^a-z]/gi, '').length >= 3);
  const keywords = lines(values(input.relevanceKeywords).map(reusable), 20, true)
    .filter((value) => value.replace(/[^a-z]/gi, '').length >= 3);
  const templates = lines(values(input.searchTitleTemplates).map(reusable), 12, false);
...
module.exports = { load, list, find, set, remove, normalise, sanitizeWeekly, file: FILE };

...
const { versionedArtwork } = require('./bundled-artwork');
...
function resolvedPreview(folder) {
  if (folder.artwork !== 'promotion') return versionedArtwork(folder.artwork || '/assets/logo-banner.png');
...
function folderCard(folder) {
  ...
  return '<div class="card mb-3"><form method="POST" action="/admin/nuvio-collections/folders/' + encodeURIComponent(folder.id) + '/save">'
    + '<div class="row g-0">'
    // `ratio` sizes a box from its aspect ratio with padding; `h-100` overrode
    // that with the row's height, and in a `row g-0` the row's height is set by
    // the body column beside it. The two rules fought, so the artwork stretched
    // or collapsed depending on how much text the card carried and the body text
    // never lined up with the top of the image.
    + '<div class="col-md-4"><div class="ratio ratio-16x9"><img src="' + escapeHtml(resolvedPreview(folder)) + '" alt="" class="object-fit-cover rounded-start" onerror="this.src=\'/assets/logo-banner.png\'"></div></div>'
    ...
    // The name list used to sit inside the <summary>, where a long one wraps
    // under the disclosure marker and indents every line but the first.
    + '<details class="mt-3"><summary class="text-secondary">Included promotions (' + escapeHtml(String(folder.promotions.length)) + ')</summary>'
    + (includedNames.length ? '<div class="text-secondary small mt-1">' + escapeHtml(includedNames.join(', ')) + '</div>' : '')
    + '<div class="form-selectgroup mt-2">' + promotionChoices(folder.promotions, folder.id) + '</div></details>'
...
function folderInput(body) {
  body = body || {};
  // Keep accepting the short-lived wizard field too: a browser with an older
  // page open should not lose its folder edit during a rolling deployment.
  const artworkChoice = body.artworkChoice || body.artwork;
  const artwork = artworkChoice === 'custom' ? body.customArtwork : artworkChoice;

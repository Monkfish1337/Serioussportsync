...
class NntpConnectionPool {
  constructor(config, options) {
    ...
    this.options = Object.assign({}, options || {}, {
      timeoutMs: Number(config && config.timeoutMs) || (options && options.timeoutMs),
    });
...
function getPool(config) {
  ...
  // Performance settings can be changed live from the Usenet page without
  // changing provider credentials (and therefore without changing poolKey).
  pool.config = config;
  pool.options.timeoutMs = Number(config.timeoutMs) || pool.options.timeoutMs;
  pool.setLimit(config.maxConnections);
  return pool;
}

function snapshot() {
  return Array.from(registry.values()).map((pool) => ({
    host: pool.config.host,
    total: pool.total,
    idle: pool.idle.length,
    busy: Math.max(0, pool.total - pool.idle.length),
    waiting: pool.waiting.length,
    limit: pool.maxConnections,
  }));
}

module.exports = {
  NntpConnectionPool, getPool, poolKey, snapshot, IDLE_MS, DEFAULT_CONNECTIONS, _registry: registry,
};

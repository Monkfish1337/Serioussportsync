...
// These overflow competitions now belong to the sport-level Discovery feeds.
const RETIRED_PROMOTIONS = new Set(['wnba', 'ncaaf', 'laliga', 'efl-championship',
  'seriea', 'brasileirao', 'ligue1', 'bundesliga', 'eredivisie']);
...
function isIngestible(record, ownPromotionIds) {
  ...
  return !matches.some((match) => match && match.promotion &&
    !RETIRED_PROMOTIONS.has(match.promotion) && !ownPromotionIds.has(match.promotion));
}
...
module.exports = { fetchAll, toRaw, cleanName, eventKey, isIngestible, promotionIdFor, SPORTS, RETIRED_PROMOTIONS };

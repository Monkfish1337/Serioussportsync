...
function parseMultiStatus(xml, requestUrl) {
  ...
  while ((match = responsePattern.exec(String(xml || '')))) {
    ...
    let url;
    try {
      // WebDAV servers commonly emit absolute hrefs using their container or
      // reverse-proxy hostname. An href identifies a path; it is not an
      // authorization to send credentials to that advertised origin. Rebase
      // the returned path onto the configured/requested origin.
      const advertised = new URL(href, requestUrl);
      url = new URL(advertised.pathname + advertised.search, requestOrigin).toString();
    }
    catch (_) { continue; }

...
async function scrape({ promotion, event, searchTitles, log, budgetMs, researchMode, throwOnFailure, fetchImpl }) {
  log = log || (() => {});
  ...
  if (researchMode === true) request.researchMode = true;
  ...
  const researchTimeoutMs = parseInt(process.env.COMPANION_RESEARCH_TIMEOUT_MS || '60000', 10);
  const transportCeilingMs = researchMode === true ? researchTimeoutMs : configuredTimeoutMs;
  const timeoutMs = requestedBudgetMs > 0
    ? Math.min(transportCeilingMs, Math.max(1000, requestedBudgetMs) + 1000)
    : transportCeilingMs;
  let res;
  try {
...
async function intelligenceSearch({ queries, protocol, limit, fetchImpl }) {
  const cfg = settings.getCompanion();
  if (!cfg.url) return { ok: false, error: 'Companion not configured', results: [] };
  const url = cfg.url.replace(/\/+$/, '') + '/api/intelligence/search';
  const headers = { 'Content-Type': 'application/json', Accept: 'application/json' };
  if (cfg.authToken) headers.Authorization = 'Bearer ' + cfg.authToken;
  let response;
  try {
    response = await (fetchImpl || fetch)(url, httpAgent.fetchOpts({
      method: 'POST', headers,
      body: JSON.stringify({ queries: Array.isArray(queries) ? queries.slice(0, 12) : [],
        protocol: protocol || '', limit: Math.max(1, Math.min(500, Number(limit) || 250)) }),
      timeout: 10000,
    }, url));
  } catch (error) {
    return { ok: false, error: 'Release Intelligence unavailable', results: [] };
  }
  if (!response.ok) return { ok: false,
    error: response.status === 404 ? 'Companion update required' : 'Release Intelligence HTTP ' + response.status,
    results: [] };
  let payload;
  try { payload = await boundedBody.readJson(response, MAX_RESPONSE_BYTES, 'Release Intelligence response'); }
  catch (_) { return { ok: false, error: 'Release Intelligence returned an invalid response', results: [] }; }
  const results = [];
  for (const item of payload && payload.results || []) {
    ...
    results.push({
      title, size: Number(item.size) || 0,
      publishedAt: item.publishedAt || item.lastSeenAt || null,
      indexer: origins.map((origin) => String(origin.indexer || origin.sourceName || '').slice(0, 100))
        .filter(Boolean).slice(0, 3).join(' + ') || 'Release Intelligence',
    });
  }
  return { ok: true, results, stats: payload && payload.stats || null };
}

module.exports = { scrape, intelligenceSearch };

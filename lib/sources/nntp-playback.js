...
async function serve(req, res, descriptor, config, options) {
  ...
  try {
    ...
    while (position < range.endExclusive) {
      ...
      if (!pending) pending = readDescriptorRange(descriptor, position, length,
      ...
      if (nextPosition < range.endExclusive) {
        ...
        pending = readDescriptorRange(descriptor, nextPosition, nextLength,
          config, opts, activeSessions);
      }
    ...
    if (cancelled && pending) await pending.catch(() => {});
    if (!cancelled) {
      res.end();

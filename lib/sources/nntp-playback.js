...
async function serve(req, res, descriptor, config, options) {
  ...
  let pending = null;
  const readOptions = {...opts, isCancelled: () => cancelled || shuttingDown};
  ...
  try {
    ...
    const connectionWindow = Math.min(32,
      config.maxConnections || nntpPool.DEFAULT_CONNECTIONS);
    const windowSize = Math.max(1, Number(opts.windowSizeBytes) || Math.min(32 * 1024 * 1024,
      Math.max(1024 * 1024, baseChunkSize * connectionWindow)));
    while (position < range.endExclusive) {
      const length = Math.min(windowSize, range.endExclusive - position);
      if (!pending) pending = startRead(position, length);
      ...
      // Start the following window before writing the current one. This
      // overlaps NNTP work with socket backpressure/player consumption while
      // retaining at most two bounded windows in memory.
      if (nextPosition < range.endExclusive) {
        const nextLength = Math.min(windowSize, range.endExclusive - nextPosition);
        pending = startRead(nextPosition, nextLength);
      }
      if (!res.write(chunk)) await waitForDrain(res, pending);
    ...
    if (!cancelled) {
      ...
      log('nntp: served bytes ' + range.start + '-' + (range.endExclusive - 1)
        + ' (prefetch ' + connectionWindow + ', pipelined)');
    }
  } catch (error) {

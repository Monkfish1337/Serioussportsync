...
async function resolveCandidate(config, candidate, options) {
  ...
  try {
    const article = await withSession(cfg, opts,
      (session) => session.body(file.segments[0].messageId));
...
async function serve(req, res, descriptor, config, options) {
  ...
  try {
    const pool = opts.connect ? null : nntpPool.getPool(config);
    const fetchPart = async (index) => {
      if (index === 0 && descriptor.firstPart) return descriptor.firstPart;
      const run = async (session) => {
        activeSessions.add(session);
        try {
          return decodeArticle(await session.body(descriptor.segments[index].messageId));
        } finally { activeSessions.delete(session); }
      };
      if (pool) return pool.run(run);
    ...
    while (position < range.endExclusive) {
      ...
      const width = Math.min(16, config.maxConnections || 8,
        descriptor.segments.length - segmentIndex);
      const batch = await Promise.all(Array.from({ length: width }, (_, offset) =>
        fetchPart(segmentIndex + offset)));
      for (const decoded of batch) {
        if (cancelled || position >= range.endExclusive) break;

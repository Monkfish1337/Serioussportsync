...
// Download the NZB document an indexer returned, following a bounded number
// of redirects and capping the response size. Used to fetch the release
// SSS itself parses for direct-video/RAR segments before opening them over
// NNTP — no third-party download/upload service is involved.
...
async function resolveCandidate(config, candidate, options) {
  ...
  const xml = await (opts.downloadNzb || downloadNzb)(item.nzbUrl, opts);
  ...
  try {
    ...
    throw new PlaybackProviderError(
      'archive-required', 'This archive is compressed, encrypted, damaged, or unsupported; choose another release, or a TorBox or Easynews row', {
        provider: 'nntp', retryable: false, httpStatus: 422,
      });
  } catch (error) {
    if (error instanceof PlaybackProviderError) throw error;
...
module.exports = {
  ...
  readNntpFileRange, readDescriptorRange, downloadNzb,

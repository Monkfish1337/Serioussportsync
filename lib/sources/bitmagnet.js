...
async function search(queryString, options) {
  ...
  try {
    res = await (opts.fetchImpl || fetch)(url, httpAgent.fetchOpts({
      ...
      timeout: Number(opts.timeoutMs) || Number(cfg.timeoutMs) || 15000,
    }, url));
  } catch (err) {
...
async function multiSearch(queries, options) {
  ...
  const log = opts.log || (() => {});
  const cfg = opts.config || settings.getBitmagnet();
  let unique = Array.from(new Set((queries || [])
    .map((q) => String(q || '').trim()).filter(Boolean)));

  if (opts.queryReview) unique = opts.queryReview.filter(unique);
  if (!cfg.url) {
  ...
  let truncated = false;
  let failed = false;
  const deadlineAt = deadlineAfter(opts.deadlineMs);
  let next = 0;

  async function worker() {
    while (true) {
      if (deadlineAt && Date.now() >= deadlineAt) { failed = true; return; }
      const index = next++;
      ...
      let page;
      try {
        const timeoutMs = Math.max(1, Math.min(Number(cfg.timeoutMs) || 15000,
          deadlineAt ? deadlineAt - Date.now() : Infinity));
        page = await withinDeadline(() => search(q, Object.assign({}, opts, { config: cfg, log, timeoutMs, throwOnFailure: true })), deadlineAt);
        completed++;
      } catch (error) {
        failed = true;
      ...
      if (!page) continue;
      if (opts.queryReview) opts.queryReview.record(q, 'success', page.items.map(toCandidate).filter(Boolean), Date.now()-queryStarted);
  ...
  if (opts.queryReview) opts.queryReview.finish(unique);
  const seen = new Set();
  const out = [];
  ...
  return opts.detailed
    ? { ok: completed > 0, partial: failed || truncated || completed < unique.length,
      error: completed > 0 ? null : 'all-failed', results: out }
    : out;
}

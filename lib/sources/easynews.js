...
const {withinDeadline, deadlineAfter} = require('../search-deadline');
...
async function multiSearch(queries, options) {
  ...
  const selectedQueries = (opts.queryReview ? opts.queryReview.filter(Array.from(new Set((queries || []).filter(Boolean)))) : Array.from(new Set((queries || []).map((q) => String(q || '').trim()).filter(Boolean))))
    .slice(0, maxQueries);
  const seen = new Set();
  const merged = [];
  let completed = 0;
  let partial = false;
  const deadlineAt = deadlineAfter(opts.totalTimeoutMs);
  ...
  for (const q of selectedQueries) {
    ...
    if (deadlineAt && Date.now() >= deadlineAt) {
      partial = true;
      log('  easynews: query budget reached after ' + completed + ' request(s)');
      break;
    }
    log('  easynews: query "' + q + '"');
    let r;
    try {
      const timeoutMs = Math.max(1, Math.min(Number(opts.timeoutMs) || 20000,
        deadlineAt ? deadlineAt - Date.now() : Infinity));
      r = await withinDeadline(() => search(q, Object.assign({}, opts, { log, timeoutMs })), deadlineAt);
    } catch (error) { if (opts.queryReview) opts.queryReview.record(q, /timeout|deadline/i.test(error.message) ? 'timeout' : 'error', [], Date.now()-queryStarted); partial = true; break; }
    if (!r.ok) {
      partial = true;
      if (opts.queryReview) opts.queryReview.record(q, /timeout|deadline/i.test(r.error || '') ? 'timeout' : 'error', [], Date.now()-queryStarted);
      log('    -> ' + (r.error || 'failed'));
      continue;
    }
    completed++;
    if (opts.queryReview) opts.queryReview.record(q, 'success', r.results, Date.now()-queryStarted);
    ...
    if (q !== selectedQueries[selectedQueries.length - 1]) {
      const pause = Number(opts.queryDelayMs) > 0 ? Number(opts.queryDelayMs) : 800;
      if (deadlineAt && Date.now() + pause >= deadlineAt) { partial = true; break; }
      await new Promise((res2) => setTimeout(res2, pause));
    }
  }

  if (opts.queryReview) opts.queryReview.finish(selectedQueries);
  log('  easynews: ' + merged.length + ' unique result(s) across ' + completed + ' queries');
  return {
    ok: completed > 0 || selectedQueries.length === 0,
    partial,
    error: completed > 0 || selectedQueries.length === 0 ? null : 'all-failed',
    results: merged, dlFarm, dlPort, downURL,
  };
}

...
function checkSourceCooldown() {
  const until=Date.parse(load().sourceRetryAt || '');
  if(until>Date.now()) throw new Error('Sport-Video source cooldown until '+new Date(until).toISOString());
}
function observeSourceResponse(response) {
  if(response.status===429 || response.status===503) {
    const delay=Math.max(3600000,require('../discovery-cadence').retryAfter(response.headers?.get('retry-after')));
    const state=load();state.sourceRetryAt=new Date(Date.now()+delay).toISOString();save(state);
  }
}
async function fetchBounded(url, options) {
  ...
  for (let redirects = 0; redirects <= 2; redirects += 1) {
    ...
    observeSourceResponse(response);
...
// A release catalogue cannot legitimately publish content months in the
// future. The upstream search index occasionally contains mistyped future
// years, and release-first ingestion would otherwise turn those typos into
// convincing but impossible fixture rows. Allow tomorrow for timezone edges.
function plausibleReleaseDate(date, nowMs) {
  ...
  return stamp <= now.getTime() + 86400000;
}
...
function parseSearchIndex(text, nowMs) {
  const entries = [];
  const seen = new Set();
  ...
  while ((match = re.exec(String(text || '')))) {
    ...
    if (!date || !plausibleReleaseDate(date, nowMs) || seen.has(file.toLowerCase())) continue;
...
function parseCatalog(html, category, nowMs) {
  const releases = [];
  ...
  while ((match = re.exec(String(html || ''))) && releases.length < 1000) {
    ...
    const date = extractDate(title);
    if (!title || !date || !plausibleReleaseDate(date, nowMs)
      || /^(?:index|schedule|oldergames)\.html$/i.test(file)) continue;
    let detailUrl;
...
// How many distinct torrents a detail page offers. Only the first is used;
// this measures whether pages with several quality variants (1080p and 4K,
// Discussions #40) are common enough to justify one record per variant.
function torrentVariants(html, detailUrl) {
  const names = [];
  const seen = new Set();
  for (const match of String(html || '').matchAll(/href=["']([^"']+\.torrent)["']/gi)) {
    let url;
    try { url = new URL(decodeHtml(match[1]), detailUrl).toString(); } catch (_) { continue; }
    if (seen.has(url)) continue;
    seen.add(url);
    if (names.length < 6) names.push(decodeURIComponent(path.basename(new URL(url).pathname)).slice(0, 200));
  }
...
function parseDetail(html, record) {
  ...
  const variants = torrentVariants(html, record.detailUrl);
  if (variants.count > 1) {
    console.log('[sport-video] detail page offers ' + variants.count + ' torrents; using the first', {
      module: 'sport-video', title: record.title, variants: variants.names,
    });
  }
  ...
  return Object.assign({}, record, {
    ...
    torrentVariants: variants.count,
    torrentVariantNames: variants.count > 1 ? variants.names : undefined,
...
async function scan(options) {
  ...
  running = (async () => {
    ...
    try {
      ...
      // Also removes bad future rows retained by an older image. They cannot
      // be rediscovered after the parser guard above, but without this prune
      // the merge deliberately carries unseen historical rows forward.
      const retained = merged.filter((record) => plausibleReleaseDate(record.date)).slice(0, MAX_RELEASES);
      ...
      const prepare = retained.filter((record) => record.matches && record.matches.length
        && record.matches.some(match=>{
          const event=eventById.get(match.eventId);
          return event && require('../discovery-cadence').readyAt(event)<=Date.now();
        })
        && record.matches.some(match => settings.sourceDiscoveryIncludes('sport-video',match.eventId))
        && !record.infoHash && matchAgeDays(record) <= windowDays
        && (!record.prepareRetryAt || Date.parse(record.prepareRetryAt)<=Date.now())
      ...
      // One detail at a time; index polling must not turn into a request burst.
      let lastPrepare=0;
      await mapLimit(prepare, 1, async (record) => {
        if(Date.parse(load().sourceRetryAt || '')>Date.now()) return;
        if(lastPrepare) await new Promise(resolve=>setTimeout(resolve,Math.max(0,10000-(Date.now()-lastPrepare))));
        lastPrepare=Date.now();
        try {
          const hydrated = await hydrateRecord(record);
          Object.assign(record, hydrated, { prepareError: '',prepareFailures:0,prepareRetryAt:null });
          runtime.prepared += 1;
        } catch (error) {
          record.prepareError = String(error.message || error).slice(0, 300);
          record.prepareAttemptedAt = new Date().toISOString();
          record.prepareFailures=(Number(record.prepareFailures)||0)+1;
          const delay=/HTTP 404|HTTP 410/.test(record.prepareError)?24*3600000:Math.min(24*3600000,3600000*2**Math.min(record.prepareFailures-1,5));
          record.prepareRetryAt=new Date(Date.now()+delay).toISOString();
      ...
      const state = {
        ...
        sourceRetryAt:load().sourceRetryAt || null,
      };
      save(state);
      ...
      currentState.sourceRetryAt=load().sourceRetryAt || null;
...
async function autoWarmMatched(releases, options) {
  const cfg = settings.getSportVideo();
  const wanted = new Set((cfg.autoWarmPromotions || []).filter(id => settings.sourceDiscoveryIncludes('sport-video',id + ':')));
  if (!wanted.size) return { attempted: 0, warmed: 0, ready: 0 };
  const streams = require('../streams');
  const users = require('../users');
  const promotions = require('../promotions');
  const { effectiveCatalogSelection, promotionSelectedForUser } = require('../catalog-selection');
  ...
  for (const record of eligible) {
    ...
    for (const profile of profiles) {
      ...
      if (promotion && !promotionSelectedForUser(profile.config, promotion)) continue;
...
function status() {
  ...
  return {
    ...
    // Preparation and matching are independent, so "prepared" could read
    // higher than "matched" and imply a funnel that is not one. A release keeps
    // its torrent identity when a later scan disconnects it — which is exactly
    // what happened to the American-football releases prepared while the NFL
    // matcher was over-accepting. Counting them separately makes that visible
    // instead of puzzling.
    preparedMatched: releases.filter((record) =>
      record.infoHash && record.matches && record.matches.length).length,
    preparedOrphans: releases.filter((record) =>
      record.infoHash && !(record.matches && record.matches.length)).length,
    unmatched: releases.filter((record) => !record.matches || !record.matches.length).length,
    // Variant measurement (Discussions #40). Pages prepared before it existed
    // carry no count and are left out of both numbers.
    variantPagesChecked: releases.filter((record) => Number.isInteger(record.torrentVariants)).length,
    multiVariantPages: releases.filter((record) => record.torrentVariants > 1).length,
    multiVariantExamples: releases.filter((record) => record.torrentVariants > 1).slice(-5)
      .map((record) => ({ title: record.title, count: record.torrentVariants, names: record.torrentVariantNames || [] })),

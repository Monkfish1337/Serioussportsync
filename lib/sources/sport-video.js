...
function matchRelease(record, events, promotions) {
  const matches = [];
    ...
    try {
      ...
      const pages = await mapLimit(categories, 3, async (category) => {
        ...
        const response = await fetchBounded(ORIGIN + pathName, {
          suffix: '.html', maxBytes: 1024 * 1024, label: 'Sport-Video ' + category + ' catalogue',
        });
      ...
      for (const record of discovered.values()) {
        ...
        next.matches = matchRelease(next, events, promotions);
        merged.push(next);
      }
      ...
      runtime.matched = retained.filter((record) => record.matches && record.matches.length).length;
      ...
      console.log('[sport-video] scan complete', {
        module: 'sport-video', discovered: runtime.discovered, matched: runtime.matched,
...
async function candidatesForEvent(eventId, options) {
  ...
  const limit = Math.min(5, Number(options && options.limit) || 5);
...
module.exports = {
  ORIGIN, RSS_URL, CATEGORY_PATHS,
  parseRss, parseCatalog, parseDetail, torrentMetadata, matchRelease,

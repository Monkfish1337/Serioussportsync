...
function cleanConfig(config) {
  ...
  if (!['http:', 'https:'].includes(parsed.protocol) || parsed.username || parsed.password) {
    throw new PlaybackProviderError('invalid-config', 'NZB DAV URL must use HTTP(S) and must not contain credentials', {
      provider: 'nzbdav', retryable: false, httpStatus: 400,
    });
  }
...
async function apiRequest(config, params, options) {
  ...
  try {
    ...
    body = await boundedBody.readText(response, MAX_API_RESPONSE_BYTES, 'NZB DAV response');
  } catch (error) {

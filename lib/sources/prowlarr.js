...
async function hydrateAll(results, log, concurrency, options) {
  ...
  async function worker() {
    while (true) {
      if (opts.deadlineAt && Date.now() >= opts.deadlineAt) return;
      const idx = i++;
...
async function multiSearch(queries, options) {
  ...
  const hydrationState = {remaining: opts.hydrationLimit === undefined ? 25 : opts.hydrationLimit, failedIndexers: new Set()};
  ...
  return opts.detailed
    ? { ok: completed > 0 || planned.length === 0, partial, error: completed > 0 || planned.length === 0 ? null : 'all-failed', results: out }
    : out;
}
module.exports = {
  search,

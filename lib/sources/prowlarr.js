...
async function multiSearch(queries, options) {
  ...
  const deadlineAt = Number(opts.deadlineMs) > 0
    ? Date.now() + Number(opts.deadlineMs) * 0.7 : 0;
  ...
  for (const q of planned) {
    ...
    let results;
    try {
      results = await search(q, Object.assign({}, opts, { log, throwOnFailure: true }));
      completed++;
    } catch (error) {
    ...
    if (planned.length > 1) await delay(150);
  }

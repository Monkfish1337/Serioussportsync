...
async function multiSearch(queries, options) {
  ...
  const hydrationState = {remaining: 25, failedIndexers: new Set()};
  ...
  for (const q of planned) {
    ...
    if (!opts.titlesOnly) {
      const batch = hydrationTail.then(async () => {
        ...
        const count = await hydrateAll(results, log, 4, Object.assign({}, opts, {deadlineAt: fullDeadlineAt, hydrationState}));

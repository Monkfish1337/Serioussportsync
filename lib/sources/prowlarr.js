...
async function multiSearch(queries, options) {
  ...
  // Leave room for hydrateAll below, which is part of the same deadline.
  ...
  const deadlineAt = usableBudget ? startedAt + usableBudget * 0.7 : 0;
  ...
  const hydrated = await hydrateAll(collected, log, 4, Object.assign({}, opts, { deadlineAt: fullDeadlineAt }));
  let dropped = 0;

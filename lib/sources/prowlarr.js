...
async function multiSearch(queries, options) {
  ...
  const planned = queries.slice(0, maxQueries);
  ...
  if (opts.titlesOnly) return {ok: completed > 0, partial, results: collected.map(r => ({title: r.title, size: r.size, indexer: r.indexer, publishDate: r.publishDate}))};
  let dropped = 0;
  ...
  return opts.detailed
    ? { ok: completed > 0, partial, error: completed > 0 ? null : 'all-failed', results: out }
    : out;
}
module.exports = {
  search,

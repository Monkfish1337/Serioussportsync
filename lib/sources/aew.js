...
function toRaw(record) {
  ...
  return {
    ...
    // "AEW: Dynamite", "AEW: Dynamite/Collision", "AEW: All Out PPV". Only the
    // weekly-show split below reads it; fromWiki ignores it.
    eventType: String(record.eventType || '').trim() || null,
    source: { type: 'aew', eventId: String(record._id) },
  };
}

// The weekly tapings, for the AEW Dynamite and AEW Collision catalogues.
//
// Those catalogues are TheSportsDB-backed, and the free key stops short of
// AEW's future episodes for the same reason it stopped short of the PPVs: a
// fifteen-event season cap spent on weeks already past. AEW's schedule lists
// every announced taping, so it fills in the Upcoming rows.
//
// Which show a record belongs to comes from its type and title together:
// "AEW: Dynamite" is a Dynamite, "AEW Fright Night: Dynamite & Collision" is
// both. A combined taping is only a Dynamite here. Dynamite airs on the taping
// night; the Collision taped alongside it airs on a later date AEW does not
// publish, and weekly matching is strict on the air date, so guessing it would
// produce an episode that matches nothing (or the wrong week).
const SHOW_WORDS = /\b(?:Dynamite|Collision)\b/gi;
function weeklyEpisodes(events, show) {
  const want = String(show || '').toLowerCase();
  const out = [];
  for (const event of events || []) {
    if (!event || !event.sourceId || !event.date) continue;
    const label = (event.eventType || '') + ' ' + (event.name || '');
    const hasDynamite = /\bDynamite\b/i.test(label);
    const hasCollision = /\bCollision\b/i.test(label);
    const belongs = want === 'dynamite' ? hasDynamite
      : want === 'collision' ? (hasCollision && !hasDynamite)
        : false;
    if (!belongs) continue;
    // "Dynamite Collision Indianapolis" -> "Indianapolis";
    // "Fright Night Dynamite Collision" -> "Fright Night".
    const suffix = String(event.name || '').replace(SHOW_WORDS, ' ')
      .replace(/[&/:,–-]+/g, ' ').replace(/\s+/g, ' ').trim()
      || String(event.city || '').split(',')[0].trim();
    const title = 'AEW ' + show.charAt(0).toUpperCase() + show.slice(1).toLowerCase();
    out.push(Object.assign({}, event, {
      sourceId: 'aew-' + event.sourceId,
...
module.exports = {
  fetchAll, parseEvents, toRaw, weeklyEpisodes, extractWarmup, eventRecordsFrom,

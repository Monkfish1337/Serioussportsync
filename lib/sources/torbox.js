...
async function resolveCached(result, ctx) {
  ...
  if (!created || !created.torrent_id) return { ok: false };
  const torrentId = created.torrent_id;

  const info = await torrentInfo(torrentId, token, log);
  if (!info || !Array.isArray(info.files)) return { ok: false };
  ...
  if (!downloadUrl) return { ok: false };
...
module.exports = {
  ...
  resolveCached,
  warmCache,
  checkCached, createTorrent, torrentInfo, requestDownload,
};

...
function providerConfig(userConfig) {
  ...
  return {
    enabled: cfg.diyUsenetEnabled === true && cfg.diyNativeSearchEnabled === true,
...
async function search(queries, config, options) {
  ...
  let unique = Array.from(new Set((queries || []).map((q) => String(q || '').trim())
  ...
  const settled = await Promise.allSettled(unique.map(async query => {
    const start = Date.now();
    try {
      const results = await searchOne(query, cfg, opts);
      if (opts.queryReview) opts.queryReview.record(query, 'success', results, Date.now()-start);
      return results;
    } catch (error) {
      if (opts.queryReview) opts.queryReview.record(query, /timeout|deadline/i.test(error.message) ? 'timeout' : 'error', [], Date.now()-start);
      throw error;
    }
  }));
  const seen = new Set();
  ...
  for (const outcome of settled) {
    if (outcome.status === 'rejected') {
      ...
      log('  native usenet: one variant ' + (error.code || 'failed') + ' — '
  ...
  log('  native usenet: ' + merged.length + ' unique candidate(s); '
    + (unique.length - failed) + '/' + unique.length + ' variant(s) completed');
  return { ok: failed < unique.length, error: failed === unique.length ? 'all-failed' : null, results: merged };
}

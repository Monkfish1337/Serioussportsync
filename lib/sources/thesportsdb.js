...
async function fetchSeasonAllRounds(leagueId, season, log, deadlineAt) {
  log = log || (() => {});
  const collected = new Map();
...
async function fetchAll(opts) {
  ...
  for (const season of seasons) {
    ...
    for (const e of await fetchSeasonAllRounds(leagueId, season, log, deadlineAt)) rounds.push(e);
  ...
  // these events "All Out", "Revolution", "Full Gear", and a name already in the
  // set costs nothing to skip. In the steady state most names are present, so a
  // refresh pays for the handful that are not rather than all fourteen. A
  // preview still pays for none.
  if (Array.isArray(opts.knownEvents) && opts.knownEvents.length) {
    const have = Array.from(dedup.values())
      .map((e) => String(e && e.strEvent || '').toLowerCase());
    const missing = opts.knownEvents
      .filter((name) => {
        const needle = String(name || '').toLowerCase().trim();
        return needle && !have.some((title) => title.includes(needle));
      });
    if (opts.skipNamedLookups) {
      log('-> named events: skipped for this preview (' + opts.knownEvents.length + ' name(s))');
    } else if (!missing.length) {
      log('-> named events: not needed, the list endpoints returned all '
        + opts.knownEvents.length + ' known card(s)');
    } else {
      log('-> named events (searchevents): ' + missing.length + ' of '
        + opts.knownEvents.length + ' known card(s) missing from the list endpoints: '
        + missing.join(', '));
      try {
...
module.exports = {
  ...
  fetchSeasonAllRounds, fetchNamedEvent, fetchNamedEvents, fetchAll, resolveSeasonIds,

...
async function fetchAll(opts) {
  ...
  //
  // So: only when the list endpoints failed to reach a single future event,
  // which is the exact condition these exist for. A league whose own schedule
  // comes through pays nothing. And never during a preview, which is
  // interactive, writes nothing, and should show what the ordinary endpoints
  // return.
  if (Array.isArray(opts.knownEvents) && opts.knownEvents.length) {
    const today = new Date().toISOString().slice(0, 10);
    const reachedFuture = Array.from(dedup.values())
      .some((e) => String(e && e.dateEvent || '') >= today);
    if (opts.skipNamedLookups) {
      log('-> named events: skipped for this preview (' + opts.knownEvents.length + ' name(s))');
    } else if (reachedFuture) {
      log('-> named events: not needed, the list endpoints reached a future event');
    } else {
      log('-> named events (searchevents): no future event from the list endpoints, '
        + 'asking for ' + opts.knownEvents.length + ' known card(s)');
      try {
        for (const e of await fetchNamedEvents(leagueId, opts.knownEvents, log)) {

...
const STYLE = '<style>'
  ...
  + '.dx-actions{display:flex;flex-wrap:wrap;gap:8px;align-items:center;margin-top:14px}.dx-actions form{margin:0 0 0 auto}'
  ...
  + '@media(max-width:560px){.dx-actions form{margin-left:0}}'
  + '</style>';
...
const button = (link, kind) => (link ? '<a class="btn sm ' + kind + '" href="' + esc(link.href) + '"' + external(link.href) + '>' + esc(link.label) + '</a>' : '');
...
function findingCard(f, stageLabel, open) {
  ...
  return '<details class="dx-finding" data-severity="' + f.severity + '"' + (open ? ' open' : '') + '><summary>'
    ...
    + '<form method="POST" action="/admin/diagnosis/hide"><input type="hidden" name="id" value="' + esc(f.id) + '">'

...
function fromWiki(raw, promotion) {
  ...
  // Game state, read by discovery to skip cancelled and postponed fixtures.
  // MLB supplies it at the top level; ESPN inside its provenance block.
  const status = raw.status || (raw.source && raw.source.status);
  if (status) ev.status = String(status);

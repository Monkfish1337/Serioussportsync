...
function renderCatalogGate(gate, coverage) {
  ...
  const rows = (Array.isArray(cover.promotions) ? cover.promotions : []).map((row) => {
    ...
    return '<tr><td class="fw-medium">' + escapeHtml(row.promotion || 'unassigned') + '</td>'
      + '<td class="text-end">' + rowTotal + '</td>'
      + '<td class="text-end">' + rowCovered + '</td>'
      + '<td class="text-end">' + (Number(row.upcoming) || 0) + '</td>'
      + '<td class="text-end"><span class="badge ' + (share >= 50 ? 'bg-green-lt' : share > 0 ? 'bg-orange-lt' : 'bg-red-lt')
      + '">' + shown + ' shown</span></td></tr>';
  }).join('') || '<tr><td colspan="5" class="text-secondary">No events stored yet.</td></tr>';

  const unavailable = cover.available === null || cover.available === undefined;
  const warning = unavailable
    ? '<div class="alert alert-warning">Availability data could not be read, so the gate would fail open and hide nothing.</div>'
    : '';

  return '<div class="card mb-3"><div class="card-header"><div>'
    + '<h3 class="card-title">Catalog availability gate</h3>'
    + '<div class="text-secondary small">The fixture feeds are a schedule; catalogs are meant to be a library. '
    + 'With the gate on, a catalog lists only events something has actually been found for.</div></div>'
    + '<div class="card-actions"><span class="badge ' + (state.enabled ? 'bg-green-lt' : 'bg-secondary-lt') + '">'
    + (state.enabled ? 'On' : 'Off') + '</span></div></div>'
    + '<div class="card-body">' + warning
    + '<p class="text-secondary mb-3">' + covered + ' of ' + total + ' stored events (' + percent
    + '%) have known content right now. Curated events are always shown.</p>'
    + '<form method="POST" action="/admin/database/catalog-gate">'
    + '<label class="form-check form-switch"><input class="form-check-input" type="checkbox" name="enabled" value="on"'
    + (state.enabled ? ' checked' : '') + '><span class="form-check-label">Only show events with known content</span></label>'
    + '<label class="form-check form-switch"><input class="form-check-input" type="checkbox" name="keepUpcoming" value="on"'
    + (state.keepUpcoming ? ' checked' : '') + '><span class="form-check-label">Keep future fixtures visible even with nothing found yet</span></label>'
    + '<button class="btn btn-primary mt-3" type="submit">Save gate</button>'
    + '</form></div>'
    + '<div class="table-responsive"><table class="table card-table table-vcenter">'
    + '<thead><tr><th>Promotion</th><th class="text-end">Events</th><th class="text-end">With content</th>'
    + '<th class="text-end">Future, none found</th><th class="text-end">Gate result</th></tr></thead>'
    + '<tbody>' + rows + '</tbody></table></div></div>';
}

function renderBody(input) {
  const data = input || {};
  if (data.maintenance) return '<h2>Database maintenance</h2><p>Discovery controls and recent searches are in <a href="/admin/discovery?tab=preparation">Discovery → Bitmagnet</a>.</p>' + (data.flash ? '<div class="alert alert-info">'+escapeHtml(data.flash)+'</div>' : '') + renderMaintenance();
  ...
  const searchRows = searches.map((row) => {
    ...
    const explanation = !hasOutcome ? 'Playback outcome has not been recorded yet.' : Number(row.readyCount)>0 ? 'Ready links were observed for the account used by this search.' : Number(row.matchedCount)>0 ? 'Releases matched, but no ready links were observed. Check provider credentials, cache availability and Logs.' : Number(row.resultCount)>0 ? 'Candidates were found, but none matched this event. Review matching rules and team aliases.' : 'No candidates were recorded. Check query coverage and provider errors in Logs.';
    const funnel = '<div class="d-flex align-items-center gap-1 flex-wrap">'
      + '<span class="badge bg-secondary-lt" title="Candidates returned by discovery">'
      + Number(row.resultCount || 0) + ' discovered</span>'
      + (hasOutcome
        ? '<span class="text-secondary">›</span><span class="badge bg-blue-lt" title="Candidates matching this event">'
          + Number(row.matchedCount || 0) + ' matched</span>'
          + '<span class="text-secondary">›</span><span class="badge bg-green-lt" title="Immediately available from the playback provider">'
          + Number(row.readyCount || 0) + ' ready</span>'
        : '<span class="badge bg-secondary-lt" title="Outcome detail will appear after the next search">outcome pending</span>')
      + '</div><div class="text-secondary small mt-1">'+escapeHtml(explanation)+'</div>';
  ...
  return '<div class="page-header"><div class="row align-items-center"><div class="col">'
    + '<h2 class="page-title">' + (data.discovery ? 'Bitmagnet' : 'Database') + '</h2>'
    ...
    + '</div>'
    + '<div class="row row-cards mb-3">'
    + '<div class="col-lg-7"><div class="card h-100"><div class="card-header"><h3 class="card-title">Bitmagnet preparation</h3>'
    ...
    + '<div class="col-lg-5"><div class="card h-100"><div class="card-header"><h3 class="card-title">Bitmagnet preparation settings</h3></div>'

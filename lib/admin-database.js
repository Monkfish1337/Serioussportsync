...
function renderBody(input) {
  const data = input || {};
  if (data.maintenance) return '<h2>Database maintenance</h2><p>Discovery controls and recent searches are in <a href="/admin/discovery?tab=preparation">Discovery → Preparation</a>.</p>' + (data.flash ? '<div class="alert alert-info">'+escapeHtml(data.flash)+'</div>' : '') + renderMaintenance();
  ...
  return '<div class="page-header"><div class="row align-items-center"><div class="col">'
    + '<h2 class="page-title">' + (data.discovery ? 'Automatic preparation' : 'Database') + '</h2>'

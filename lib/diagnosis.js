...
const CHECKS = {
  ...
  coverage(src, now) {
    ...
    for (const group of result.promotions || []) {
      ...
      const reasons = ranked.map(([r, n]) => n + ' · ' + r);
      ...
      out.push(finding(unselected ? 'warning' : (group.missing / group.total >= 0.5 ? 'critical' : 'warning'), 'Discovery',
        name + ': ' + group.missing + ' of ' + group.total + ' recent events have no saved torrent',
        unselected
          ? 'No background source searches this promotion, so nothing is saved ahead of time; torrents are found only by live search when an event is opened. Add it to the Prowlarr queue (or Bitmagnet preparation) if you want its events prepared.'
          : /awaiting retry/i.test(top)
            ? 'The Prowlarr queue searched these and found nothing usable. When a promotion that used to be covered drops suddenly, an indexer has usually stopped returning downloads — see the indexer findings.'
            : 'See the reasons below and the missing events list.',
        unselected
          ? { label: 'Prowlarr promotion selection', href: '/admin/discovery?tab=prowlarr' }
          : { label: 'Missing events', href: '/admin/discovery?tab=overview&promotion=' + encodeURIComponent(group.id) },
        reasons.slice(0, 3)));
    }
    return out;
  },
  ...
  indexers(src, now) {
    ...
    const out = [];
    // An indexer that used to save matches and has saved none lately, although
    // it is still being searched. 720pier's expired login looked exactly like
    // this: searches kept returning results, the torrents behind them could
    // not be downloaded, and an empty search reset the failure counter, so it
    // never showed as failing.
    const lastMatch = new Map();
    for (const event of status.matchedEvents || []) {
      for (const name of event.indexers || []) {
        if (!lastMatch.has(name) || event.at > lastMatch.get(name)) lastMatch.set(name, event.at);
      }
    }
    const flagged = new Set();
    for (const i of indexers) {
      const last = lastMatch.get(i.name);
      const searchedToday = Number(i.requests) > 0 && i.day === today(now);
      if ((Number(i.successes) || 0) >= 10 && searchedToday && last && now - last > 2 * DAY) {
        flagged.add(i.name);
        const top = Number(i.successes) === best;
        out.push(finding(top ? 'critical' : 'warning', 'Indexers',
          i.name + ' has not found a usable torrent since ' + new Date(last).toISOString().slice(0, 10) + (top ? ' (your most productive indexer)' : ''),
          'It is still being searched (' + i.requests + ' requests today) but nothing it returned has been saved for ' + Math.floor((now - last) / DAY) + ' days. For a login-based tracker this is usually an expired cookie or session in Prowlarr: its searches work, but the torrent downloads fail. Refresh the login in Prowlarr, then reset the cooldown here.',
          { label: 'Indexer budgets', href: '/admin/discovery?tab=prowlarr' },
          [i.successes + ' successful searches in total']));
      }
    }
    for (const i of indexers) {
      if (flagged.has(i.name) || (Number(i.failures) || 0) < 3) continue;
  ...
  learnedRules(src) {
    ...
    const names = new Map(Array.from(promotions.values()).map((p) => [p.id, p.name]));
    const out = [];
    for (const saved of src.overrides()) {
      // Judge the rules as they are used. A weekly show's saved rules are
      // stripped of episode numbers (S34E36, #1739) when they load, so those
      // entries are stored but never searched with.

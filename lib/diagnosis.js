    ...
    for (const group of result.promotions || []) {
      ...
      if (unselected) {
        ...
        out.push(finding('coverage:' + group.id, 'coverage', group.missing / group.total >= 0.5 ? 'critical' : 'warning', title, {
          ...
          steps: ['Fix any Sources findings first.', 'Troubleshoot one of the missing events to see where it stops.'],
          fix: { label: 'Missing events', href: '/admin/discovery?tab=overview&promotion=' + encodeURIComponent(group.id) }, evidence,
        }));
      }
    }
    return out;
...
function investigate(query, opts) {
  ...
  if (by.length || releases.length) {
    ...
    steps.push({ id: 'searched', label: 'Searched', verdict: 'warn',
      ...
      fix: queueSelected ? PROWLARR_TAB : { label: 'Add it to the Prowlarr queue', href: '/admin/discovery?tab=prowlarr' } });
  }
  ...
  if (releases.length) {
    ...
    steps.push({ id: 'found', label: 'Release found', verdict: 'fail', summary: 'No source returned a usable release.',
      ...
      fix: { label: 'Search by hand', href: '/admin/discovery/manual?eventId=' + id },
      links: [{ label: 'Check Sources', href: '/admin/diagnosis#stage-sources' }] });
  }

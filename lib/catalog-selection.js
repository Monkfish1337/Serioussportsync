...
function promotionSelectedForUser(userConfig, promotion) {
  if (!promotion) return false;
  if (promotion.autoTeam !== true) return true;
  const selected = userConfig && userConfig.teamPromotions;
  return Array.isArray(selected) && selected.includes(promotion.id);
}

function catalogSelectedForUser(userConfig, promotion, catalog, selection) {
  if (!promotionSelectedForUser(userConfig, promotion)) return false;
  const selected = selection === undefined ? effectiveCatalogSelection(userConfig) : selection;
  return !selected || selected.has(catalog.id);
}

module.exports = { effectiveCatalogSelection, promotionSelectedForUser,
  catalogSelectedForUser, CURRENT_DEFAULTS_VERSION };

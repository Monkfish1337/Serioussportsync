...
const { effectiveCatalogSelection, promotionSelectedForUser, catalogSelectedForUser } = require('./catalog-selection');
...
function buildManifest(opts) {
  ...
  // available: torrent discovery, per-user Built-in Usenet, or per-user Easynews.
  ...
  // Built-in Usenet requires a configured native indexer for discovery and a
  // configured native NNTP provider for playback.
  const haveDiyUsenet = !!(userCfg && diyUsenetStatus.status(userCfg).ready);
  const streamEnabled = haveTorrent || haveUuRows || haveEasynewsRows || haveDiyUsenet;

  const orderedPromotions = orderByIds(
    promotions.enabled.filter((p) => promotionSelectedForUser(userCfg, p)),
  ...
  const allCatalogs = [];
  // 0.95.0 — hiding a row is not the same as not serving it.
  //
  // showCatalogsOnHome was one boolean for every catalog, and the only other
  // control — the Catalogs step's switch — decided whether a catalog reached
  // the manifest at all. So "I don't want this on my home screen" could only be
  // expressed by removing the catalog, and a Nuvio collection folder pointing
  // at a catalog that is no longer in the manifest renders empty. That is the
  // reported bug: disable rows, push collections, open Nuvio, nothing there.
  // Enabling everything fixed the folders and left every catalog duplicated as
  // a home row underneath them.
  //
  // Two separate questions now: is it served (the catalogs selection), and does
  // it get a home row (this). A hidden-id list rather than a shown-id list, so
  // a catalog added in a later version appears by default instead of silently
  // not existing for everyone who saved before it shipped.
  const hiddenHomeRows = new Set(Array.isArray(userCfg && userCfg.homeRowsHidden)
    ? userCfg.homeRowsHidden : []);
  const showAllInHome = !(userCfg && userCfg.showCatalogsOnHome === false);
  ...
  for (const p of orderedPromotions) {
    ...
    for (const c of orderedCatalogs) {
      const showInHome = showAllInHome && !hiddenHomeRows.has(c.id);
      if (!catalogSelectedForUser(userCfg, p, c, selected)) continue;
      allCatalogs.push({
  ...
  const logo       = baseOrigin ? (baseOrigin + '/assets/logo.png?v=20260922')        : config.logo;
  const background = baseOrigin ? (baseOrigin + '/assets/logo-banner.png?v=20260922') : config.background;

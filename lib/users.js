...
// values from older installs are read transparently (crypto-keys.decrypt
...
const SECRET_FIELDS = [
  'rd', 'tb', 'pm', 'torboxApiKey', 'uuManifestUrl',
  'easynewsUsername', 'easynewsPassword',
  'diySearchApiKey', 'nntpUsername', 'nntpPassword',
];
const MAX_PASSWORD_LENGTH = 256;

function encryptUserKeysInPlace(user) {
  if (!user) return;
  // 0.53.0: the install/API token is a bearer credential too. Encrypt it at
  // rest while keeping the plaintext value available only in request memory.
  if (user.apiToken) user.apiToken = cryptoKeys.encrypt(user.apiToken);
  if (user.config) {
...
// Returns a SHALLOW clone with bearer/provider credentials decrypted. Never
// mutate the raw on-disk record.
function decryptedUser(user) {
  ...
  const clone = Object.assign({}, user, { config: cloneCfg });
  if (clone.apiToken) clone.apiToken = cryptoKeys.decrypt(clone.apiToken);
  return clone;
}
...
function loadAll() {
  try {
    ...
    catch (err) { if (err.code === 'ENOENT') return { users: [], updatedAt: null }; throw err; }
    const state = JSON.parse(raw);
    if (!state || !Array.isArray(state.users) || state.users.some(u => !u || typeof u.id !== 'string' || typeof u.username !== 'string' || typeof u.passwordHash !== 'string' || !['admin','user'].includes(u.role)) || ['invites','accessRequests','accessReviews'].some(key => state[key] !== undefined && !Array.isArray(state[key]))) throw new Error('Invalid account store');
    return state;
  } catch (err) {
    console.error('[users] account store unavailable:', err.code || err.name || 'invalid data');
    const error = new Error('Account storage is unavailable. Restore or repair users.json before continuing. Existing accounts have not been replaced.');
...
async function verifyPassword(plain, hash) {
  ...
  if (String(plain).length > MAX_PASSWORD_LENGTH) return false;
...
async function verifyDummy(plain) {
  try { await bcrypt.compare(String(plain || '').slice(0, MAX_PASSWORD_LENGTH), DUMMY_HASH); } catch {}
  return false;
}
...
async function createUser({ username, password, role, initialAdminOnly = false }) {
  if (!username || !password) throw new Error('username and password required');
  if (String(password).length < 8) throw new Error('password must be at least 8 characters');
  if (String(password).length > MAX_PASSWORD_LENGTH) throw new Error('password must be at most 256 characters');
  if (!/^[A-Za-z0-9_.-]{3,32}$/.test(username)) {
    throw new Error('username must be 3-32 chars, [A-Za-z0-9_.-]');
  }
  const passwordHash = await hashPassword(password);
  const state = loadAll();
  if (initialAdminOnly && state.users.length) throw new Error('Setup already complete.');
  ...
  const user = newUserRecord(username, passwordHash, finalRole);
  state.users.push(user);
  saveAll(state);
  return user;
}

function newUserRecord(username, passwordHash, finalRole) {
  return {
    ...
    lastSeen: null,
    // 0.33.0: torboxApiKey — per-user TorBox key used by the metadata addon
    // to resolve hashes returned by the companion scraper into playable
    // URLs. Per-user isolation preserved; key never leaves this addon.
    ...
    // Built-in Usenet (native indexer search + native NNTP playback) is the
    // one Usenet backend: diyUsenetEnabled is its Configure wizard switch.
    // Search and playback readiness come from their saved connection details.
    config: {
      torboxEnabled: true,
      ...
      catalogs: [],
      teamPromotions: [],
      // Explicit "every row switched off", which an empty `catalogs` cannot
      // express — that means "all". See lib/catalog-selection.js.
      catalogsNone: false,
      // Catalog ids that are served but kept off the home screen. A hidden
      // list, not a shown list, so catalogs added later appear by default.
      homeRowsHidden: [],
...
function listAccessRequests() {
  const state = loadAll(), cutoff = Date.now() - accessPolicy(state).expiryDays * 86400000;
  const current = (state.accessRequests || []).filter(r => Date.parse(r.createdAt) > cutoff);
  if (current.length !== (state.accessRequests || []).length) { state.accessRequests = current; saveAll(state); }
  return current.map(({ id, username, createdAt }) => ({ id, username, createdAt }));
}

function accessPolicy(state = loadAll()) {
  return { enabled: state.accessPolicy?.enabled !== false, expiryDays: Math.max(1, Math.min(90, Number(state.accessPolicy?.expiryDays) || 14)) };
}
function setAccessPolicy({enabled, expiryDays}) {
  const days = Number(expiryDays);
  if (!Number.isInteger(days) || days < 1 || days > 90) throw new Error('Choose an expiry from 1 to 90 days.');
  const state = loadAll(); state.accessPolicy = {enabled:enabled === true, expiryDays:days}; saveAll(state);
}
function listAccessReviews() { return (loadAll().accessReviews || []).slice().reverse(); }

async function requestAccess({ username, password }) {
  username = String(username || '').trim();
  password = String(password || '');
  if (!/^[A-Za-z0-9_.-]{3,32}$/.test(username)) throw new Error('Username must be 3–32 letters, numbers, dots, underscores or hyphens.');
  if (password.length < 8 || password.length > MAX_PASSWORD_LENGTH) throw new Error('Password must be 8–256 characters.');
  function check(state) {
    if (!accessPolicy(state).enabled) throw new Error('Access requests are closed. Contact the administrator.');
    if ((state.accessRequests || []).length >= 1000) throw new Error('Access requests are currently full. Contact the administrator.');
    if ([...state.users, ...(state.accessRequests || []), ...(state.invites || [])].some(u => u.username.toLowerCase() === username.toLowerCase())) throw new Error('That username is unavailable or already pending.');
  }
  listAccessRequests();
  check(loadAll());
  const passwordHash = await hashPassword(password);
  const state = loadAll();
  check(state); // Re-read after hashing so concurrent requests cannot overwrite accounts.
  state.accessRequests = [...(state.accessRequests || []), { id: crypto.randomBytes(16).toString('hex'), username, passwordHash, createdAt: new Date().toISOString() }];
  saveAll(state);
}

function reviewAccessRequest(id, approve, actor) {
  listAccessRequests();
  const state = loadAll();
  const request = (state.accessRequests || []).find(r => r.id === id);
  if (!request) throw new Error('Access request not found.');
  if (approve) {
    if (state.users.some(u => u.username.toLowerCase() === request.username.toLowerCase())) throw new Error('That username already has an account. Decline this request.');
    state.users.push(newUserRecord(request.username, request.passwordHash, 'user'));
  }
  state.accessRequests = state.accessRequests.filter(r => r.id !== id);
  state.accessReviews = [...(state.accessReviews || []), {username:request.username, outcome:approve ? 'approved' : 'declined', at:new Date().toISOString(), adminId:actor?.id || null, adminUsername:actor?.username || null}].slice(-500);
  saveAll(state);
...
function sanitize(user) {
  if (!user) return null;
  const safeToken = user.apiToken ? cryptoKeys.decrypt(user.apiToken) : '';
  return {
    id: user.id,
    ...
    apiTokenPreview: safeToken ? safeToken.slice(0, 8) + '…' : null,
    config: Object.fromEntries(Object.entries(user.config || {}).map(([key, value]) => [
      key, SECRET_FIELDS.includes(key) ? (value ? 'configured' : '') : value,
    ])),
  };
}
...
async function setPassword(id, newPlain) {
  ...
  if (String(newPlain).length > MAX_PASSWORD_LENGTH) {
...
module.exports = {
  accessPolicy, setAccessPolicy, listAccessReviews, requestAccess, listAccessRequests, reviewAccessRequest,

...
function listAccessRequests() {
  return (loadAll().accessRequests || []).map(({ id, username, createdAt }) => ({ id, username, createdAt }));
}
...
function reviewAccessRequest(id, approve) {
  const state = loadAll();
...
module.exports = {
  requestAccess, listAccessRequests, reviewAccessRequest,

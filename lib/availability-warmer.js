...
function configuredProfiles() {
  const profiles = users.listUsers().map((user) => users.findById(user.id)).filter(Boolean)
    .map((user) => ({ id: user.id, username: user.username, config: require('./diy-access').playbackConfig(user) }));
...
function profileIncludesEvent(profile, event) {
  ...
  if (!promotionSelectedForUser(profile.config, promotion)) return false;
  return promotion.catalogs.some((catalog) => (!selected || selected.has(catalog.id))
...
async function execute(options) {
  ...
  const ready = all.filter(event => {
    const promotion=promotions.getByEventId(event.id);
    return promotion && promotion.enabled!==false && !/cancel|postpon/i.test(event.status || '')
      && require('./discovery-cadence').readyAt(event)<=Date.parse(opts.now || new Date().toISOString())
      && settings.sourceDiscoveryIncludes('bitmagnet',event.id);
  });
  const batch = nextBatch(ready, opts.maxEvents || warmConfig.maxEventsPerRun);
  ...
  // Per-query chatter from the sources is indented; this run's own progress
  // and summaries are not. One line per query per event is roughly twenty
  // thousand lines a run against a five-thousand-line log buffer, which
  // flushes every stream request out of /admin/logs — measured at 37 seconds
  // of retained history and zero stream entries, exactly when a stream
  // request was what needed reading. Operators who want it can turn
  // "verbose index build" on under Logs.
  const verboseBuild = settings.getLogPreferences().verboseIndexBuild === true;
  const eventLog = (prefix) => (message) => {
    if (!verboseBuild && /^\s/.test(String(message || ''))) return;
    log(prefix + ': ' + message);
  };

  state.lastStartedAt = new Date().toISOString();
  state.lastReason = opts.reason || 'scheduled';
  state.eligibleEvents = ready.length;
    ...
    for (const profile of item.profiles) {
      ...
      try {
        ...
        const result = await prefetch({
          ...
          prepare: {...warmConfig,prepareUsenet:false,prepareEasynews:false},
          skipProviders,
          log: eventLog(profile.username || 'account'),
        });

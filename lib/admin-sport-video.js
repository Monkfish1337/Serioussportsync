...
function row(record, cached, torboxConfigured) {
  ...
  return '<tr data-search="' + escapeHtml([record.title, record.category, match && match.eventTitle, state].join(' ').toLowerCase())
    ...
    + '<td>' + matchHtml + '</td><td>' + badge(state)
      ...
      + (record.prepareRetryAt ? '<div class="text-secondary small">Automatic retry: ' + escapeHtml(require('./display-time').displayTime(record.prepareRetryAt)) + '</div>' : '')
...
function renderBody(data) {
  ...
  return flash
    ...
    // 0.90.7 — the control wall, folded.
    //
    // Fifteen inputs sat in one flat column, so the four that decide whether
    // Sport-Video does anything at all — the master switch, automatic
    // scanning, which sports, how often — were visually indistinguishable from
    // the startup delay. Everything else now folds into named sections.
    //
    // <details>, not `disabled` and not `display:none` on a fieldset: a closed
    // <details> still SUBMITS its inputs, where a disabled input submits
    // nothing and would silently clear the setting on the next save. That trap
    // has now cost this project two bugs, so the summary lines below carry the
    // current value — a section you cannot see must still tell you what it is
    // set to, or folding it away is just hiding state.
    +   '.sv-section{border:1px solid var(--tblr-border-color);border-radius:10px;margin-top:14px}'
    ...
    +   '.sv-section>summary::before{content:"\\25B8";display:inline-block;transition:transform .15s ease;color:var(--tblr-secondary)}'
    +   '.sv-section[open]>summary::before{transform:rotate(90deg)}'
    +   '.sv-section>summary .sv-section-state{font-weight:400;font-size:.8rem;color:var(--tblr-secondary)}'
    +   '.sv-section>.sv-section-body{padding:0 14px 15px}'
    + '</style>'
    ...
    + '<div class="row mt-3"><div class="col-sm-4"><label class="form-label">Every (hours)</label><input class="form-control" type="number" min="1" max="168" step="1" name="intervalHours" value="'
      + escapeHtml(cfg.intervalHours || 6) + '"></div></div>'

    // Warming submits torrents to TorBox on the operator's account, so it gets
    // its own section rather than being buried in "advanced" — and its summary
    // says plainly whether anything is armed.
    + section('Automatic warming', warmSummary,
      ...
      + '<div>' + warmChecks + '</div>'
      + '<div class="row mt-3"><div class="col-sm-6"><label class="form-label">Auto-warm releases per scan</label>'
        + '<input class="form-control" type="number" min="1" max="50" name="autoWarmPerScan" value="'
        + escapeHtml(autoWarmPerScan) + '">'
        + '<div class="form-hint">Caps TorBox submissions from a single scan.</div></div>'
      + '<div class="col-sm-6"><label class="form-label">Automatic window (days)</label>'
        + '<input class="form-control" type="number" min="1" max="90" name="autoWarmWindowDays" value="'
        + escapeHtml(autoWarmWindowDays) + '">'
        + '<div class="form-hint">Automatic preparation and warming stop for fixtures older than this. TorBox keeps a cached copy for at least 30 days, so an older fixture is either still cached and needs nothing, or has aged out. The Prepare and Warm buttons ignore this limit.</div></div></div>',
      (cfg.autoWarmPromotions || []).length > 0)

    + (teamFilterBlock
      ? section('Team limits', teamSummary, teamFilterBlock, teamFilterCount > 0)
      : '')

    + section('Advanced tuning', 'Startup delay, prepare limit, archive fallback',
      '<div class="row"><div class="col-sm-4"><label class="form-label">Startup delay (seconds)</label>'
        + '<input class="form-control" type="number" min="10" max="3600" name="startDelaySeconds" value="'
        + escapeHtml(cfg.startDelaySeconds || 90) + '">'
        + '<div class="form-hint">How long after boot the first scan runs.</div></div>'
      + '<div class="col-sm-4"><label class="form-label">Prepare per scan</label>'
        + '<input class="form-control" type="number" min="1" max="200" name="maxDetailsPerScan" value="'
        + escapeHtml(cfg.maxDetailsPerScan || 50) + '">'
        + '<div class="form-hint">Torrent detail fetches per scan.</div></div>'
      + '<div class="col-sm-4"><label class="form-label">Archive pages per scan</label>'
        + '<input class="form-control" type="number" min="0" max="60" name="archivePages" value="'
        + escapeHtml(archivePages) + '">'
        + '<div class="form-hint">Fallback only: used when the site search index cannot be read. About 60 releases per page, newest first. Set 0 to read the sport pages only.</div></div></div>',
      false)
    + '</div></div>'
    ...
    // Remember which sections were left open. An operator tuning one setting
    // reopens the same section on every save-and-reload, and re-folding it each
    // time is the kind of small friction that makes a page feel hostile. A
    // section holding a non-default value still opens by default on a fresh
    // browser; this only remembers a deliberate change. Wrapped because
    ...
    // 0.90.7 — eleven equal-weight rows became a state line, the counts that
    // explain a disappointing scan, and a fold for the rest.
    //
    // Every field was drawn identically, so "Last error" and "Archive pages
    // read" carried the same visual weight and the panel had to be read
    // top-to-bottom to answer "did the scan work?". The three that answer it
    // are pulled out; the four that explain WHY a release you expected is
    // missing are a compact grid; the diagnostics that only matter when
    // something is actually wrong fold away.
    ...
    + '<div class="d-flex align-items-baseline justify-content-between gap-2">'
      + '<div class="text-secondary text-uppercase small fw-bold">' + escapeHtml(status.current || 'Idle') + '</div>'
      + '<div class="text-secondary small">' + escapeHtml(formatTime(status.completedAt)) + '</div>'
    + '</div>'
    + (status.lastError
      ? '<div class="alert alert-danger mt-2 mb-0 py-2"><div class="small fw-bold">Last error</div>'
        + '<div class="small">' + escapeHtml(status.lastError) + '</div></div>'
      : '<div class="text-success small mt-1">No errors on the last run</div>')

    // The four numbers that explain a release you expected and did not get.
    ...
    + [
      ...
      ['Team filter', status.outsideTeamFilter || 0, 'Skipped by team limits'],
    ].map(function counter(entry) {
      return '<div class="col-6"><div class="border rounded p-2">'
        + '<div class="h3 m-0">' + escapeHtml(entry[1]) + '</div>'
        + '<div class="small fw-medium">' + escapeHtml(entry[0]) + '</div>'
        + '<div class="text-secondary" style="font-size:.72rem">' + escapeHtml(entry[2]) + '</div>'
        + '</div></div>';
    }).join('')
    + '</div>'

    + section('Scan details',
      escapeHtml(String(status.discoverySource || 'Not yet run')),
      '<div class="datagrid">'
      ...
      + '<div class="datagrid-item"><div class="datagrid-title">Pages with several torrents</div><div class="datagrid-content">'
        ...
        + '<div class="text-secondary small">Only the first torrent on a page is used.</div></div></div>'
      + '</div>'
      + ((status.multiVariantExamples || []).length
        ? '<div class="mt-2 small"><div class="fw-medium">Recent pages with several torrents</div><ul class="mb-0">'
          + status.multiVariantExamples.map(function example(item) {
            return '<li>' + escapeHtml(item.title) + ' · ' + escapeHtml(item.count) + ' torrents'
              + (item.names.length ? '<div class="text-secondary text-break">' + item.names.map(escapeHtml).join('<br>') + '</div>' : '')
              + '</li>';
          }).join('') + '</ul></div>'
        : ''),
      false)

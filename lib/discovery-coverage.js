...
function recentEvents(events,now=Date.now()) {
  return [...new Map(events.filter(e=>{
    ...
    const at=Date.parse(e.date+(time?'T'+String(time).slice(0,8)+'Z':'T23:59:59Z'));
    return Number.isFinite(at) && at<=now && at>=now-7*DAY;
  }).map(e=>[e.id,e])).values()];
}
function coverage(data,now=Date.now()) {
  const events=recentEvents(data.events,now),byId=new Map(events.map(e=>[e.id,e]));
  ...
  for(const e of data.queue.eventStates || []) if(e.matched && sources.has(e.id)) {titles.add(e.id);if(e.seeded) sources.get(e.id).add('Prowlarr');}
  for(const e of data.queue.matchedEvents || []) if(sources.has(e.event || e.id)) {titles.add(e.event || e.id);if(e.warmable) sources.get(e.event || e.id).add('Prowlarr');}
  ...
  const rows=events.map(event=>{
    ...
    else if(p?.enabled===false) reason='Promotion disabled';
    else if(state) reason=state.state;
    else if(titles.has(event.id)) reason='Title matched — usable release identity not prepared';

...
function describe(source) {
  ...
  if (source.type === 'api-football') return 'Competition ' + source.leagueId;
  if (source.type === 'uefa') return 'Official competition ' + source.competitionId + ' · no key required';
  ...
  if (source.type === 'json-feed') return 'Custom JSON/API · ' + source.url;
...
function renderBody(opts) {
  ...
  return `
    <div class="page-header"><div class="row align-items-center g-3"><div class="col"><h2 class="page-title">Metadata</h2><p class="text-secondary mt-1">Manage reusable event providers here, then assign one while creating or editing a promotion.</p></div><div class="col-auto"><a class="btn btn-primary" href="#providerCreator">Create provider</a></div></div></div>
    ${flash}
    ...
    <div class="card" id="providerCreator"><div class="card-header"><div><div class="text-uppercase text-primary fw-bold small">Provider creator</div><h3 class="card-title mt-1">Create an event provider</h3></div></div><div class="card-body">
      <p class="text-secondary">Use a ready-made adapter, or connect any public JSON/API schedule by showing SSS where its event list, names, and dates live. Test results before saving; no catalog is changed by a test.</p>
      <form method="POST" action="/admin/metadata-sources/create" id="metadataSourceForm">
        <div class="row g-3">
          ...
          <div class="col-md-4"><label class="form-label" for="m-type">Provider type</label><select class="form-select" id="m-type" name="type"><optgroup label="Build your own"><option value="json-feed">Custom JSON/API schedule</option></optgroup><optgroup label="Ready-made adapters"><option value="uefa">UEFA official competition</option><option value="api-football">API-Football competition</option><option value="mlb">MLB official schedule</option><option value="thesportsdb">TheSportsDB league</option><option value="football-data">football-data.org</option><option value="tmdb">TMDB TV series</option><option value="onefc">ONE official schedule</option></optgroup></select></div>
        </div>
        <div class="adapter-fields mt-3" data-adapter="json-feed">
          <div class="alert alert-info"><strong>Connect a public JSON schedule</strong><div class="small mt-1">Open the API URL in a browser, then enter the dotted paths visible in its JSON. SSS never runs code from the provider.</div></div>
          <div class="row g-3">
            <div class="col-12"><label class="form-label" for="m-json-url">JSON/API URL</label><input class="form-control text-mono" type="url" id="m-json-url" name="url" placeholder="https://api.example.com/events"></div>
            <div class="col-md-6"><label class="form-label" for="m-array-path">Event list path</label><input class="form-control text-mono" id="m-array-path" name="arrayPath" placeholder="data.events"><small class="text-secondary">Leave blank when the response itself is an array.</small></div>
            <div class="col-md-6"><label class="form-label" for="m-id-field">Unique event ID field <span class="text-secondary">optional</span></label><input class="form-control text-mono" id="m-id-field" name="idField" placeholder="id"></div>
            <div class="col-md-6"><label class="form-label" for="m-name-field">Event name field</label><input class="form-control text-mono" id="m-name-field" name="nameField" placeholder="title"></div>
            <div class="col-md-6"><label class="form-label" for="m-date-field">Event date field</label><input class="form-control text-mono" id="m-date-field" name="dateField" placeholder="start.date"></div>
            <div class="col-md-4"><label class="form-label" for="m-time-field">Start time field <span class="text-secondary">optional</span></label><input class="form-control text-mono" id="m-time-field" name="timeField" placeholder="start.time"></div>
            <div class="col-md-4"><label class="form-label" for="m-venue-field">Venue field <span class="text-secondary">optional</span></label><input class="form-control text-mono" id="m-venue-field" name="venueField" placeholder="venue.name"></div>
            <div class="col-md-4"><label class="form-label" for="m-poster-field">Artwork field <span class="text-secondary">optional</span></label><input class="form-control text-mono" id="m-poster-field" name="posterField" placeholder="images.poster"></div>
            <div class="col-12"><label class="form-label" for="m-description-field">Description field <span class="text-secondary">optional</span></label><input class="form-control text-mono" id="m-description-field" name="descriptionField" placeholder="description"></div>
          </div>
        </div>
        ...
        <div class="adapter-fields mt-3" data-adapter="api-football"><label class="form-label">API-Football competition ID</label><input class="form-control" name="apiFootballLeagueId" placeholder="2"><small class="text-secondary">Champions League is competition 2. Find other IDs in the API-Football dashboard.</small></div>
        <div class="adapter-fields mt-3" data-adapter="uefa"><label class="form-label">UEFA competition ID</label><input class="form-control" name="uefaCompetitionId" placeholder="1"><small class="text-secondary">Champions League is competition 1. Uses UEFA's official public fixture feed; no account or API key is required.</small></div>
        ...
        <div class="d-flex gap-2 mt-3"><button class="btn btn-outline-info" id="previewDraftSource" type="button">Test provider &amp; show events</button><button class="btn btn-primary" type="submit">Save provider</button></div>
      </form>
    </div></div>
    <script>(function(){

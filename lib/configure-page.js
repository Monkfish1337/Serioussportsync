...
function clientScript(at, isFirstRun) {
  return '(' + String(function (startAt, firstRun) {
    ...
    document.addEventListener('click', function (e) {
      ...
      // The folder endpoint accepts the same field name as the standalone
      // editor. Sending `artwork` left artworkChoice empty, so every wizard
      // save was rejected and the later Nuvio push had nothing to update.
      body.append('artworkChoice', box.getAttribute('data-artwork'));

...
// ---------------------------------------------------------------- step 3
function stepCatalogs(promotionList, selected, selectAll, folderOf, hiddenHomeRows) {
  const hidden = hiddenHomeRows instanceof Set ? hiddenHomeRows : new Set(hiddenHomeRows || []);
  const rows = promotionList.map((p) => {
    const ids = (p.catalogs || []).map((c) => c.id);
    const on = selectAll || ids.some((id) => selected.has(id));
    // A promotion is "on the home screen" if any of its catalogs is not hidden.
    const homeOn = ids.some((id) => !hidden.has(id));
    ...
    return '<div class="row" draggable="true" data-id="' + escapeHtml(p.id) + '">'
      ...
      // Two questions, not one. "Served" puts the catalog in the manifest at
      // all; "Home row" decides whether it also gets a row on the home screen.
      // They were the same switch, so the only way to clear the home screen was
      // to stop serving the catalog — and a Nuvio collection folder pointing at
      // a catalog that is not in the manifest renders empty. That is why
      // pushing collections with rows disabled produced empty folders.
      + '<label class="sw" title="Serve this promotion\'s catalogs at all">'
      + '<input type="checkbox" class="promo-on"'
      + ' data-id="' + escapeHtml(p.id) + '"' + (on ? ' checked' : '')
      + ' aria-label="Serve ' + escapeHtml(p.name) + '"><i></i>'
      + '<b style="font-size:12px;color:var(--ink-3)">Served</b></label>'
      + '<label class="sw" title="Also give it a row on the Nuvio home screen">'
      + '<input type="checkbox" class="promo-home"'
      + ' data-id="' + escapeHtml(p.id) + '"' + (homeOn ? ' checked' : '')
      + ' aria-label="Home row for ' + escapeHtml(p.name) + '"><i></i>'
  ...
  return ''
    ...
    + '<p>Drag to set the order they appear in. <strong>Served</strong> decides whether a catalog exists for your client at all; <strong>Home row</strong> decides whether it also gets a row on the home screen. A catalog you have filed into a Nuvio folder needs to stay served, or the folder opens empty — turn its home row off instead.</p></div>'
    + '<div class="panel">'
    + '<div class="panel-head"><div><h3>Your rows</h3><p id="catalog-count"></p></div>'
    + '<div style="margin-left:auto;display:flex;gap:8px;align-items:center;flex-wrap:wrap">'
    // Twenty-nine promotions is a lot of clicking to get to "just the two I
    // watch", and the same again to undo it.
    + '<button class="btn ghost sm" type="button" id="catalogs-all">Enable all</button>'
    + '<button class="btn ghost sm" type="button" id="catalogs-none">Disable all</button>'
    + '<button class="btn ghost sm" type="button" id="home-rows-all">All home rows</button>'
    + '<button class="btn ghost sm" type="button" id="home-rows-none">No home rows</button>'
    + '</div></div>'
    + '<div class="rows" id="catalog-rows">' + rows + '</div>'
    + '<div class="panel-foot"><div class="hint" style="margin:0">'
    + '<strong>Served</strong>, <strong>on the home screen</strong> and <strong>in a collection</strong> are three separate things. '
    + 'Un-serving a catalog removes it from your manifest, which empties any collection folder pointing at it. '
    + 'To tidy the home screen, switch off the home row and leave it served.'
    + '</div></div></div>'
    ...
    // The marker that tells /account/save this form carried the catalog step,
    // so that zero catalogs posted means "the user switched them all off"
    // rather than "this form had no catalog fields".
    + '<input type="hidden" name="catalogSelection" value="1">'
    + '<div id="catalog-inputs"></div>';
}

// ---------------------------------------------------------------- step 4
// 0.95.0 — the whole collection editor lives here now.
//
// It used to be a read-only list with one "Save folder" button, and everything
// else — renaming a folder, changing its artwork, creating one, deleting one,
// the collection title and backdrop — was on /admin/nuvio-collections in the
// old Tabler markup. So the step that is *about* Nuvio folders could not
// actually edit them, and the two pages disagreed about what a folder was.
// Same endpoints, one screen.
//
// Nothing here carries a `name` attribute. These controls sit inside the
// Configure form's DOM, and anything named would be posted to /account/save
// alongside the account settings. Each control is addressed by `data-f` and
// read by the client when its own Save is pressed.
const ARTWORK_CHOICES = [
  { value: 'promotion', label: 'First promotion artwork' },
  ...
  { value: '/assets/collection-big-3.png', label: 'Big 3' },
  { value: '/assets/collection-unmatched.png', label: 'Unmatched' },
  { value: 'custom', label: 'Custom image URL' },
];
const TILE_SHAPES = ['landscape', 'square', 'poster'];

function artworkSplit(artwork) {
  const known = ARTWORK_CHOICES.some((c) => c.value === artwork && c.value !== 'custom');
...
function memberList(promotionList, chosen) {
  if (!promotionList.length) return '<div class="hint">No promotions yet.</div>';
  // Capped height with its own scroll. Unbounded, a 29-promotion list grew out
  // of its grid cell and ran down across the folders beside it.
  return '<div class="fold-body" style="max-height:260px;overflow:auto">'
    + promotionList.map((p) => '<label class="sw" style="display:flex;padding:4px 0">'
...
function folderEditor(folder, promotionList, byId) {
  const chosen = new Set(folder.promotions || []);
  const art = artworkSplit(folder.artwork);
  const names = Array.from(chosen).map((id) => byId[id]).filter(Boolean);
  // A folder can hold an id whose promotion is gone or switched off. Saying so
  // beats a count that disagrees with the list beside it, which is what "1"
  // over "Empty — not exported" was.
  const missing = chosen.size - names.length;
  const summary = names.join(' · ')
    + (missing > 0 ? (names.length ? ' · ' : '') + missing + ' no longer available' : '')
    || 'Empty — not exported';

  return '<div class="panel" style="margin:0" data-folder="' + escapeHtml(folder.id) + '">'
    + '<div class="panel-head"><div><h3>' + escapeHtml(folder.title) + '</h3>'
    + '<p>' + escapeHtml(summary) + '</p></div>'
    + '<span class="chip plain" data-tone="' + (names.length ? 'ok' : 'off') + '" style="margin-left:auto">'
    + names.length + '</span></div>'
    + '<div class="panel-body">'
    + '<div class="grid-2" style="gap:12px">'
    + '<label class="f"><span>Folder title</span>'
    + '<input class="t" data-f="title" maxlength="80" value="' + escapeHtml(folder.title) + '"></label>'
    + '<label class="f"><span>Tile shape</span><select class="t" data-f="tileShape">'
    + selectOptions(TILE_SHAPES, folder.tileShape || 'landscape') + '</select></label>'
    + '<label class="f"><span>Folder image</span><select class="t" data-f="artworkChoice">'
    + selectOptions(ARTWORK_CHOICES, art.choice) + '</select></label>'
    + '<label class="f" data-custom-art' + (art.choice === 'custom' ? '' : ' hidden') + '>'
    + '<span>Custom image URL</span>'
    + '<input class="t mono" type="url" data-f="customArtwork" value="' + escapeHtml(art.custom) + '"'
    + ' placeholder="https://example.com/folder.jpg"></label>'
    + '</div>'
    + '<label class="sw" style="margin-top:12px"><input type="checkbox" data-f="hideTitle"'
    + (folder.hideTitle ? ' checked' : '') + '><i></i>'
    + '<b>Hide the title over the artwork</b></label>'
    + '<details class="fold" style="margin-top:12px"><summary><div class="row-main">'
    + '<h3 style="font-size:14px">Catalogs in this folder</h3>'
    + '<span class="row-sub">A promotion belongs to one folder — adding it here takes it out of any other</span>'
    + '</div></summary>' + memberList(promotionList, chosen) + '</details>'
    + '<div style="margin-top:12px;display:flex;gap:10px;align-items:center;flex-wrap:wrap">'
    + '<button class="btn ghost sm" type="button" data-save-folder="' + escapeHtml(folder.id) + '">Save folder</button>'
    + '<button class="btn ghost sm" type="button" data-remove-folder="' + escapeHtml(folder.id) + '">Remove</button>'
...
function stepCollections(collections, promotionList, isAdmin) {
  ...
  const folderList = collections.folders || [];
  const backdrop = artworkSplit(collection.backdropImage);

  const assigned = new Set(folderList.flatMap((f) => f.promotions || []));
  const unassigned = promotionList.filter((p) => !assigned.has(p.id));
  const unassignedNote = unassigned.length
    ? '<div class="note" data-tone="warn"><div><b>' + unassigned.length
      + ' catalog' + (unassigned.length === 1 ? '' : 's') + ' in no folder</b>'
      + '<span>' + escapeHtml(unassigned.map((p) => p.name).join(', '))
      + '. They still appear as their own rows; they are just not grouped.</span></div></div>'
    : '';

  const head = '<div class="lede"><h2>Nuvio collections</h2>'
  ...
  const push = '<div style="margin-top:22px">' + nuvioAccount.panel() + '</div>';

  if (!isAdmin) {
    const readOnly = folderList.map((folder) => {
      const names = (folder.promotions || []).map((id) => byId[id]).filter(Boolean);
      return '<div class="panel" style="margin:0"><div class="panel-head"><div>'
        + '<h3>' + escapeHtml(folder.title) + '</h3>'
        + '<p>' + escapeHtml(names.join(' · ') || 'Empty — not exported') + '</p></div>'
        + '<span class="chip plain" data-tone="off" style="margin-left:auto">' + names.length + '</span>'
        + '</div></div>';
    }).join('');
    return head + unassignedNote
      + '<div class="grid-2" style="margin-top:16px">' + (readOnly || '<div class="hint">No folders yet.</div>') + '</div>'
      + '<div class="hint" style="margin-top:12px">Folders are shared by everyone on this server, so an admin edits them.</div>'
      + push;
  }

  const settingsPanel = '<div class="panel" id="collection-settings">'
    + '<div class="panel-head"><div><h3>The collection itself</h3>'
    + '<p>How the whole SeriousSportSync collection appears in Nuvio</p></div>'
    + '<span class="row-sub" data-collection-state style="margin-left:auto"></span></div>'
    + '<div class="panel-body">'
    + '<div class="grid-2" style="gap:12px">'
    + '<label class="f"><span>Collection title</span>'
    + '<input class="t" data-c="title" maxlength="80" value="' + escapeHtml(collection.title || '') + '"></label>'
    + '<label class="f"><span>Backdrop image</span>'
    + '<input class="t mono" data-c="backdropImage" value="'
    + escapeHtml(backdrop.custom || collection.backdropImage || '') + '"'
    + ' placeholder="/assets/logo-banner.png"></label>'
    + '</div>'
    + '<div style="display:flex;gap:22px;flex-wrap:wrap;margin-top:12px">'
    + '<label class="sw"><input type="checkbox" data-c="pinToTop"'
    + (collection.pinToTop ? ' checked' : '') + '><i></i><b>Pin to the top</b></label>'
    + '<label class="sw"><input type="checkbox" data-c="showAllTab"'
    + (collection.showAllTab !== false ? ' checked' : '') + '><i></i><b>Show the All tab</b></label>'
    + '</div>'
    + '<div style="margin-top:14px"><button class="btn ghost" type="button" id="save-collection">Save collection</button></div>'
    + '</div></div>';

  const newFolder = '<div class="panel" data-folder="" id="new-folder">'
    + '<div class="panel-head"><div><h3>New folder</h3>'
    + '<p>Group catalogs that have no folder yet</p></div></div>'
    + '<div class="panel-body">'
    + '<div class="grid-2" style="gap:12px">'
    + '<label class="f"><span>Folder title</span>'
    + '<input class="t" data-f="title" maxlength="80" placeholder="Major League Baseball"></label>'
    + '<label class="f"><span>Tile shape</span><select class="t" data-f="tileShape">'
    + selectOptions(TILE_SHAPES, 'landscape') + '</select></label>'
    + '<label class="f"><span>Folder image</span><select class="t" data-f="artworkChoice">'
    + selectOptions(ARTWORK_CHOICES, '/assets/logo-banner.png') + '</select></label>'
    + '<label class="f" data-custom-art hidden><span>Custom image URL</span>'
    + '<input class="t mono" type="url" data-f="customArtwork" placeholder="https://example.com/folder.jpg"></label>'
    + '</div>'
    + '<label class="sw" style="margin-top:12px"><input type="checkbox" data-f="hideTitle"><i></i>'
    + '<b>Hide the title over the artwork</b></label>'
    + '<details class="fold" style="margin-top:12px"><summary><div class="row-main">'
    + '<h3 style="font-size:14px">Catalogs in this folder</h3></div></summary>'
    + memberList(promotionList, new Set()) + '</details>'
    + '<div style="margin-top:12px;display:flex;gap:10px;align-items:center">'
    + '<button class="btn ghost sm" type="button" data-save-folder="">Create folder</button>'
    + '<span class="row-sub" data-folder-state=""></span></div>'
    + '</div></div>';

  const folders = folderList.map((f) => folderEditor(f, promotionList, byId)).join('');

  return head + unassignedNote
    + '<div style="margin-top:16px">' + settingsPanel + '</div>'
    + '<div class="grid-2" style="margin-top:16px;align-items:start">' + folders + '</div>'
    + '<div style="margin-top:16px">' + newFolder + '</div>'
    + push;
}

// ---------------------------------------------------------------- step 5
function stepInstall(installUrl, eventCount) {
  ...
  return ''
    ...
    // A brand-new install has nothing to serve yet, and nothing used to say so.
    // The first refresh walks 29 promotions one at a time and the TheSportsDB
    // adapter waits between requests, so the rows stay empty for a while after
    // setup finishes. Handing over a manifest without mentioning that makes a
    // working install look like a broken one — and the next thing the user does
    // is run the check below, which then reports that it has no fixture to try.
    + (eventCount === 0
      ? '<div class="note" data-tone="warn"><div><b>Your catalogs are still filling</b>'
        + '<span>No fixtures have been stored yet. The first refresh works through '
        + 'every promotion in turn and takes a while — install the manifest now if '
        + 'you like, the rows fill in on their own. Promotions shows the count per '
        + 'league as it goes.</span></div></div>'
      : '')
...
function render(input) {
  ...
  const panels = [
    ...
    stepCatalogs(data.promotions || [], data.selected || new Set(), data.selectAll, data.folderOf || {}, data.hiddenHomeRows),
    stepCollections(data.collections || {}, data.promotions || [], !!data.isAdmin),
    stepInstall(data.installUrl || '', Number(data.eventCount) || 0),
...
function clientScript(at, isFirstRun) {
  return '(' + String(function (startAt, firstRun) {
    ...
    function syncCatalogs() {
      ...
      // Hidden home rows are posted as their own list. A catalog can be served
      // and hidden at the same time, which is the whole point.
      var hiddenHome = [];
      Array.prototype.forEach.call(rows.children, function (row) {
        var home = row.querySelector('.promo-home');
        if (home && !home.checked) {
          row.querySelectorAll('.cat-id').forEach(function (input) { hiddenHome.push(input.value); });
        }
      });
      ...
      }).join('') + hiddenHome.map(function (id) {
        return '<input type="hidden" name="homeRowsHidden" value="' + id.replace(/"/g, '&quot;') + '">';
      }).join('') + '<input type="hidden" name="showCatalogsOnHome" value="on">';
      var live = rows.querySelectorAll('.promo-on:checked').length;
      document.getElementById('catalog-count').textContent = live === 0
        ? 'No rows — your catalogs will be empty until you switch some back on'
        : live + ' of ' + rows.children.length + ' promotions served';
    }

    function setAllCatalogs(on) {
      rows.querySelectorAll('.promo-on').forEach(function (input) { input.checked = on; });
      syncCatalogs();
    }
    var homeAll = document.getElementById('home-rows-all');
    var homeNone = document.getElementById('home-rows-none');
    function setAllHomeRows(on) {
      rows.querySelectorAll('.promo-home').forEach(function (input) { input.checked = on; });
      syncCatalogs();
    }
    if (homeAll) homeAll.addEventListener('click', function () { setAllHomeRows(true); });
    if (homeNone) homeNone.addEventListener('click', function () { setAllHomeRows(false); });
    var allBtn = document.getElementById('catalogs-all');
    var noneBtn = document.getElementById('catalogs-none');
    if (allBtn) allBtn.addEventListener('click', function () { setAllCatalogs(true); });
    if (noneBtn) noneBtn.addEventListener('click', function () { setAllCatalogs(false); });
          ...
          .then(function (data) {
            ...
            host.innerHTML = data.teams.map(function (team) {
              ...
              return '<button type="button" class="team" data-team="' + team.id + '" data-key="' + key + '"'
                ...
                // A tinted border alone was too quiet to answer "which ones did
                // I pick?" at a glance across twenty or thirty tiles.
                + '<span class="team-tick" aria-hidden="true">✓</span></button>';
            }).join('');
          })
    ...
    /* Collections ------------------------------------------------------ */
    // No nested <form> anywhere in this step: these panels sit inside the
    // Configure form, and a form inside a form is not valid HTML — the browser
    // closes the outer one at the first </form>, which is exactly how the Save
    // button was detached from Configure in 0.89.0. Everything posts by fetch.
    function collectionsPost(url, body, state, done) {
      if (state) state.textContent = 'Saving…';
      fetch(url, {
        method: 'POST', body: body, redirect: 'follow',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
          // Without this the endpoint answers with a 302 to the admin page,
          // fetch follows it, and a REJECTED save arrives here as HTTP 200.
          // Every failure reported itself as "Saved" and the user was left
        ...
        if (data && data.ok) { if (done) done(); return; }
        if (state) state.textContent = (data && data.error) || 'Could not save';
      }).catch(function () {
        if (state) state.textContent = 'Could not save';
      });
    }

    // Reload rather than patch the DOM. Saving a folder can move a promotion
    // out of another folder, so more changes than the one just made need to be
    // on screen; re-rendering from the server is the only honest way to show
    // that.
    function reloadCollections() { window.location.href = '/account?step=3&flash=saved'; }

    function folderBody(box) {
      var body = new URLSearchParams();
      var get = function (field) { return box.querySelector('[data-f="' + field + '"]'); };
      body.append('title', get('title').value);
      body.append('artworkChoice', get('artworkChoice').value);
      body.append('customArtwork', get('customArtwork') ? get('customArtwork').value : '');
      body.append('tileShape', get('tileShape').value);
      // hideTitle was never sent, so every save from this page silently
      // switched it back off.
      if (get('hideTitle').checked) body.append('hideTitle', '1');
    ...
    document.addEventListener('change', function (e) {
      var select = e.target.closest && e.target.closest('[data-f="artworkChoice"]');
      if (!select) return;
      var custom = select.closest('.panel').querySelector('[data-custom-art]');
    ...
    document.addEventListener('click', function (e) {
      ...
      var box = btn.closest('[data-folder]');
      var state = box.querySelector('[data-folder-state]');
      var url = id
        ? '/admin/nuvio-collections/folders/' + encodeURIComponent(id) + '/save'
        : '/admin/nuvio-collections/folders/create';
      btn.disabled = true;
      collectionsPost(url, folderBody(box), state, reloadCollections);
      setTimeout(function () { btn.disabled = false; }, 1500);
    });

    document.addEventListener('click', function (e) {
      var btn = e.target.closest && e.target.closest('[data-remove-folder]');
      if (!btn) return;
      var id = btn.getAttribute('data-remove-folder');
      if (!window.confirm('Remove this folder from the Nuvio export?\n\n'
        + 'The promotions in it and their events are not deleted — they keep their own rows.')) return;
      var state = btn.closest('[data-folder]').querySelector('[data-folder-state]');
      btn.disabled = true;
      collectionsPost('/admin/nuvio-collections/folders/' + encodeURIComponent(id) + '/delete',
        new URLSearchParams(), state, reloadCollections);
      setTimeout(function () { btn.disabled = false; }, 1500);
    });

    var saveCollection = document.getElementById('save-collection');
    if (saveCollection) saveCollection.addEventListener('click', function () {
      var panel = document.getElementById('collection-settings');
      var state = panel.querySelector('[data-collection-state]');
      var get = function (field) { return panel.querySelector('[data-c="' + field + '"]'); };
      var body = new URLSearchParams();
      body.append('title', get('title').value);
      body.append('backdropImage', get('backdropImage').value);
      if (get('pinToTop').checked) body.append('pinToTop', '1');
      if (get('showAllTab').checked) body.append('showAllTab', '1');
      saveCollection.disabled = true;
      collectionsPost('/admin/nuvio-collections/save', body, state, reloadCollections);
      setTimeout(function () { saveCollection.disabled = false; }, 1500);
    });
    ...
    if (verify) verify.addEventListener('click', function () {
      ...
      out.innerHTML = '<div class="hint">Searching every configured pipeline, with a longer deadline than '
        + 'a real stream request gets — so a slow provider shows up as slow rather than as missing. '
        + 'This can take up to 30 seconds.</div>';
      fetch('/account/verify', { method: 'POST', headers: { Accept: 'application/json' } })
        ...
        .then(function (data) {
          ...
          out.innerHTML =
            ...
            + '</div>'
            // Which discovery sources were consulted. A switched-off source and
            // an empty index used to read identically, because every torrent
            // source is folded into the TorBox row.
            + (data.discovery && data.discovery.length
              ? '<div class="hint" style="margin-top:10px"><strong>Torrent sources asked:</strong> '
                + (data.discovery.filter(function (d) { return d.asked; })
                    .map(function (d) { return d.name; }).join(', ') || 'none — nothing was searched')
                + '</div>'
              : '')
            + (any
              ? '<div class="hint">' + data.total + ' stream'
                + (data.total === 1 ? '' : 's') + ' returned for '
                + data.event.name + '.'
                + (data.attempts && data.attempts.length > 1
                  ? ' Checked ' + data.attempts.length + ' fixtures and stopped at the first that returned something.'
                  : '')
                + '</div>'
              // Several fixtures, from different promotions, all empty. That is
              // worth saying plainly, because one miss never meant anything and
              // the old copy had to spend a paragraph admitting it.
              : '<div class="note" data-tone="warn" style="margin-top:10px"><div>'
                + '<b>Nothing found for any of the ' + (data.attempts ? data.attempts.length : 1)
                + ' fixtures checked</b><span>'
                + (data.attempts || []).map(function (a) {
                    return a.promotion + ' — ' + a.name + ' (' + a.date + '): '
                      + (a.error ? a.error : a.rows + ' streams');
                  }).join('<br>')
                + '<br><br>Across different promotions, that points at the configuration rather '
                + 'than at the fixtures. Check the credentials on step 1 and the torrent sources '
                + 'listed above.</span></div></div>');
        })
        .catch(function () {

...
function stepCatalogs(promotionList, selected, selectAll, folderOf) {
  const rows = promotionList.map((p) => {
    ...
    return '<div class="row" draggable="true" data-id="' + escapeHtml(p.id) + '">'
      ...
      + (p.poster
        ? '<img class="tile" src="' + escapeHtml(p.poster) + '" alt="" loading="lazy">'
        : '<span class="tile">' + escapeHtml(p.name.slice(0, 3).toUpperCase()) + '</span>')
      + '<span class="row-main"><span class="row-name">' + escapeHtml(p.name) + '</span>'
...
function stepInstall(installUrl) {
  ...
  return ''
    ...
    + '<p>Runs one real fixture from your catalogs through every pipeline you configured</p></div>'
...
function render(input) {
  ...
  return shell.page({
    ...
    body: body,
    actions: actions,
    script: clientScript(at, isFirstRun),
  });
}
    ...
    if (verify) verify.addEventListener('click', function () {
      ...
      fetch('/account/verify', { method: 'POST', headers: { Accept: 'application/json' } })
        ...
        .then(function (data) {
          ...
          var any = data.pipelines.some(function (p) { return p.rows > 0; });
          state.textContent = any ? 'Passed' : 'No streams';
          ...
          out.innerHTML =
            ...
            + '<span class="row-sub">' + data.event.date + ' · ' + data.event.promotion + '</span></span></div>'
            ...
            + '</div>'
            + (any ? '' : '<div class="hint">Nothing came back. Check the service credentials on step 1 — the arrows at the top take you straight there.</div>');
        })
        .catch(function () {

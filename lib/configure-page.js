...
// ---------------------------------------------------------------- step 1
function stepServices(cfg, isAdmin) {
  ...
  const diyOn = isAdmin && cfg.diyUsenetEnabled === true;
  ...
  return ''
    ...
    + serviceCard({
      name: 'Built-in Usenet', role: 'Your own Newznab/Prowlarr indexer + NNTP provider',
      stateText: isAdmin ? (diyOn ? 'On' : 'Off') : 'Admin only', stateTone: diyOn ? 'ok' : 'off',
      body: !isAdmin ? '<div class="hint">Available to administrators only because this pipeline uses server resources.</div><input type="checkbox" disabled aria-label="Built-in Usenet is admin only">' : toggle('diyUsenetEnabled', diyOn, 'Enable the Built-in Usenet pipeline',
        'Its indexer and NNTP provider settings live on the Built-in Usenet page.')
        + '<div class="hint">' + (diyOn
          ? '<a href="/account/usenet">Open Built-in Usenet settings</a> to add or change your indexer and provider.'
          : 'Switch this on and save, then <a href="/account/usenet">Open Built-in Usenet settings</a> to add your indexer and provider.')
        + '</div>',
    })
    + '</div>'
...
function stepTeams(choosers, teamPromotions) {
  ...
  return ''
    ...
    + '<p>Choose the teams you follow. Their catalogs appear only in your account; other people make their own choices.</p></div>'
    ...
    + '<span class="row-sub" style="flex:1;min-width:220px">Selecting adds both Upcoming and Recent to your catalogs. Deselecting removes them from your manifest without deleting saved fixtures.</span>'
    + '</div></div>'
    + folds
...
function stepInstall(installUrl, eventCount) {
  ...
  return ''
    ...
    + require('./https-origin-warning').httpsOriginWarning('manifest-url', 'note', 'data-tone="warn"')
    + '<div class="grid-3">'

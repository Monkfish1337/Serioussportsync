...
function load() {
  ...
  // An `inbox` key from a version that wrote one is read here and dropped: it
  // is not in EMPTY(), so the next save writes the file without it. Nothing
  // migrates, because nothing ever consumed it.
  delete cache.inbox;
...
function setOverride(id, input) {
  ...
  if (patch.date && !Object.prototype.hasOwnProperty.call(patch, 'dateLocal')) {
    patch.dateLocal = patch.date;
  }
...
function compose(sourceEvents) {
  ...
  return combined
    ...
    .map((e) => {
      ...
      const event = Object.assign({}, e, patch);
      // Legacy date-only overrides must also change the date clients display.
      if (patch.date && !patch.dateLocal) event.dateLocal = patch.date;
      return event;
    })
    .sort((a, b) => String(b.date || '').localeCompare(String(a.date || '')));
}

// The review inbox was removed in 1.0.
//
// `recordInbox` wrote candidates the promotion filter had excluded, and
// candidates that looked like duplicates, to a list capped at 500. Nothing
// ever read that list: `updateInbox` had zero callers, no page rendered the
// items, and no export included them. It had been accumulating records nobody
// could see or act on since it was written, and the duplicate half cost an
// O(n) walk of every stored event for every candidate of every refresh.
//
// Deleted rather than surfaced, deliberately: a reviewer queue is only worth
// having if someone reviews it, and a filter that excludes an event is usually
// doing its job. `inbox` is still read out of any existing file below and then
// dropped, so an install upgrading from a version that wrote one does not carry
// the dead weight forward. An existing file keeps its inbox on disk until the
// next save of this store, which then writes it out.

module.exports = {
  load, save, filePath, compose, buildEvent, upsertManual, setOverride,
  removeOverride, setDisabled, deleteManual, lines,
};

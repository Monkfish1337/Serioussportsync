...
const eventAvailability = require('./event-availability');
const { catalogSelectedForUser } = require('./catalog-selection');
...
function handleCatalog({ type, id, extra = {} }, opts = {}) {
  if (type !== config.addonType) return { metas: [] };
  const entry = findCatalog(id);
  if (!entry) return { metas: [] };

  const { promotion, def } = entry;
  if (opts.user && !catalogSelectedForUser(opts.user.config || {}, promotion, def)) return { metas: [] };
  ...
  // Availability gate. The fixture feeds are a schedule; the catalogs are
  // meant to be a library. Without this, most entries are events nothing has
  // ever been found for, which reads as the addon being broken rather than the
  // content not existing. Curated events are exempt — the operator put those
  // there deliberately.
  const gate = opts.gate || settings.getCatalogGate();
  if (gate.enabled) {
    const available = (opts.available !== undefined)
      ? opts.available
      : eventAvailability.snapshot();
    // A null snapshot means the gate could not be evaluated; it fails open
    // rather than emptying every catalog.
    if (available) {
      const today = new Date().toISOString().slice(0, 10);
      pool = pool.filter((ev) => {
        if (ev.manual) return true;
        if (available.has(ev.id)) return true;
        return gate.keepUpcoming && ev.date && ev.date > today;
      });
    }
  }

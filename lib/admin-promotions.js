...
function renderBody(opts) {
  ...
  for (const p of all) {
    ...
    const clearCacheBtn = ''
      ...
      + ' onsubmit="return confirm(\'Clear saved source and availability results for &quot;' + escapeHtml(p.name) + '&quot;? The next stream request will search again. Other promotions are not affected.\');">'
      +   '<button type="submit" class="btn btn-sm btn-outline-warning" title="Clear only this promotion’s saved source and availability results">Clear source cache</button>'
      + '</form>';

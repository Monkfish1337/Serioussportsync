...
(function(){
  ...
  updateSourceMode(); updateSourceProvider();
    ...
    <div class="card mb-3 promotion-wizard" id="promotionWizard"${showWizard ? '' : ' hidden'}>
      ...
      <div class="card-body">
        <form method="POST" action="${escapeHtml(formAction)}" id="promotionWizardForm">
          ...
          <p class="text-secondary">Choose the easiest option you have. Test it here before continuing.</p>
          <div class="source-paths">
            <label class="source-path"><strong><input type="radio" name="sourceMode" value="existing"${editingSourceRef || isEdit ? ' checked' : ''}>Saved schedule</strong><span class="text-secondary small">Reuse a source already configured in SSS.</span></label>
            <label class="source-path"><strong><input type="radio" name="sourceMode" value="provider"${!editingSourceRef && !isEdit ? ' checked' : ''}>Provider ID</strong><span class="text-secondary small">Paste a SportsDB, TMDB, or football provider ID.</span></label>
            <label class="source-path"><strong><input type="radio" name="sourceMode" value="website">Official website</strong><span class="text-secondary small">Paste a recognised official schedule page.</span></label>
          </div>
          <div class="source-config" data-source-mode="existing">
            <label class="form-label" for="p-source-ref">Saved event schedule</label>
            <select class="form-select" id="p-source-ref" name="sourceRef">${registeredSourceOptions}</select>
            <div class="form-hint">Schedules remain reusable from <a href="/admin/metadata">Metadata</a>.</div>
          </div>
          <div class="source-config" data-source-mode="provider">
            <div class="row g-3"><div class="col-md-5"><label class="form-label" for="p-source-type">Provider</label><select class="form-select" id="p-source-type" name="sourceType"><option value="thesportsdb">TheSportsDB league</option><option value="mlb">MLB official schedule</option><option value="onefc">ONE official schedule</option><option value="football-data">football-data.org</option><option value="tmdb">TMDB TV series</option></select></div><div class="col-md-7"><label class="form-label" for="p-source-name">Schedule name <span class="text-secondary">optional</span></label><input class="form-control" id="p-source-name" name="sourceName" placeholder="Generated from the promotion name"></div></div>
            <input type="hidden" name="sourceId" id="p-source-id" value="">
            <div class="mt-3 source-provider-fields" data-source-provider="thesportsdb"><label class="form-label" for="leagueId-input">TheSportsDB league ID</label><input class="form-control" id="leagueId-input" name="leagueId" inputmode="numeric" placeholder="4443"><div class="form-hint">The number shown on the league page.</div></div>
            <div class="mt-3 source-provider-fields" data-source-provider="football-data"><div class="row g-2"><div class="col-md-6"><label class="form-label">Team ID</label><input class="form-control" name="teamId" inputmode="numeric" placeholder="66"></div><div class="col-md-6"><label class="form-label" for="competitionId-input">Competition ID or code</label><input class="form-control" id="competitionId-input" name="competitionId" placeholder="PL or 2021"></div></div><div class="form-hint">Enter either a team or competition.</div></div>
            <div class="mt-3 source-provider-fields" data-source-provider="tmdb"><label class="form-label" for="tvId-input">TMDB TV series ID</label><input class="form-control" id="tvId-input" name="tvIds" placeholder="224"><div class="form-hint">Multiple series can be comma-separated.</div></div>
            <div class="mt-3 source-provider-fields alert alert-success" data-source-provider="mlb">No ID or API key is needed. SSS uses MLB's official schedule.</div>
            <div class="mt-3 source-provider-fields alert alert-success" data-source-provider="onefc">No ID is needed. SSS uses ONE Championship's official schedule.</div>
          </div>
          <div class="source-config" data-source-mode="website">
            <label class="form-label" for="p-source-url">Official schedule website</label><input class="form-control" type="url" id="p-source-url" name="sourceUrl" placeholder="https://watch.onefc.com/upcoming-events">
            <div class="form-hint">SSS recognises official ONE, MLB, TheSportsDB league, and TMDB TV pages. Unknown websites need a purpose-built adapter; SSS will tell you before anything is saved.</div>
          </div>

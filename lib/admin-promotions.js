...
const {selectTorrentQueries} = require('./discovery-plan');
...
// 0.95.0 — how many events, and how recent.
//
// This table used to show a CATALOGS count — 2, 4, 6 — which is a property of
// the promotion's definition and never changes. It could not tell an operator
// the one thing they come here to find out.
//
// AEW sat in this list with zero upcoming events for weeks and nothing on the
// page said so; it was found because somebody happened to know that All Out
// exists. An empty promotion and a healthy one with a quiet week looked
// identical, and at 34 promotions "notice it by eye" is not a process.
function eventStats(events) {
  const today = new Date().toISOString().slice(0, 10);
  const stats = new Map();
  for (const event of events || []) {
    const id = event && event.promotion;
    if (!id) continue;
    const date = String(event.date || '');
    const entry = stats.get(id) || { total: 0, upcoming: 0, newest: '' };
    entry.total += 1;
    if (date >= today) entry.upcoming += 1;
    if (date > entry.newest) entry.newest = date;
    stats.set(id, entry);
  }
  return stats;
}

// Three states, because "0 upcoming" means something different depending on
// whether the promotion has any events at all.
function eventCell(entry) {
  if (!entry || !entry.total) {
    return '<span class="badge bg-red-lt">No events</span>'
      + '<br><small class="text-secondary">refresh has never returned anything</small>';
  }
  const newest = entry.newest
    ? '<br><small class="text-secondary">newest ' + escapeHtml(entry.newest) + '</small>' : '';
  if (!entry.upcoming) {
    return '<span class="badge bg-orange-lt">Nothing upcoming</span>'
      + '<br><small class="text-secondary">' + entry.total + ' stored, all in the past</small>'
      + newest;
  }
  return '<strong>' + entry.upcoming + '</strong> upcoming'
    + '<br><small class="text-secondary">' + entry.total + ' stored</small>' + newest;
}

function renderBody(opts) {
  opts = opts || {};
  const stats = eventStats(opts.events);
  const flash = opts.flash || null;
  const editId = opts.editId || null;
  const all = promotions.all;
  // Team catalogs are materialized once so a user's selection can be restored
  // without another provider request. They are managed from Configure → Your
  // Teams and are not independent promotions an administrator should tune,
  // refresh or delete here. Listing every roster made this page hundreds of
  // rows long and made healthy built-ins difficult to review.
  const managed = all.filter((promotion) => !promotion.autoTeam);
  const teamCatalogCount = all.length - managed.length;
  const startDates = settings.getPromotionMetadataStartDates();
  ...
  for (const p of managed) {
    ...
    const sourceDetail = source.type === 'thesportsdb' ? 'league ' + (source.leagueId || '?')
      ...
      : source.type === 'onefc' ? 'watch.onefc.com'
      : source.type === 'aew' ? 'allelitewrestling.com, no key'
      : source.type === 'espn' ? (source.league ? 'league ' + source.league : 'public scoreboard feed')
      : source.type === 'sport-video' ? 'releases, matched to no other promotion'
      : source.type === 'mlb' ? 'official schedule, no key'
      : source.type || 'unknown';
    ...
    // "Embedded source" was on 16 of 34 rows — every promotion whose source is
    // not in the metadata registry — over a sub-line like `espn` or
    // `competition PD`. The majority of the table therefore named its source
    // in a way nobody could act on, beside a minority that named it properly
    // ("TheSportsDB · UFC"). Same concept, two presentations, one screen.
    const ADAPTER_NAMES = {
      thesportsdb: 'TheSportsDB', 'football-data': 'football-data.org',
      'api-football': 'API-Football', uefa: 'Official UEFA', tmdb: 'TMDB',
      onefc: 'Official ONE', mlb: 'Official MLB', espn: 'ESPN', aew: 'Official AEW',
      'sport-video': 'Sport-Video', 'json-feed': 'Custom JSON feed',
    };
    const sourceName = currentDefinition
      ? currentDefinition.name
      : (ADAPTER_NAMES[source.type] || source.type || 'Unknown source');
    const sourceControl = '<strong>' + escapeHtml(sourceName) + '</strong>'
    ...
    const refreshBtn = ''
      ...
      + '</form>';
    const clearCacheBtn = ''
      + '<form method="POST" action="/admin/promotions/' + encodeURIComponent(p.id) + '/clear-cache" class="d-inline-block me-1"'
      + ' onsubmit="return confirm(\'Clear saved source results for &quot;' + escapeHtml(p.name) + '&quot;? The next stream request will search again. Other promotions are not affected.\');">'
      +   '<button type="submit" class="btn btn-sm btn-outline-warning" title="Clear only this promotion’s saved source results">Clear source cache</button>'
      + '</form>';
    ...
    const reviewBtn = '<a class="btn btn-sm btn-outline-secondary me-1" href="/admin/promotions/' + encodeURIComponent(p.id) + '/aliases">Review aliases</a>';
    const actions = reviewBtn + (p.isCustom
      ? ''
        + refreshBtn
        + clearCacheBtn
        + nuvioBtn
        ...
        + '</form>'
      : refreshBtn + clearCacheBtn + nuvioBtn
    ...
    const inheritedStart = p.defaultMetadataStartDate
      || process.env.EVENT_WINDOW_START_DATE || '2025-01-01';
    const effectiveStart = startDates[p.id] || inheritedStart;
    const startDateControl = '<details class="mt-1"><summary class="small link-primary">Metadata from '
      + escapeHtml(effectiveStart) + '</summary><form method="POST" action="/admin/promotions/'
      + encodeURIComponent(p.id) + '/metadata-start-date" class="mt-2">'
      + '<label class="form-label small mb-1" for="metadata-start-' + escapeHtml(p.id) + '">Metadata from</label>'
      + '<div class="d-flex gap-1"><input class="form-control form-control-sm" type="date"'
      + ' id="metadata-start-' + escapeHtml(p.id) + '" name="metadataStartDate"'
      + ' value="' + escapeHtml(startDates[p.id] || '') + '" aria-label="Metadata start date for '
      + escapeHtml(p.name) + '">'
      + '<button class="btn btn-sm btn-outline-primary" type="submit">Save</button></div>'
      + '<small class="text-secondary">Blank uses ' + escapeHtml(inheritedStart)
      + '. Saving removes older stored events; refresh to fetch earlier ones.</small></form></details>';
    rows += ''
      + '<tr>'
      +   selectCell
      +   '<td>' + provenance + '</td>'
      +   '<td><strong>' + escapeHtml(p.name) + '</strong><br><span class="text-secondary text-mono small">id=' + escapeHtml(p.id) + '</span>'
      +     '<br><span class="text-secondary small">' + escapeHtml(p.posterShape || 'landscape')
      +     ' \u00b7 ' + escapeHtml(String((p.catalogs || []).length)) + ' catalog'
      +     ((p.catalogs || []).length === 1 ? '' : 's') + '</span>' + startDateControl + '</td>'
      +   '<td>' + sourceControl + '</td>'
      +   '<td>' + eventCell(stats.get(p.id)) + '</td>'
      +   '<td class="promo-actions">' + actions + '</td>'
      + '</tr>';
  }
...
(function(){
  ...
  function renderEventDiff(target, result) {
    ...
    var c=result.counts||{};
    // A preview that skipped the slow fetch paths cannot speak about removals,
    // so it does not show a removed count at all. It used to: AEW read
    // "3 events · 29 removed" and listed cards a real refresh fetches without
    // trouble, which is what made a working promotion look destroyed.
    var head='Preview: '+c.after+' events seen · +'+c.added+' added · ~'+c.updated+' updated · ='+c.unchanged+' unchanged';
    if(!result.partial)head+=' · -'+c.removed+' removed';
    diffLine(target,head,'fw-semibold small');
    if(result.partial&&result.partialNote)diffLine(target,result.partialNote,'text-secondary small');
    var LABELS={added:'Added',updated:'Updated',removed:'Removed',unseen:'Not seen by this preview'};
    ['added','updated','removed','unseen'].forEach(function(kind){var rows=(result.samples&&result.samples[kind])||[];if(!rows.length)return;diffLine(target,LABELS[kind]+': '+rows.map(function(e){return (e.date||'?')+' '+e.name+(e.sourceId?' (#'+e.sourceId+')':'');}).join(' | '),'text-secondary small');});
  }
  ...
  if(researchBtn)researchBtn.addEventListener('click',async function(){
    ...
    researchBtn.disabled=true;researchBtn.textContent='Researching configured sources…';confirmedResearchTitles=[];if(researchOut)researchOut.textContent='Running broad and exact searches. Prowlarr uses its configured live search window.';
    try{
  ...
  const body = `
    ...
    <div class="alert alert-info mb-3"><strong>Team catalogs are managed in Configure → Your Teams.</strong>
      ${teamCatalogCount ? teamCatalogCount + ' materialized team catalogs stay prepared for quick account selection and are hidden from this promotion workspace.' : 'They appear here only after a team roster has been loaded.'}
    </div>
    ...
    <div class="card mb-4">
      ...
      <!-- align-top, not table-vcenter. Every row here is a primary line plus a
           secondary one — name over id, source over detail, upcoming over
           stored — and the cells are wildly different heights. Middle-aligning
           floated the one-line cells (the kind badge, the poster shape, the
           catalog count) to the vertical centre of a three-line row while the
           multi-line cells started at the top, so nothing shared a baseline.
           Adding the Events column made that worse by adding a third line. -->
      <div class="table-responsive">
        <table class="table card-table align-top">
          <thead><tr><th class="w-1"><input class="form-check-input m-0" type="checkbox" id="promotionSelectAll" aria-label="Select every enabled promotion"></th><th>Kind</th><th>Name</th><th>Metadata source</th><th>Events</th><th class="promo-actions-head"></th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </div>
    </div>
      ...
      <div class="card-body">
        <form method="POST" action="${escapeHtml(formAction)}" id="promotionWizardForm">
          ...
          <div class="row">
            ...
            <div class="col-md-4 mb-2">
              <label class="form-check">
                <input class="form-check-input" type="checkbox" name="disablePipelineUu" value="1"${(ed.disabledPipelines && (ed.disabledPipelines.indexOf('diy-usenet') !== -1 || ed.disabledPipelines.indexOf('uu') !== -1 || ed.disabledPipelines.indexOf('newsnab') !== -1)) ? ' checked' : ''}>
                <span class="form-check-label">Disable Built-in Usenet pipeline</span>
              </label>
              <small class="text-secondary d-block">Skips native indexer search and native NNTP playback.</small>
            </div>
            <div class="col-md-4 mb-2">
              <label class="form-check">
...
function renderMatchingLab(promotionId, opts) {
  ...
  const storedSaved = promotionOverrides.find(promotion.id) || {};
  const saved = promotion.weeklyShow && storedSaved.promotionId
    ? promotionOverrides.sanitizeWeekly(storedSaved) : storedSaved;
  const defaultRequireDate = promotion.weeklyShow === true || ['football-data', 'api-football', 'uefa']
  ...
  const events = store.getEvents().filter((event) => {
    ...
    const today = new Date().toISOString().slice(0, 10);
    const ad = String(a.dateLocal || a.date || ''), bd = String(b.dateLocal || b.date || '');
    const af = ad > today, bf = bd > today;
    return Number(af) - Number(bf) || (af
      ? ad.localeCompare(bd)
      : bd.localeCompare(ad));
  }).slice(0, 100);
  const eventOptions = events.map((event) => {
    const catalogDate = event.dateLocal || event.date || '';
    return '<option value="' + escapeHtml(event.id) + '" data-name="'
      + escapeHtml(event.name || '') + '" data-date="' + escapeHtml(catalogDate) + '">'
      + escapeHtml((catalogDate || 'No date') + ' · ' + (event.name || event.id)) + '</option>';
  }).join('');
  ...
  return `
    ...
    <div class="card mb-3"><div class="card-header"><h3 class="card-title">Matching Lab ${savedBadge}</h3></div><div class="card-body">
      <form method="POST" action="/admin/promotions/${encodeURIComponent(promotion.id)}/matching-override" id="matchingLabForm">
        ...
        <div class="mt-3"><label class="form-label" for="lab-query">Optional research search terms</label><input class="form-control" id="lab-query" placeholder="e.g. WWE Friday Night SmackDown 2026 09 18"><div class="form-hint">Use this to check a different title or a nearby air date. Separate multiple searches with semicolons. The event date still decides what counts as a match.</div></div>
        <label class="form-check mt-3"><input class="form-check-input" type="checkbox" id="lab-live-plan" checked><span class="form-check-label">Use saved promotion rules and live search queries</span></label>
        <div class="form-hint">Untick to explore the draft rules below. ${require('./prowlarr-discovery').usesQueue(promotion) ? 'Prowlarr suggestions come from saved releases. <a href="/admin/prowlarr-discovery">Queue a missing past game or review discovery progress</a>.' : ''}</div>
        ...
        <div class="form-hint mt-2">Confirm releases, then save. If the rule fields are empty, Save creates rules from your confirmed titles. Use Create rules to preview them first.</div><hr class="my-4"><div class="row g-3"><div class="col-md-6"><label class="form-label">Confirmed release titles</label><textarea class="form-control text-mono" rows="6" id="lab-good" name="goodExamples"></textarea></div><div class="col-md-6"><label class="form-label">Confirmed unrelated titles</label><textarea class="form-control text-mono" rows="6" id="lab-bad" name="badExamples"></textarea></div></div>
    ...
    <script>(function(){
      ...
      function researchBody(){var body=new URLSearchParams();if(document.getElementById('lab-live-plan').checked)body.append('promotionId',${JSON.stringify(promotion.id)});body.append('eventId',select?select.value:'');body.append('name',document.getElementById('lab-name').value);body.append('eventName',name.value);body.append('eventDate',date.value);body.append('query',document.getElementById('lab-query').value);body.append('promotionAliases',document.getElementById('lab-aliases').value);body.append('relevanceKeywords',document.getElementById('lab-keywords').value);body.append('searchTitleTemplates',document.getElementById('lab-templates').value);body.append('exclusionKeywords',document.getElementById('lab-exclusions').value);if(document.getElementById('lab-date').checked)body.append('requireDateInTitle','1');if(document.getElementById('lab-foreign').checked)body.append('allowForeignLanguage','1');return body;}
...
function saveMatchingOverride(promotionId, body) {
  ...
  const hasRules = ['promotionAliases', 'relevanceKeywords', 'searchTitleTemplates']
    .some(key => Array.isArray(input[key]) ? input[key].some(value => String(value || '').trim()) : String(input[key] || '').trim());
  const confirmed = String(input.goodExamples || '').split(/\r?\n/).map(title => title.trim()).filter(Boolean);
  if (!hasRules && confirmed.length) {
    const unrelated = String(input.badExamples || '').split(/\r?\n/).map(title => title.trim()).filter(Boolean);
    const generated = promotionAliases.suggestPromotionSetup(promotion.name, confirmed, unrelated);
    input.promotionAliases = generated.aliases;
    input.relevanceKeywords = generated.keywords;
    input.searchTitleTemplates = generated.searchTitleTemplates;
    input.exclusionKeywords = [].concat(generated.exclusions || [],
      String(input.exclusionKeywords || '').split(/[\r\n,]+/).map(word => word.trim()).filter(Boolean));
  }
  const saved = promotionOverrides.set(promotion.id, promotion.name,
    promotion.weeklyShow ? promotionOverrides.sanitizeWeekly(input) : input);
  promotions.reload();
  return saved;
}
...
function persistFromForm(body, opts) {
  ...
  const spec = {
    ...
    disabledPipelines: [
      ...
      body.disablePipelineUu === '1'       ? 'diy-usenet' : null,
...
function researchDraft(body) {
  ...
  const livePromotion = promotions.all.find(item => item.id === String(body.promotionId || ''));
  const effectivePromotion = livePromotion || promotion;
  if (livePromotion) {
    const stored = store.getEvent(String(body.eventId || ''));
    if (stored && stored.name === event.name && stored.date === event.date) Object.assign(event, stored);
  }
  ...
  const generated = effectivePromotion.searchTitles(event);
  ...
  let queries = Array.from(new Set(manual.concat(broad, generated)
    .map((value) => String(value || '').replace(/\s+/g, ' ').trim()).filter(Boolean)))
    .slice(0, 6);
  if (livePromotion && !manual.length) {
    queries = require('./streams')._test.selectProviderQueries(generated, event, effectivePromotion.uuMaxQueries || 6, effectivePromotion);
  }
  const torrentQueries = livePromotion && !manual.length
    ? (effectivePromotion.torrentSearchTitles ? effectivePromotion.torrentSearchTitles(event) : generated) : queries;
  return { ok: true, name, promotion: effectivePromotion, event, queries, torrentQueries };
}
...
function researchTitleClass(title, draft) {
  const noiseReason = require('./sources/release-filter').rejectionReason(title, null,
    { allowForeignLanguage: draft.promotion.allowForeignLanguage });
  if (noiseReason) return { group: 'rejected', reason: noiseReason };
...
async function researchAliases(userConfig, body, opts) {
  ...
  const bitmagnetConfig = settings.getBitmagnet();
  const prowlarrConfig = settings.getProwlarr();
  if (bitmagnetConfig.url && bitmagnetConfig.enabled !== false) {
    tasks.push({id: 'bitmagnet', name: 'Bitmagnet', queries: draft.torrentQueries, run: () =>
      (opts.bitmagnetSearch || bitmagnet.multiSearch)(draft.torrentQueries,
        {queryReview: require('./query-review').context('bitmagnet', draft.promotion, draft.event, 'research'), config: bitmagnetConfig, detailed: true, deadlineMs: timing.discoveryBudgetMs})});
  }
  // Explicit admin research must remain available independently of playback
  // live search and measured-queue selection. Fetch titles, not torrent files.
  if (prowlarrConfig.url && prowlarrConfig.apiKey && prowlarrConfig.enabled !== false) {
    const queryReview = require('./query-review').context('prowlarr', draft.promotion, draft.event, 'research');
    const planned = selectTorrentQueries(draft.torrentQueries, draft.event, draft.promotion, 60);
    const selected = (queryReview ? queryReview.filter(planned) : planned).slice(0, timing.prowlarrMaxQueries);
    tasks.push({id: 'prowlarr', name: 'Prowlarr torrents', queries: selected, run: () =>
      (opts.prowlarrSearch || prowlarr.multiSearch)(selected,
        {queryReview, detailed: true, titlesOnly: true, deadlineMs: timing.prowlarrLiveBudgetMs, timeoutMs: timing.prowlarrLiveBudgetMs})});
  } else if (require('./prowlarr-discovery').usesQueue(draft.promotion)) {
    tasks.push({id:'prowlarr',name:'Prowlarr saved releases',queries:[],run:async()=>({ok:true,results:require('./prowlarr-discovery').getDefault().researchCandidates(draft.event)})});
  }
  ...
  if (usenetIndexer.isConfigured(nativeConfig)) {
    tasks.push({ id: 'native-indexer', name: nativeConfig.name || 'DIY indexer', run: () =>
      (opts.nativeSearch || usenetIndexer.search)(draft.queries, nativeConfig,
        {queryReview: require('./query-review').context('native-indexer', draft.promotion, draft.event, 'research'), maxQueries: 6, timeoutMs: 30000 }) });
  }
  ...
  if (easynewsConfig && easynewsConfig.username) {
    tasks.push({ id: 'easynews', name: 'Easynews', run: () =>
      (opts.easynewsSearch || easynews.multiSearch)(draft.queries, Object.assign({
        queryReview: require('./query-review').context('easynews', draft.promotion, draft.event, 'research'), maxQueries: timing.easynewsMaxQueries, timeoutMs: timing.easynewsQueryTimeoutMs,
        totalTimeoutMs: Math.max(1000, timing.pipelineBudgetMs - 1500), queryDelayMs: 150,
      }, easynewsConfig)) });
  }
  const companionConfig = opts.companionConfig || settings.getCompanion();
  if (companionConfig && companionConfig.url && prowlarrConfig.liveSearchEnabled !== false && !require('./prowlarr-discovery').usesQueue(draft.promotion)) {
  ...
  if (!tasks.length) return { ok: false, error: 'Configure SSS Companion, DIY Discover, or Easynews first' };

  const settled = await Promise.allSettled(tasks.map(async task => {
    const started = Date.now();
    try { return await task.run(); }
    finally {task.durationMs = Date.now() - started;}
  }));
  const providers = [];
  ...
  settled.forEach((outcome, index) => {
    ...
    providers.push({
      ...
      queries: task.queries || draft.queries,
  ...
  for (const provider of providers) reportLines.push('- ' + provider.name + ': '
    + (provider.ok ? provider.count + ' discovered' + (provider.partial ? ' (partial)' : '') : provider.error));
  reportLines.push('', 'Queries by source:');
  for (const provider of providers) {
    reportLines.push(provider.name + ' (' + provider.durationMs + 'ms):');
    for (const query of provider.queries) reportLines.push('- ' + query);
  }

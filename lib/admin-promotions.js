...
(function(){
  ...
  function researchRow(parent,item,group){
    ...
    var action=document.createElement('button');action.type='button';action.className='btn btn-sm '+(group==='rejected'?'btn-outline-danger':'btn-outline-success');action.textContent=group==='rejected'?'Use as rejected':'Use as example';
    action.addEventListener('click',function(){if(group==='rejected')addRejectedTitle(item.title);else addReleaseTitle(item.title);action.disabled=true;action.textContent='Added';});
  ...
  if(researchBtn)researchBtn.addEventListener('click',async function(){
    ...
    researchBtn.disabled=true;researchBtn.textContent='Researching configured sources…';if(researchOut)researchOut.textContent='This can take up to 20 seconds while the configured services respond.';
    try{
          ...
          <div class="card bg-dark-lt mb-3">
            <div class="card-body">
              ...
              <div class="card border-info mb-3"><div class="card-body">
                <div class="d-flex flex-wrap align-items-start justify-content-between gap-3 mb-3"><div><h4 class="mb-1">Alias Research</h4><div class="text-secondary small">SSS searches the services already configured on your account, tests every discovered title against this promotion, and explains what matched or was rejected. Credentials and download links never leave the server.</div></div><span class="badge bg-blue-lt">admin only</span></div>
...
function researchDraft(body) {
  ...
  const queries = Array.from(new Set(manual.concat(generated,
    dotted ? [name + ' ' + dotted + ' ' + event.name, event.name + ' ' + dotted] : [])))
  ...
  if (easynewsConfig && easynewsConfig.username) {
    tasks.push({ id: 'easynews', name: 'Easynews', run: () =>
      (opts.easynewsSearch || easynews.multiSearch)(draft.queries, Object.assign({
        maxQueries: 3, timeoutMs: 5000, totalTimeoutMs: 20000, queryDelayMs: 100,
      }, easynewsConfig)) });
  }
  if (!tasks.length) return { ok: false, error: 'Configure DIY Discover, Usenet Ultimate, or Easynews under Account first' };

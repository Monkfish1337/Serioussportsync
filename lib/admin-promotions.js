...
function renderBody(opts) {
  ...
  for (const p of all) {
    ...
    const sourceControl = '<form method="POST" action="/admin/promotions/' + encodeURIComponent(p.id) + '/source" class="d-flex gap-1">'
      + '<select class="form-select form-select-sm" name="sourceRef" aria-label="Metadata source for ' + escapeHtml(p.name) + '">' + sourceOptions + '</select>'
      + '<button class="btn btn-sm btn-outline-primary" type="submit">Assign</button></form>'
      + '<small class="text-secondary">' + escapeHtml(sourceDetail) + '</small>';
  ...
  let registeredSourceOptions = '<option value="">Configure a source below</option>';
  for (const definition of sourceDefinitions) {
  ...
  for (const definition of sourceDefinitions) {
    ...
    const details = s.type === 'thesportsdb' ? 'league ' + s.leagueId
      : s.type === 'football-data' ? (s.teamId ? 'team ' + s.teamId : 'competition ' + s.competitionId)
      : s.type === 'tmdb' ? 'TV ' + (s.tvIds || [s.tvId]).filter(Boolean).join(', ')
      : s.type === 'onefc' ? 'watch.onefc.com' : '';
    const usedBy = all.filter((p) => p.sourceRef === definition.id).map((p) => p.name).join(', ');
    sourceRows += '<tr><td><strong>' + escapeHtml(definition.name) + '</strong><br><span class="text-mono text-secondary small">' + escapeHtml(definition.id) + '</span></td>'
      + '<td>' + escapeHtml(s.type) + '<br><span class="text-secondary small">' + escapeHtml(details) + '</span></td>'
      + '<td>' + escapeHtml(usedBy || 'Unassigned') + '</td><td>' + (definition.system ? '<span class="badge bg-blue-lt">system</span>' : '<span class="badge bg-green-lt">custom</span>') + '</td></tr>';
  }
...
(function(){
  ...
  if (aliasBtn) aliasBtn.addEventListener('click', async function () {
    ...
    try {
      ...
      setVerdict('ok', 'Suggested ' + (j.aliases || []).length + ' aliases and ' + (j.exclusions || []).length + ' exclusions. Review them before saving.');
    } catch (e) { setVerdict('err', e.message); }
    finally { aliasBtn.disabled = false; aliasBtn.textContent = 'Suggest aliases'; }
  });
  ...
  if (previewBtn) previewBtn.addEventListener('click', async function () {
    ...
    try {
      ...
      if (previewOut) {
        ...
        addPreviewLine(previewOut, 'Queries (' + (j.queries || []).length + '): ' + (j.queries || []).join('  |  '), 'small text-secondary mb-2');
        (j.examples || []).forEach(function(item) {
          var correct = (item.expected === 'accept') === item.accepted;
          var icon = correct ? '✓' : '✗';
          addPreviewLine(previewOut, icon + ' expected ' + item.expected + ' · ' + (item.accepted ? 'accepted' : 'rejected') + ' (' + item.reason + ') · ' + item.title,
  ...
  const body = `
    ...
    <details class="card mb-4">
      <summary class="card-header cursor-pointer"><h3 class="card-title d-inline">Metadata sources</h3><span class="text-secondary ms-2">${sourceDefinitions.length} registered</span></summary>
      <div class="table-responsive"><table class="table table-vcenter card-table"><thead><tr><th>Name</th><th>Adapter</th><th>Used by</th><th>Kind</th></tr></thead><tbody>${sourceRows}</tbody></table></div>
      <div class="card-body border-top">
        <h4>Add a reusable source</h4>
        <form method="POST" action="/admin/metadata-sources/create">
          <div class="row g-2">
            <div class="col-md-2"><label class="form-label">Source ID</label><input class="form-control" required pattern="[a-z0-9_-]{2,50}" name="id" placeholder="tsdb-nfl"></div>
            <div class="col-md-3"><label class="form-label">Name</label><input class="form-control" required maxlength="80" name="name" placeholder="TheSportsDB · NFL"></div>
            <div class="col-md-2"><label class="form-label">Adapter</label><select class="form-select" name="type"><option value="thesportsdb">TheSportsDB</option><option value="football-data">football-data.org</option><option value="tmdb">TMDB</option><option value="onefc">Official ONE</option></select></div>
            <div class="col-md-2"><label class="form-label">League ID</label><input class="form-control" name="leagueId" placeholder="4391"></div>
            <div class="col-md-3"><label class="form-label">TMDB TV IDs</label><input class="form-control" name="tvIds" placeholder="224, 3231"></div>
            <div class="col-md-3"><label class="form-label">Football team ID</label><input class="form-control" name="teamId" placeholder="66"></div>
            <div class="col-md-3"><label class="form-label">Football competition</label><input class="form-control" name="competitionId" placeholder="PL or 2021"></div>
          </div>
          <small class="text-secondary d-block mt-2">Fill only the identifier used by the selected adapter. Credentials remain in encrypted global settings.</small>
          <button class="btn btn-primary mt-2" type="submit">Add source</button>
        </form>
      </div>
    </details>
    ...
    <div class="card mb-3">
      ...
      <div class="card-body">
        <form method="POST" action="${escapeHtml(formAction)}">
          ...
          <div class="mb-3 mt-3">
            <label class="form-label" for="p-source-ref">Use an existing metadata source</label>
            <select class="form-select" id="p-source-ref" name="sourceRef">${registeredSourceOptions}</select>
            <small class="text-secondary">Choose a registered source, or configure an inline source below. Sources can be reassigned later without rebuilding the promotion.</small>
          </div>

          <details class="mb-3" id="inline-source-config"${editingSourceRef ? '' : ' open'}>
          <summary class="text-secondary mb-2">Inline source configuration</summary>
          <div class="mb-3 mt-3">
            ...
            <select class="form-select" id="p-source" name="source">
              ...
              <option value="onefc"${ed.source === 'onefc' ? ' selected' : ''}>Official ONE Championship feed</option>
            </select>
          ...
          <summary class="card-header cursor-pointer"><h4 class="card-title d-inline">Artwork and search templates</h4><span class="text-secondary ms-2">advanced</span></summary>
          <div class="card-body">
          <div class="row g-3">
            <div class="col-md-6">
          ...
          <div class="card bg-dark-lt mb-3">
            <div class="card-body">
              <h4 class="mb-1">Teach SSS from example releases <span class="badge bg-green-lt ms-2">recommended</span></h4>
              <p class="text-secondary small">Paste a few releases that should match this promotion. SSS removes dates, event numbers and quality/codec noise, then suggests reusable aliases. Nothing is saved until you submit this form.</p>
              <label class="form-label" for="p-release-examples">Known-good release titles (one per line)</label>
              <textarea class="form-control text-mono mb-2" id="p-release-examples" rows="4" placeholder="Formula.1.2026.Dutch.Grand.Prix.1080p.WEB-DL.H264&#10;F1.2026.Dutch.GP.Qualifying.2160p.UHD"></textarea>
              <label class="form-label" for="p-bad-examples">Known-bad release titles (one per line, optional)</label>
              <textarea class="form-control text-mono mb-2" id="p-bad-examples" rows="3" placeholder="Formula.2.2026.Dutch.Grand.Prix.1080p&#10;F1.Academy.2026.Round.4.720p"></textarea>
              <button class="btn btn-outline-primary mb-3" type="button" id="deriveAliases">Suggest aliases</button>
              <label class="form-label" for="p-promotion-aliases">Promotion aliases (one per line)</label>
              <textarea class="form-control text-mono mb-2" id="p-promotion-aliases" name="promotionAliases" rows="3" placeholder="Formula 1&#10;F1&#10;Formula1">${escapeHtml(promotionAliasLines)}</textarea>
              <label class="form-label" for="p-exclusions">Exclusion terms (comma-separated)</label>
              <input class="form-control text-mono mb-2" id="p-exclusions" name="exclusionKeywords" value="${escapeHtml(exclusionStr)}" placeholder="formula 2, academy">
              <small class="text-secondary d-block mb-3">Aliases become search and recognition terms. Exclusions always reject a matching release; remove any suggestion that is too broad.</small>
              <div class="row g-2">
                <div class="col-md-8"><label class="form-label" for="p-preview-event">Example metadata event name</label><input class="form-control" id="p-preview-event" placeholder="Formula 1 Dutch Grand Prix"></div>
              ...
              <button class="btn btn-outline-info mt-2" type="button" id="previewMatching">Preview matching</button>
              <div class="mt-3" id="matchingPreview"></div>
            </div>
          </div>

          <div class="mb-3">
            <label class="form-label" for="p-keywords">Relevance keywords (comma-separated)</label>
            <input class="form-control text-mono" id="p-keywords" type="text" name="relevanceKeywords" value="${escapeHtml(keywordsStr)}" placeholder="Generated from the promotion name if left blank">
            <small class="text-secondary">Optional advanced override. When blank, SSS derives recognition terms from the display name and promotion aliases.</small>
          </div>
...
function saveFromForm(body, opts) {
  ...
  let source = String(body.source || 'tsdb').trim();
  if (selectedSource) {
  ...
  const spec = {
    ...
    exclusionKeywords: String(body.exclusionKeywords || '')
  ...
  if (source === 'tsdb') {
    spec.leagueId = String((selectedSource && selectedSource.leagueId) || body.leagueId || '').trim();
  } else if (source === 'football-data') {
    if (selectedSource && selectedSource.teamId) spec.teamId = String(selectedSource.teamId);
    else spec.competitionId = String((selectedSource && selectedSource.competitionId) || body.competitionId || '').trim();
  } else if (source === 'tmdb') {
    ...
    else spec.tvId = String((ids && ids[0]) || body.tvId || '').trim();
...
function deriveAliases(name, examples, badExamples) {
  ...
  return { ok: true, aliases: result.aliases, keywords: result.keywords, exclusions: result.exclusions };
}
...
function previewMatching(body) {
  ...
  return { ok: true, event, queries: preview.searchTitles(event), examples };
}

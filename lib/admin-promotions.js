...
function renderMatchingLab(promotionId, opts) {
  ...
  const events = store.getEvents().filter((event) => {
    ...
    const today = new Date().toISOString().slice(0, 10);
    const af = String(a.date || '') > today, bf = String(b.date || '') > today;
    return Number(af) - Number(bf) || (af
      ? String(a.date || '').localeCompare(String(b.date || ''))
      : String(b.date || '').localeCompare(String(a.date || '')));
  }).slice(0, 100);
  const eventOptions = events.map((event) => '<option value="' + escapeHtml(event.id) + '" data-name="'
  ...
  return `
    ...
    <script>(function(){
      ...
      function researchBody(){var body=new URLSearchParams();if(document.getElementById('lab-live-plan').checked)body.append('promotionId',${JSON.stringify(promotion.id)});body.append('eventId',select?select.value:'');body.append('name',document.getElementById('lab-name').value);body.append('eventName',name.value);body.append('eventDate',date.value);body.append('promotionAliases',document.getElementById('lab-aliases').value);body.append('relevanceKeywords',document.getElementById('lab-keywords').value);body.append('searchTitleTemplates',document.getElementById('lab-templates').value);body.append('exclusionKeywords',document.getElementById('lab-exclusions').value);if(document.getElementById('lab-date').checked)body.append('requireDateInTitle','1');if(document.getElementById('lab-foreign').checked)body.append('allowForeignLanguage','1');return body;}
...
function saveMatchingOverride(promotionId, body) {
  ...
  const saved = promotionOverrides.set(promotion.id, promotion.name, input);
  promotions.reload();
  return saved;
}

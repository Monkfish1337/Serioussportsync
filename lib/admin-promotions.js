...
(function(){
  ...
  if (aliasBtn) aliasBtn.addEventListener('click', async function () {
    ...
    try {
      ...
      setVerdict('ok', 'Suggested ' + (j.aliases || []).length + ' aliases. Review them below before saving.');
        ...
        <form method="POST" action="${escapeHtml(formAction)}">
          ...
          <div class="card bg-dark-lt mb-3">
            <div class="card-body">
              ...
              <textarea class="form-control text-mono" id="p-promotion-aliases" name="promotionAliases" rows="3" placeholder="Formula 1&#10;F1&#10;Formula1">${escapeHtml(promotionAliasLines)}</textarea>
              <small class="text-secondary">These aliases become both search variants and recognition terms. Remove any suggestion that could identify unrelated content.</small>
            </div>
          </div>
...
function deriveAliases(name, examples) {
  const cleanName = String(name || '').trim();
  if (!cleanName) return { ok: false, error: 'Promotion name is required' };
  const result = promotionAliases.suggestPromotionSetup(cleanName, examples);
  return { ok: true, aliases: result.aliases, keywords: result.keywords };
}

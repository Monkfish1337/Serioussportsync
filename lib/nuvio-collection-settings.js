...
const DEFAULT_FOLDERS = Object.freeze([
  ...
  { id: '0bf2a789-b07e-43d3-9c15-b9aac44c9e63', title: 'Football', promotions: ['motd', 'epl', 'ucl'], artwork: '/assets/collection-football.png', tileShape: 'landscape', hideTitle: false },
  ...
  // The American big three had no folder at all, so three of the busiest
  // promotions sat loose among the ungrouped catalogs.
  { id: '5a1c9d27-7f3e-4c60-9b18-2d4e6f8a1b33', title: 'Big 3', promotions: ['nfl', 'nba', 'mlb'], artwork: '/assets/collection-big-3.png', tileShape: 'landscape', hideTitle: false },
  // The `discovered-*` catalogs exist for events pulled out of release
  // listings that matched no promotion. Loose on the home screen they read as
  // seven more leagues with full schedules, which is the opposite of what they
  // are. Gathered here they read as what they are: the unmatched pile.
  { id: 'c7e04b91-6d52-4a38-8f17-93b5c2e8a740', title: 'Unmatched', promotions: ['discovered-football', 'discovered-basketball', 'discovered-baseball', 'discovered-americanfootball', 'discovered-hockey', 'discovered-rugby', 'discovered-other'], artwork: '/assets/collection-unmatched.png', tileShape: 'landscape', hideTitle: false },
]);
...
function normalize(data) {
  ...
  let rawFolders = data && Array.isArray(data.folders) ? data.folders : fallback.folders;

  // Folders added in a later version reach an existing install once.
  //
  // A saved file replaces the defaults outright — that is what makes the
  // editor work — so a new default folder would never appear for anyone who
  // had already saved, which is everyone. Adding by id when the stored version
  // is behind fixes that, and stamping the version afterwards means a folder
  // the operator then deletes stays deleted rather than returning on every
  // load. Folders they already have are untouched, matched on id.
  const storedVersion = Number(data && data.version) || 0;
  if (data && Array.isArray(data.folders) && storedVersion < VERSION) {
    const have = new Set(rawFolders.map((folder) => String(folder && folder.id || '')));
    const added = fallback.folders.filter((folder) => !have.has(folder.id));
    if (added.length) {
      rawFolders = rawFolders.concat(clone(added));
      console.log('[nuvio-collections] added ' + added.length + ' new default folder(s): '
        + added.map((folder) => folder.title).join(', '));
    }
  }
...
module.exports = {
  ...
  VERSION,
  defaults,
  // Exported so the version-gated folder migration can be tested without
  // writing a file and reloading it.
  normalize,
  load,
  save,
  cleanImage,

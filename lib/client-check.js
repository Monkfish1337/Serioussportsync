...
function judgeStreams(body, ms, expected, pageProtocol) {
  ...
  // Reported once for the whole run (see run), not on every event, where it
  // buried everything else on an install without PUBLIC_URL.
  const httpLinks = pageProtocol === 'https:' && urls.some((url) => /^http:\/\//i.test(url)
    && !/^http:\/\/(?:127\.|10\.|192\.168\.|localhost)/i.test(url));
  ...
  return { verdict, rows: rows.length, problems, warnings, pipelines, httpLinks,
    pipelineRows: (body && body.pipelineRows) || null,
    requestId: body && body.diagnostics && body.diagnostics.requestId || null };
}

// `opts.expectEmpty`: a catalog that is empty by design (a Discovered
// catalog's Upcoming row — its events are built from releases that exist).
// `opts.publicUrlSet`: bundled artwork is only addressable with PUBLIC_URL.
function judgeCatalog(body, opts) {
  const o = opts || {};
  ...
  const warnings = [];
  if (!metas.length && o.expectEmpty) {
    return { verdict: 'info', items: 0, problems, warnings: ['empty by design: built from existing releases'] };
  }
  if (!metas.length) warnings.push('empty');
  if (noPoster) warnings.push(noPoster + ' item(s) without a poster'
    + (o.publicUrlSet === false ? ' (bundled artwork needs PUBLIC_URL)' : ''));
...
async function run(opts) {
  ...
  const releaseDerived = new Set((o.promotions || []).filter((p) => p && p.source && p.source.type === 'sport-video'
    && String(p.id).startsWith('discovered-')).flatMap((p) => (p.catalogs || []).map((c) => c.id)));
  let done = 0;
  report.catalogs = await mapLimit(catalogs, 4, async (catalog) => {
    ...
    return Object.assign({ id: catalog.id, name: catalog.name || catalog.id, type: catalog.type, ms: result.ms },
      result.ok ? judgeCatalog(result.body, {
        expectEmpty: releaseDerived.has(catalog.id) && /-upcoming$/.test(catalog.id),
        publicUrlSet: o.publicUrlSet,
      }) : { verdict: 'fail', items: 0, problems: [result.error], warnings: [] });
  });
  ...
  if (report.events.some((e) => e.httpLinks) && !report.manifest.warnings.some((w) => /PUBLIC_URL/.test(w))) {
    report.manifest.warnings.push('playback links are http:// while this page is https — set PUBLIC_URL');
    if (report.manifest.verdict === 'pass') report.manifest.verdict = 'warn';
  }

...
const defaultAdapters = {
  ...
  apiFootball: require('./sources/api-football'),
  uefa: require('./sources/uefa'),
  tmdb: require('./sources/tmdb'),
  jsonFeed: require('./sources/json-feed'),
};
...
function normalize(raw, type, promotion) {
  ...
  if (type === 'onefc' || type === 'mlb' || type === 'json-feed') return transform.fromWiki(raw, promotion);
  return null;
}
...
async function fetchRaw(definition, opts) {
  ...
  if (source.type === 'api-football') {
    const apiKey = (opts.credentials && opts.credentials.apiFootballApiKey)
      || settings.getApiFootball().apiKey;
    if (!apiKey) throw new Error('API-Football API key is not configured in Admin');

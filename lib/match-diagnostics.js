...
// Every event is replayed against every release within a day of it, so the
// overwhelming majority of rows are one sport's fixture being correctly
// rejected against another sport's release. Read raw, that looks like
// catastrophic failure — 14,758 rejections against 394 matches — when almost
// all of it is the matcher working. This scores how much of the event's own
// name appears in the release title, so a genuine near miss (the release really
// is about this fixture, and something in the rules still rejected it) can be
// sorted to the top instead of being buried under thousands of correct noes.
const OVERLAP_STOPWORDS = new Set([
  'vs', 'at', 'the', 'and', 'fc', 'cf', 'sc', 'afc', 'club', 'city', 'united',
  'live', 'stream', 'free', 'download', 'torrent', 'video', 'hd', 'full',
]);
...
function overlapTokens(value) {
  ...
  for (const token of text.match(/[a-z0-9]+/g) || []) {
    if (token.length > 2 && !OVERLAP_STOPWORDS.has(token) && !/^\d+$/.test(token)) out.add(token);
  }
  return out;
}

// Share of the event's distinctive words that appear in the release title,
// 0 to 1. A near miss is a rejected row scoring high.
function nameOverlap(eventName, releaseTitle) {
  const wanted = overlapTokens(eventName);
  if (!wanted.size) return 0;
  const found = overlapTokens(releaseTitle);
  let hits = 0;
...
function nearbyReleases(buckets, date) {
  ...
  const out = [];
  // The event's own day first: on a busy weekend the day before alone can
  // fill the cap, and the release for this very fixture would never be read.
...
// Participants are read from the fixture name because only the UEFA and
// API-Football transforms populate structured teamNames — MLB and the
// team-scoped football-data promotions do not, and those are exactly the ones
// worth narrowing. Every promotion that fields two sides names them the same
// way, so one splitter covers all of them.
...
// The selectable teams for the admin picker, derived from the catalog itself
// rather than a hardcoded club list, so it stays correct as competitions come
// and go. Ordered by how many fixtures each side appears in: a league's clubs
// rise to the top, a one-off opponent sinks.
function catalogTeams(options) {
  const events = eventsInScope(Object.assign({ days: 120 }, options || {}));
  const byPromotion = new Map();
  for (const event of events) {
    const promotion = promotions.getByEventId(event.id);
    if (!promotion) continue;
    if (!byPromotion.has(promotion.id)) {
      byPromotion.set(promotion.id, {
        promotion: promotion.id, promotionName: promotion.name, counts: new Map(),
      });
    }
    const row = byPromotion.get(promotion.id);
    for (const name of participantsOf(event)) {
      row.counts.set(name, (row.counts.get(name) || 0) + 1);
    }
  }
  return Array.from(byPromotion.values())
    .map((row) => ({
      promotion: row.promotion,
      promotionName: row.promotionName,
      teams: Array.from(row.counts.entries())
        .map(([name, fixtures]) => ({ name, fixtures }))
        .sort((a, b) => b.fixtures - a.fixtures || a.name.localeCompare(b.name)),
    }))
    .filter((row) => row.teams.length > 0)
    .sort((a, b) => a.promotionName.localeCompare(b.promotionName));
}
...
function diagnose(options) {
  ...
  const summary = {
    ...
    eventsAllRejected: 0,
    rejectionsByReason: {},
    // A rejection whose release really does look like this fixture. These are
    // the only rejections worth reading; the rest are other sports.
    nearMisses: 0,
    nearMissesByReason: {},
  };
  ...
  for (const event of events) {
    ...
    for (const record of candidates) {
      ...
      const overlap = nameOverlap(event.name, record.title || record.indexTitle || '');
      const nearMiss = outcome.decision !== 'matched' && overlap >= NEAR_MISS_THRESHOLD;
      if (outcome.decision === 'matched') matched += 1;
        ...
        if (nearMiss) {
          ...
          const nearKey = outcome.stage + ':' + outcome.reason;
          summary.nearMissesByReason[nearKey] = (summary.nearMissesByReason[nearKey] || 0) + 1;
        }
      }
      rows.push(Object.assign({}, base, {
        nameOverlap: overlap,
...
const CSV_COLUMNS = Object.freeze([
  ...
  'release_url', 'decision', 'stage', 'reason',
  // Sort on these two first. Almost every rejection is a different sport
  // entirely; near_miss=yes is the short list actually worth reading.
  'name_overlap', 'near_miss',
  'card_title_reason',
...
function toCsv(report) {
  ...
  for (const row of (report.rows || [])) {
    ...
    lines.push([
      ...
      row.nameOverlap === undefined ? '' : row.nameOverlap,
      row.nearMiss ? 'yes' : 'no',
...
module.exports = { diagnose, toCsv, explain, catalogTeams, participantsOf, CSV_COLUMNS };

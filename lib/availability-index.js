...
function createAvailabilityIndex(options) {
  ...
  `);
  // v2 adds the user-facing discovery funnel. PRAGMA inspection also makes
  // this safe for early development databases that did not record a version.
  const searchRunColumns = new Set(db.prepare('PRAGMA table_info(search_runs)').all().map((row) => row.name));
  if (!searchRunColumns.has('matched_count')) db.exec('ALTER TABLE search_runs ADD COLUMN matched_count INTEGER');
  if (!searchRunColumns.has('ready_count')) db.exec('ALTER TABLE search_runs ADD COLUMN ready_count INTEGER');
  ...
  const recordSearchTransaction = db.transaction((input) => {
    ...
    const eventAge = timestamp - Date.parse(String(input.eventDate || ''));
    const negativeTtlMs = Number.isFinite(eventAge) && eventAge >= 0 && eventAge <= 3 * 86400000
      ? Math.min(NEGATIVE_TTL_MS, 5 * 60000) : NEGATIVE_TTL_MS;
    const ttlMs = Number(input.ttlMs) > 0
      ? Number(input.ttlMs)
      : (candidates.length ? (SEARCH_TTLS[provider] || hours(null, 6)) : negativeTtlMs);
    db.prepare(`
  ...
  // Retain individual event matches independently of a complete search run.
  // In particular, a partial live answer must not create a cache hit.
  const recordEventCandidates = db.transaction((input) => {
    const provider = normalizeProvider(input.provider);
    const scope = cleanScope(input.scope);
    const timestamp = now();
    const candidates = (Array.isArray(input.results) ? input.results : []).slice(0, MAX_RESULTS_PER_SEARCH);
    const relation = db.prepare(`
      INSERT INTO event_releases(event_id,release_id,provider,scope_hash,card_part,match_score,payload_enc,first_seen_at,last_seen_at)
      VALUES (?,?,?,?,?,?,?,?,?)
      ON CONFLICT(event_id,release_id,provider,scope_hash) DO UPDATE SET
    ...
    for (const candidate of candidates) {
      const id = observe({provider, scope, state: 'discovered', candidate});
      relation.run(String(input.eventId || ''), id, provider, scope,
        classifyReleasePart(String(candidate.title || '')), Number(candidate.matchScore) || 0,
        cryptoKeys.encrypt(JSON.stringify(candidate)), timestamp, timestamp);
    }
    return {resultCount: candidates.length};
  });

  // Every release already matched to an event, whichever search found it. The
  // background builds record under their own restricted scope ("bitmagnet
  // only"), so a lookup keyed to the live scope never saw them. A matched
  // release is a fact about the event, not about the search, so the scope is
  // deliberately ignored here.
  function storedForEvent(input) {
    const item = input || {};
    const limit = Math.max(1, Math.min(100, Number(item.limit) || 50));
    const rows = db.prepare(`
      SELECT payload_enc payloadEnc FROM event_releases
      WHERE event_id=? AND provider=?
      ORDER BY match_score DESC,last_seen_at DESC LIMIT ?
    `).all(String(item.eventId || ''), normalizeProvider(item.provider), limit);
    const results = [];
    for (const row of rows) {
      try { results.push(JSON.parse(cryptoKeys.decrypt(row.payloadEnc))); }
      catch (_) { /* corrupted encrypted payloads are ignored */ }
    }
    return results;
  }

  function recordSearchOutcome(input) {
    const provider = normalizeProvider(input && input.provider);
    const scope = cleanScope(input && input.scope);
    const matchedCount = Math.max(0, Number(input && input.matchedCount) || 0);
    const readyCount = Math.max(0, Math.min(matchedCount, Number(input && input.readyCount) || 0));
    return db.prepare(`
      UPDATE search_runs SET matched_count=?,ready_count=?
      WHERE search_key=(
        SELECT search_key FROM search_runs
        WHERE event_id=? AND provider=? AND scope_hash=?
        ORDER BY searched_at DESC LIMIT 1
      )
    `).run(matchedCount, readyCount, String(input && input.eventId || ''), provider, scope).changes;
  }
  ...
  function confirmedForEvent(input) {
    const item = input || {};
    const states = Array.from(new Set((item.states || ['verified'])
      .map((value) => String(value || '').trim().toLowerCase())
      .filter((value) => ['cached', 'verified'].includes(value))));
    if (!states.length) return [];
    const limit = Math.max(1, Math.min(100, Number(item.limit) || 20));
    const placeholders = states.map(() => '?').join(',');
    const rows = db.prepare(`
      SELECT er.payload_enc payloadEnc,a.state,a.observed_at observedAt,
             a.expires_at expiresAt,a.last_success_at lastSuccessAt
      FROM event_releases er
      JOIN availability a ON a.release_id=er.release_id
      WHERE er.event_id=? AND er.provider=? AND er.scope_hash=?
        AND a.provider=? AND a.scope_hash=? AND a.state IN (${placeholders})
        AND a.expires_at>?
      ORDER BY COALESCE(a.last_success_at,a.observed_at) DESC,
               er.match_score DESC,er.last_seen_at DESC
      LIMIT ?
    `).all(
      String(item.eventId || ''), normalizeProvider(item.sourceProvider),
      cleanScope(item.sourceScope), normalizeProvider(item.availabilityProvider),
      cleanScope(item.availabilityScope), ...states, now(), limit,
    );
    increment.run('confirmed_lookups');
    const results = [];
    for (const row of rows) {
      try {
        results.push({
          candidate: JSON.parse(cryptoKeys.decrypt(row.payloadEnc)),
          state: row.state,
    ...
    if (results.length) increment.run('confirmed_hits');
    return results;
  }
  ...
  function stats() {
    ...
    return {
      ...
      searchHits: hits,
      searchMisses: misses,
      confirmedLookups: Number(counters.confirmed_lookups) || 0,
      confirmedHits: Number(counters.confirmed_hits) || 0,
      hitRate: hits + misses ? hits / (hits + misses) : 0,
      byProvider,
    };
  }

  // Every event this index has ever stored a release for, ignoring provider and
  // credential scope. The catalog availability gate asks "is there known
  // content for this event at all", which is a property of the event rather
  // than of one viewer's account — and answering it per-user would mean a
  // SQLite query per catalog request instead of one set built occasionally.
  function eventIdsWithReleases() {
    const out = new Set();
    for (const row of db.prepare('SELECT DISTINCT event_id eventId FROM event_releases').iterate()) {
      if (row && row.eventId) out.add(String(row.eventId));
    }
    return out;
  }
  ...
  function recentSearches(limit) {
    ...
    return db.prepare(`
      SELECT event_id eventId,promotion_id promotionId,provider,result_count resultCount,
             matched_count matchedCount,ready_count readyCount,
  ...
  function invalidateSearch(input) {
    const identity = searchKey(input);
    return db.prepare('DELETE FROM search_runs WHERE search_key=?').run(identity.key).changes;
  }

  // Remove saved discovery searches for one promotion without touching shared
  // release and account-availability records.  Search results are scoped to a
  // promotion; releases and availability are intentionally shared across the
  // index, so walking and deleting them can hold SQLite's write lock while the
  // server is serving streams.  The next request becomes a clean source search.
  function clearPromotion(promotionId) {
    const id = String(promotionId || '').trim();
    if (!id) return { searches: 0, eventMatches: 0, observations: 0, releases: 0 };
    const searches = db.prepare('DELETE FROM search_runs WHERE promotion_id=?').run(id).changes;
    return { searches, eventMatches: 0, observations: 0, releases: 0 };
  }
  ...
  return Object.freeze({
    ...
    candidateIdentity,
    clearPromotion,
    eventIdsWithReleases,
    eventReleaseTitles(eventIds, options = {}) {
      const ids=[...new Set(eventIds.map(String))],rows=[];
      const providers=Array.isArray(options.providers)
        ? [...new Set(options.providers.map(normalizeProvider).filter(Boolean))] : [];
      const identityKinds=Array.isArray(options.identityKinds) && options.identityKinds.length
        ? [...new Set(options.identityKinds.map(String))] : ['torrent','nzb','easynews'];
      for(let offset=0;offset<ids.length;offset+=200) {
        const batch=ids.slice(offset,offset+200);
        const providerSql=providers.length ? ` AND e.provider IN (${providers.map(()=>'?').join(',')})` : '';
        rows.push(...db.prepare(`SELECT DISTINCT e.event_id eventId,r.title,CASE WHEN r.identity_kind IN (${identityKinds.map(()=>'?').join(',')}) THEN 1 ELSE 0 END usable FROM event_releases e JOIN releases r ON r.id=e.release_id WHERE e.event_id IN (${batch.map(()=>'?').join(',')})${providerSql}`).all(...identityKinds,...batch,...providers));
      }
      return rows;
    },
...
// Release the shared index. Windows will not let a file be unlinked while a
// handle is open, so anything that creates the default index and then removes
// its directory — the test suite, a graceful shutdown — has to close it first.
// On Linux the same code passes without this, which is why the need for it only
// ever shows up on a Windows machine.
function closeDefault() {
  if (!defaultIndex) return false;
  try { defaultIndex.close(); } catch (_) { /* already closed */ }

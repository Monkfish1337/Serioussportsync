...
function createAvailabilityIndex(options) {
  ...
  return Object.freeze({
    ...
    eventReleaseTitles(eventIds) {
      const ids=[...new Set(eventIds.map(String))],rows=[];
      for(let offset=0;offset<ids.length;offset+=200) {
        const batch=ids.slice(offset,offset+200);
        rows.push(...db.prepare(`SELECT DISTINCT e.event_id eventId,r.title,CASE WHEN r.identity_kind IN ('torrent','nzb','easynews') THEN 1 ELSE 0 END usable FROM event_releases e JOIN releases r ON r.id=e.release_id WHERE e.event_id IN (${batch.map(()=>'?').join(',')})`).all(...batch));
      }
      return rows;
    },

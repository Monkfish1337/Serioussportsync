...
function createAvailabilityIndex(options) {
  ...
  // Remove discovery and availability state for one promotion without touching
  // any other promotion's cached searches.  This is deliberately broader than
  // just expiring `search_runs`: a stale per-account "unavailable" result
  // would otherwise prevent the fresh discovery from checking TorBox again.
  function clearPromotion(promotionId) {
    ...
    return db.transaction(() => {
      const releaseRows = db.prepare(`
        SELECT DISTINCT er.release_id releaseId
        FROM event_releases er
        JOIN search_runs sr ON sr.event_id=er.event_id
        WHERE sr.promotion_id=?
      `).all(id);
      const releaseIds = releaseRows.map((row) => row.releaseId);
      const eventMatches = db.prepare(`
        SELECT COUNT(*) FROM event_releases
        WHERE event_id IN (SELECT DISTINCT event_id FROM search_runs WHERE promotion_id=?)
      `).pluck().get(id) || 0;
      const searches = db.prepare('SELECT COUNT(*) FROM search_runs WHERE promotion_id=?').pluck().get(id) || 0;
      let observations = 0;
      if (releaseIds.length) {
        const placeholders = releaseIds.map(() => '?').join(',');
        observations = db.prepare(`DELETE FROM availability WHERE release_id IN (${placeholders})`).run(...releaseIds).changes;
      }
      db.prepare(`DELETE FROM event_releases
        WHERE event_id IN (SELECT DISTINCT event_id FROM search_runs WHERE promotion_id=?)`).run(id);
      db.prepare('DELETE FROM search_runs WHERE promotion_id=?').run(id);
      const releases = db.prepare(`DELETE FROM releases
        WHERE NOT EXISTS (SELECT 1 FROM event_releases er WHERE er.release_id=releases.id)
          AND NOT EXISTS (SELECT 1 FROM availability a WHERE a.release_id=releases.id)`).run().changes;
      return { searches, eventMatches, observations, releases };
    })();
  }

...
function createQueue(file, dependencies = {}) {
  ...
  function status() {return {options:options(),indexers:db.prepare('SELECT indexer id,name,requests,day,failures,next_at FROM limits WHERE scope=?').all(scope()),jobs:db.prepare('SELECT event,indexer,status,next_at FROM jobs WHERE scope=? ORDER BY next_at LIMIT 50').all(scope()),matches:db.prepare('SELECT COUNT(*) n FROM matches WHERE scope=?').get(scope()).n};}

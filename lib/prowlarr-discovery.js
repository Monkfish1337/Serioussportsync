...
function createQueue(file, dependencies = {}) {
  ...
  async function run(runOptions = {}) {
    ...
    if (!opt.enabled || pw.enabled===false || !pw.url || !pw.apiKey) return {skipped:'paused'};
    // Another search is in flight; a Search now waits for it. The queue's own
    // timer stands aside while a Search now is running, so it cannot take
    // the slot between two of the sweep's searches.
    if (busy) return {skipped:'busy'};
    if (!force && sweep && !sweep.finishedAt) return {skipped:'sweep'};
    if (force) {
    ...
    try {
      ...
      // receiving their first search, nor the reverse: with a backlog of first
      // searches (a newly added league) a retry could wait for days. Every
      // third search goes to a retry more than six hours overdue; newest games
      // still lead within each group, and admin priorities always win.
      const turns = db.prepare('SELECT COUNT(*) n FROM queries WHERE scope=?').get(runScope).n;
      const overdue = turns % 3 === 2 ? new Set(db.prepare('SELECT event FROM jobs WHERE scope=? GROUP BY event HAVING MIN(next_at)<?')
        .all(runScope,now()-6*HOUR).map(row=>row.event).filter(id=>searched.has(id))) : new Set();
      fixtures.sort((a,b)=>(priorities.get(b.id)||0)-(priorities.get(a.id)||0)
        || Number(overdue.has(b.id))-Number(overdue.has(a.id))
        || Number(searched.has(a.id))-Number(searched.has(b.id)));
      ...
      const result = await search([query],{indexerId:indexer.id,maxQueries:1,detailed:true,
        ...
        hydrationLimit:3,hydrationConcurrency:1,hydrateUnseeded:true,hashCache,fetchImpl:measuredFetch,log,
  ...
  async function manualSearch(event, query) {
    ...
    if(pw && pw.enabled!==false && pw.url && pw.apiKey) tasks.push({name:'Prowlarr',run:()=>
      (dependencies.manualProwlarrSearch||prowlarr.multiSearch)(queries,{detailed:true,deadlineMs:timing.prowlarrLiveBudgetMs,timeoutMs:timing.prowlarrLiveBudgetMs,hydrationLimit:20,hydrationConcurrency:2,hydrateUnseeded:true})});

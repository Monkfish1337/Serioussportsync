...
function createQueue(file, dependencies = {}) {
  ...
  async function run() {
    ...
    let chosen;
    try {
      const fixtures = eligible(getEvents(),now(),opt.lookbackDays,opt.promotions).filter(e => {const p=getPromo(e.id); return settings.discoveryIncludes(e.id) && p && p.enabled!==false && !(p.disabledPipelines || []).includes('torbox');});
  ...
  function enqueue(event) {
    if (!settings.discoveryIncludes(event.id) || !eligible([event],now(),options().lookbackDays,options().promotions).length) throw new Error('Choose a selected past event within the discovery window.');

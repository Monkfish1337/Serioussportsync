...
function render(data) {
  ...
  const diagnosis = require('./admin-diagnosis');
  return diagnosis.STYLE + diagnosis.tabs('client-check')
    + '<div class="lede"><p>Opens an account\'s addon the way Nuvio does (manifest, catalogs, event details, links) and reports what a client sees. Nothing is played or sent to TorBox, but opening an event searches live.</p></div>'
    ...
    + '<div class="col-md-3"><label class="form-label">Or one event ID</label><input class="form-control" name="eventId" value="' + esc(data.eventId || '') + '" placeholder="e.g. motogp:2368493"></div>'

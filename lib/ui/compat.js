...
/* A table cell inherits vertical-align: middle from the table by default, so
   dropping .table-vcenter is not enough to change anything — it has to be said.
   Rows whose cells are a primary line plus a secondary one need top: middle
   floats the one-line cells (a badge, a single word, a count) to the centre of
   a three-line row while the multi-line cells start at the top, and nothing
   shares a baseline. A badge is worse still, because an inline-block sits its
   bottom margin edge on the text baseline. */
.table.align-top td, .table.align-top th { vertical-align: top; }
.table tbody tr:hover td { background: var(--surface-2); }
/* A scroll container only scrolls if something stops it growing. Without a
   width bound this expanded to its content and pushed the whole card off the
   right of the page instead.

   Scrolling is the fallback, not the fix. Measured in a headless browser at a
   1440px viewport, the Promotions table wanted 1386px inside a 1018px card: it
   scrolled, but the scrollbar sits under a 34-row table where nobody finds it,
   so the Refresh and Edit buttons simply read as chopped off. The columns were
   cut down instead — Poster and Catalogs folded into the name cell — and the
   action buttons wrap. */
.table-responsive { overflow-x: auto; max-width: 100%; -webkit-overflow-scrolling: touch; }
.table-responsive > .table { min-width: 720px; }

/* Five buttons on one nowrap line were 690px of a 1386px table on their own.
   Wrapping them costs a row of height and nothing else. */
.promo-actions-head { width: 250px; }
.table td.promo-actions { white-space: normal; width: 250px; }
.table td.promo-actions .btn { margin-bottom: 4px; }

...
function getProwlarr() {
  ...
  return {
    ...
    liveSearchEnabled: p.liveSearchEnabled !== false,
  };
}
function getProwlarrDiscovery() {
  const p = loadAll().prowlarrDiscovery || {};
  const number = (key, fallback, min, max) => Math.max(min, Math.min(max, Number(p[key]) || fallback));
  return {enabled: p.enabled === undefined ? process.env.PROWLARR_DISCOVERY_ENABLED !== '0' : p.enabled !== false, intervalSeconds: number('intervalSeconds', 120, 60, 3600),
    dailyRequests: number('dailyRequests', 100, 10, 1000), lookbackDays: number('lookbackDays', 30, 1, 90),
    timeoutSeconds: number('timeoutSeconds', 120, 30, 180),
    promotions: getSourceDiscoveryPromotions('prowlarr') || (Array.isArray(p.promotions) ? p.promotions.slice() : ['mlb','nba','nfl'])};
}
function setProwlarrDiscovery(input) {
  if (input.promotions !== undefined) {
    const known = new Set(require('./promotions').all.map(p=>p.id));
    input = {...input,promotions:[...new Set([].concat(input.promotions || []).map(String))]};
    if (input.promotions.some(id=>!known.has(id))) throw new Error('Unknown discovery promotion');
  }
  const st = loadAll();
  st.prowlarrDiscovery = {...(st.prowlarrDiscovery || {}), ...input};
  if (input.promotions !== undefined) st.sourceDiscoveryPromotions = {...st.sourceDiscoveryPromotions,prowlarr:input.promotions};
  saveAll(st);
}
function getZilean() {
...
// Discovery timing — the cut-offs that decide time against results.
//
// Every one of these was a constant or an env var, which meant tuning the
// trade-off required a redeploy and knowing the variable names. They are the
// most consequential numbers in the whole stream path: Prowlarr answers in
// about two seconds per query, so a five-second discovery budget is the
// difference between two queries and six, and that in turn decided whether the
// query which reaches rutracker was ever sent at all.
//
// Saved values win over the environment, and the environment still wins over
// the defaults, so an existing deployment keeps whatever it had.
const DISCOVERY_TIMING_DEFAULTS = Object.freeze({
  // How long the whole stream request may take before pipelines are abandoned.
  pipelineBudgetMs: 9500,
  // How long discovery inside it may take. Must leave room for relevance
  // filtering, dedupe and the TorBox cache check, so it is clamped below the
  // pipeline budget when both are read.
  discoveryBudgetMs: 5000,
  // Prowlarr is a sequential fan-out to remote trackers. This is the count it
  // is offered; the budget above decides how many it finishes.
  prowlarrMaxQueries: 6,
  // Per-request HTTP timeout for one Prowlarr search.
  prowlarrQueryTimeoutMs: 15000,
  prowlarrLiveBudgetMs: 50000,
  // The background index build, which has nobody waiting on it.
  indexBuildBudgetMs: 25000,
  easynewsMaxQueries: 6,
  easynewsQueryTimeoutMs: 5000,
  fastResponseGraceMs: 0,
});

function getDiscoveryTiming() {
  const saved = loadAll().discoveryTiming || {};
  const env = {
    pipelineBudgetMs: process.env.STREAM_PIPELINE_TIMEOUT_MS,
    discoveryBudgetMs: process.env.STREAM_DISCOVERY_BUDGET_MS,
    indexBuildBudgetMs: process.env.AVAILABILITY_WARM_PROVIDER_TIMEOUT_MS,
  };
  const pick = (key) => {
    const fromSaved = parseInt(saved[key], 10);
    if (Number.isFinite(fromSaved) && fromSaved > 0) return fromSaved;
    const fromEnv = parseInt(env[key], 10);
    if (Number.isFinite(fromEnv) && fromEnv > 0) return fromEnv;
    return DISCOVERY_TIMING_DEFAULTS[key];
  };
  const out = {};
  for (const key of Object.keys(DISCOVERY_TIMING_DEFAULTS)) out[key] = pick(key);
  // A discovery budget at or above the pipeline budget starves everything that
  // has to happen after discovery, so it is clamped rather than refused: the
  // operator sees the number they typed take effect up to the point where it
  // would break the request.
  out.discoveryBudgetMs = Math.max(1000,
    Math.min(out.discoveryBudgetMs, out.pipelineBudgetMs - 1000));
  return out;
}

function setDiscoveryTiming(input) {
  const st = loadAll();
  const previous = st.discoveryTiming || {};
  const bounds = {
    pipelineBudgetMs: [2000, 120000],
    discoveryBudgetMs: [1000, 120000],
    prowlarrMaxQueries: [1, 60],
    prowlarrQueryTimeoutMs: [1000, 120000],
    prowlarrLiveBudgetMs: [1000, 120000],
    indexBuildBudgetMs: [1000, 600000],
    easynewsMaxQueries: [1, 6],
    easynewsQueryTimeoutMs: [500, 120000],
    fastResponseGraceMs: [0, 120000],
  };
  const next = {};
  for (const key of Object.keys(DISCOVERY_TIMING_DEFAULTS)) {
    const raw = input && Object.prototype.hasOwnProperty.call(input, key) ? input[key] : previous[key];
    const parsed = parseInt(raw, 10);
    if (!Number.isFinite(parsed)) continue;          // blank = fall back to env/default
    const [min, max] = bounds[key];
    next[key] = Math.max(min, Math.min(max, parsed));
  }
  st.discoveryTiming = next;
  saveAll(st);
  return getDiscoveryTiming();
}
...
function setProwlarr({ url, apiKey, enabled, liveSearchEnabled }) {
  const st = loadAll();
  ...
  st.prowlarr = {
    ...
    apiKey: str(apiKey),
    enabled: flag(enabled, previous.enabled !== false),
    liveSearchEnabled: flag(liveSearchEnabled, previous.liveSearchEnabled !== false),
  };
  st.updatedAt = new Date().toISOString();
  saveAll(st);
  return st;
...
// 0.95.0 — TMDB, so Match of the Day is configurable from the interface.
//
// It was the one shipped promotion whose key had no field anywhere: it needs
// TMDB_API_KEY, the Server page offered football-data.org and API-Football and
// nothing else, and an install without that variable simply showed no events
// with no way to find out why.
function getTmdb() {
  const value = loadAll().tmdb || {};
  return { apiKey: str(value.apiKey) || (config.tmdb && config.tmdb.apiKey) || '' };
}

function setTmdb({ apiKey }) {
  const st = loadAll();
  st.tmdb = { apiKey: str(apiKey) };
  st.updatedAt = new Date().toISOString();
  saveAll(st);
  return st;
}
...
function getSportVideo() {
  ...
  return {
    ...
    intervalHours: effectiveNumber(stored.intervalHours, 1, 1, 168),
    startDelaySeconds: Math.round(effectiveNumber(stored.startDelaySeconds, 90, 10, 3600)),
    maxDetailsPerScan: Math.round(effectiveNumber(stored.maxDetailsPerScan, 25, 1, 200)),
...
function getLogPreferences() {
  ...
  return {
    ...
    // Off by default, and the default matters more than it sounds.
    //
    // The background index build logs one line per query per event. At 379
    // events and 54 variants that is roughly twenty thousand lines a run
    // against a five-thousand-line buffer, so a run flushes everything else
    // out: /admin/logs held 37 seconds of history and not one stream request,
    // exactly when a stream request was the thing that needed looking at. The
    // build's summaries still log either way; only its per-query chatter is
    // suppressed.
    verboseIndexBuild: typeof stored.verboseIndexBuild === 'boolean'
      ? stored.verboseIndexBuild
      : /^(1|true|yes|on)$/i.test(String(process.env.LOG_VERBOSE_INDEX_BUILD || '')),
  };
}
...
function setLogPreferences(input) {
  const st = loadAll();
  st.logs = {
    detailedRejections: Boolean(input && input.detailedRejections),
    verboseIndexBuild: Boolean(input && input.verboseIndexBuild),
  };
  st.updatedAt = new Date().toISOString();
  saveAll(st);
...
function getDisplayTimeZone() {
  const zone = String(loadAll().displayTimeZone || process.env.TZ || 'Europe/London');
...
function setDisplayTimeZone(value) {
  ...
  try { new Intl.DateTimeFormat('en-GB', {timeZone: zone}); }
  catch (_) { throw new Error('Choose a valid time zone'); }
  if (!zone) throw new Error('Choose a valid time zone');
  const state = loadAll(); state.displayTimeZone = zone; saveAll(state);
  return zone;
}

// Native Usenet engine controls are server-wide. Provider credentials remain
// encrypted on the admin account, while these values describe how the one
// process shares memory and NNTP capacity between playback requests.
const USENET_ENGINE_PROFILES = Object.freeze({
  balanced: { startupSegments: 1, prefetchSegments: 24, articleRetries: 2, articleTimeoutMs: 15000, maxConcurrentStreams: 4 },
  'low-latency': { startupSegments: 1, prefetchSegments: 12, articleRetries: 1, articleTimeoutMs: 10000, maxConcurrentStreams: 4 },
  resilient: { startupSegments: 2, prefetchSegments: 48, articleRetries: 3, articleTimeoutMs: 25000, maxConcurrentStreams: 2 },
});

function getUsenetEngine() {
  const stored = loadAll().usenetEngine || {};
  const profile = Object.prototype.hasOwnProperty.call(USENET_ENGINE_PROFILES, stored.profile)
    ? stored.profile : 'balanced';
  const defaults = USENET_ENGINE_PROFILES[profile];
  return {
    profile,
    startupSegments: Math.round(effectiveNumber(stored.startupSegments, defaults.startupSegments, 1, 8)),
    prefetchSegments: Math.round(effectiveNumber(stored.prefetchSegments, defaults.prefetchSegments, 2, 256)),
    articleRetries: Math.round(effectiveNumber(stored.articleRetries, defaults.articleRetries, 0, 5)),
    articleTimeoutMs: Math.round(effectiveNumber(stored.articleTimeoutMs, defaults.articleTimeoutMs, 3000, 60000)),
    maxConcurrentStreams: Math.round(effectiveNumber(stored.maxConcurrentStreams, defaults.maxConcurrentStreams, 1, 16)),
  };
}

function setUsenetEngine(input) {
  const profile = String(input && input.profile || 'balanced');
  if (!Object.prototype.hasOwnProperty.call(USENET_ENGINE_PROFILES, profile)) {
    throw new Error('Choose a valid Usenet performance profile');
  }
  const defaults = USENET_ENGINE_PROFILES[profile];
  const state = loadAll();
  state.usenetEngine = {
    profile,
    startupSegments: Math.round(boundedNumber(input.startupSegments || defaults.startupSegments, 'Startup segments', 1, 8)),
    prefetchSegments: Math.round(boundedNumber(input.prefetchSegments || defaults.prefetchSegments, 'Prefetch segments', 2, 256)),
    articleRetries: Math.round(boundedNumber(input.articleRetries === '' || input.articleRetries == null ? defaults.articleRetries : input.articleRetries, 'Article retries', 0, 5)),
    articleTimeoutMs: Math.round(boundedNumber(input.articleTimeoutMs || defaults.articleTimeoutMs, 'Article timeout', 3000, 60000)),
    maxConcurrentStreams: Math.round(boundedNumber(input.maxConcurrentStreams || defaults.maxConcurrentStreams, 'Concurrent streams', 1, 16)),
  };
  state.updatedAt = new Date().toISOString();
  saveAll(state);
  return getUsenetEngine();
}

// Native NNTP playback may need a LAN route even when the addon was installed
// through a public manifest. Only the native playback URL uses this origin;
// catalog, stream discovery and other providers keep their request origin.
function getDiyPlaybackOrigin() {
  return str(loadAll().diyPlaybackOrigin);
}

function setDiyPlaybackOrigin(input) {
  const value = str(input);
  let origin = '';
  if (value) {
    const cleaned = security.cleanHttpUrl(value, { label: 'DIY LAN playback URL', allowEmpty: false });
    const parsed = new URL(cleaned);
    if (parsed.pathname !== '/' || parsed.search || parsed.hash) {
      throw new Error('DIY LAN playback URL must be only a scheme, host and optional port');
    }
    origin = parsed.origin;
  }
  const state = loadAll();
  state.diyPlaybackOrigin = origin;
  saveAll(state);
  return origin;
}

function getPromotionMetadataStartDates() {
  const saved = loadAll().promotionMetadataStartDates;
  return saved && typeof saved === 'object' && !Array.isArray(saved) ? { ...saved } : {};
}
function setPromotionMetadataStartDate(promotionId, value) {
  const id = String(promotionId || '').trim();
  const date = String(value || '').trim();
  if (!id || !require('./promotions').all.some((promotion) => promotion.id === id)) {
    throw new Error('Promotion not found');
  }
  if (date && (!/^\d{4}-\d{2}-\d{2}$/.test(date)
    || !Number.isFinite(Date.parse(date + 'T00:00:00Z'))
    || new Date(date + 'T00:00:00Z').toISOString().slice(0, 10) !== date)) {
    throw new Error('Enter a real date in YYYY-MM-DD format');
  }
  const state = loadAll();
  state.promotionMetadataStartDates = { ...(state.promotionMetadataStartDates || {}) };
  if (date) state.promotionMetadataStartDates[id] = date;
  else delete state.promotionMetadataStartDates[id];
  saveAll(state);
  return date;
}

module.exports = {
  getPromotionMetadataStartDates, setPromotionMetadataStartDate,
  getSourceDiscoveryPromotions, setSourceDiscoveryPromotions, sourceDiscoveryIncludes,
  getDisplayTimeZone, setDisplayTimeZone,
  getAppearance, setAppearance,
  getCatalogGate, setCatalogGate,
  getProwlarr, setProwlarr, getProwlarrDiscovery, setProwlarrDiscovery, getZilean, setSources,
  getBitmagnet, setBitmagnet,
  getDiscoveryTiming, setDiscoveryTiming, DISCOVERY_TIMING_DEFAULTS,
  ...
  getUsenetEngine, setUsenetEngine, USENET_ENGINE_PROFILES,
  getDiyPlaybackOrigin, setDiyPlaybackOrigin,
};

// Null preserves existing coverage; an explicit empty selection pauses event work.
function getSourceDiscoveryPromotions(source) {
  const state=loadAll();
  const value=state.sourceDiscoveryPromotions?.[source];
  if (Array.isArray(value)) return value.slice();
  // Migrate the old shared restriction lazily without expanding scheduled work.
  const legacy=state.discoveryPromotions;
  if (!Array.isArray(legacy)) return null;
  if (source==='prowlarr') return (state.prowlarrDiscovery?.promotions || ['mlb','nba','nfl']).filter(id=>legacy.includes(id));
  return legacy.slice();
}
function setSourceDiscoveryPromotions(source,input) {
  if (!['prowlarr','sport-video','bitmagnet'].includes(source)) throw new Error('Unknown discovery source');
  const known = new Set(require('./promotions').all.map(p => p.id));
  const ids = [...new Set([].concat(input || []).map(String))];
  if (ids.some(id => !known.has(id))) throw new Error('Unknown discovery promotion');
  const state = loadAll();
  state.sourceDiscoveryPromotions = {...state.sourceDiscoveryPromotions,[source]:ids};
  if (source==='prowlarr') state.prowlarrDiscovery={...state.prowlarrDiscovery,promotions:ids};
  saveAll(state);
  return ids;
}
function sourceDiscoveryIncludes(source,eventId) {
  const selected = getSourceDiscoveryPromotions(source);
  return selected === null || selected.includes(String(eventId || '').split(':')[0]);
}

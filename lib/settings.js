...
function getProwlarrDiscovery() {
  ...
  return {enabled: p.enabled === undefined ? process.env.PROWLARR_DISCOVERY_ENABLED !== '0' : p.enabled !== false, intervalSeconds: number('intervalSeconds', 120, 60, 3600),
    ...
    promotions: Array.isArray(p.promotions) ? p.promotions.slice() : ['mlb','nba','nfl']};
}
...
module.exports = {
  getDiscoveryPromotions, setDiscoveryPromotions, discoveryIncludes,
...
function getDiscoveryPromotions() {
  const value = loadAll().discoveryPromotions;
  return Array.isArray(value) ? value.slice() : null;
}
function setDiscoveryPromotions(input) {
  const known = new Set(require('./promotions').all.map(p => p.id));
  const ids = [...new Set([].concat(input || []).map(String))];
  if (ids.some(id => !known.has(id))) throw new Error('Unknown discovery promotion');
  const state = loadAll();
  state.discoveryPromotions = ids;
  saveAll(state);
  return ids;
}
function discoveryIncludes(eventId) {
  const selected = getDiscoveryPromotions();

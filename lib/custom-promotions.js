...
function isTeamCatalog(spec) {
  return !!(spec && (spec.teamId || (spec.teamFilter &&
    (spec.teamFilter.id || (Array.isArray(spec.teamFilter.names) && spec.teamFilter.names.length)))));
}
...
function load() {
  try {
    ...
    // Upgrade existing team catalogs without requiring users to recreate them.
    let changed = false;
    for (const spec of data.promotions) {
      if (isTeamCatalog(spec) && spec.posterShape !== 'square') {
        spec.posterShape = 'square';
        changed = true;
      }
    }
...
const VALID_SOURCES = ['tsdb', 'football-data', 'api-football', 'uefa', 'tmdb', 'onefc', 'mlb', 'espn'];
...
function validateSpec(spec, existingIds) {
  ...
  if (source === 'tsdb') {
    ...
    const teamId = String(spec.teamId || '').trim();
    ...
    if (teamId && !/^\d+$/.test(teamId)) return { ok: false, error: 'teamId must be numeric' };
    if (!teamId && !/^(\d+|[A-Za-z0-9]{2,4})$/.test(competitionId)) {
      return { ok: false, error: 'competitionId must be numeric or 2-4 char code' };
    }
  } else if (source === 'espn') {
    // The ESPN adapter serves several leagues from one endpoint family, so the
    // league identifier is the per-promotion reference.
    const league = String(spec.league || '').trim().toLowerCase();
    const allowed = Object.keys(require('./sources/espn').LEAGUES);
    if (allowed.indexOf(league) === -1) {
      return { ok: false, error: 'league must be one of: ' + allowed.join(', ') };
    }
  } else if (source === 'api-football') {
    const leagueId = String(spec.leagueId || '').trim();
    if (!/^\d+$/.test(leagueId)) return { ok: false, error: 'leagueId must be numeric (API-Football competition id)' };
  } else if (source === 'uefa') {
    const competitionId = String(spec.competitionId || '').trim();
    if (!/^\d+$/.test(competitionId)) return { ok: false, error: 'competitionId must be numeric (UEFA competition id)' };
  } else if (source === 'tmdb') {
    ...
    const tvIds = Array.isArray(spec.tvIds) ? spec.tvIds.map(String) : [String(spec.tvId || '')];
    if (!tvIds.length || tvIds.some((tvId) => !/^\d+$/.test(tvId))) {
  ...
  if (spec.promotionAliases !== undefined && spec.promotionAliases !== null) {
    if (!Array.isArray(spec.promotionAliases)) {
      return { ok: false, error: 'promotionAliases must be an array of strings' };
    }
    if (spec.promotionAliases.length > 20) {
      return { ok: false, error: 'promotionAliases supports at most 20 aliases' };
    }
    if (spec.promotionAliases.some((v) => !String(v || '').trim() || String(v).trim().length > 64)) {
      return { ok: false, error: 'promotionAliases entries must be 1-64 characters' };
    }
  }

  if (spec.exclusionKeywords !== undefined && spec.exclusionKeywords !== null) {
    if (!Array.isArray(spec.exclusionKeywords) || spec.exclusionKeywords.length > 20) {
      return { ok: false, error: 'exclusionKeywords must be an array of at most 20 strings' };
    }
    if (spec.exclusionKeywords.some((v) => !String(v || '').trim() || String(v).trim().length > 64)) {
      return { ok: false, error: 'exclusionKeywords entries must be 1-64 characters' };
    }
    const matchingRules = promotionRuleTools.sanitizeMatchingRules(
      name, spec.promotionAliases || [], cleanKeywords, spec.exclusionKeywords
    );
    if (matchingRules.removedExclusions.length) {
      return { ok: false, error: 'Reject words conflict with promotion names/aliases: ' + matchingRules.removedExclusions.join(', ') };
    }
  }
  ...
  if (spec.allowForeignLanguage !== undefined && spec.allowForeignLanguage !== null
      && typeof spec.allowForeignLanguage !== 'boolean') {
    return { ok: false, error: 'allowForeignLanguage must be true or false' };
  }
...
function normaliseSpec(spec) {
  const source = String(spec.source || 'tsdb').trim();
  const aliases = dedupeText((Array.isArray(spec.promotionAliases) ? spec.promotionAliases : [])
    .map((alias) => promotionRuleTools.stripEventStageSuffix(alias)));
  const keywords = dedupeLower(Array.isArray(spec.relevanceKeywords) ? spec.relevanceKeywords : []);
  const matchingRules = promotionRuleTools.sanitizeMatchingRules(
    spec.name, aliases, keywords,
    dedupeLower(Array.isArray(spec.exclusionKeywords) ? spec.exclusionKeywords : [])
  );
  const out = {
    ...
    source,
    sourceRef: String(spec.sourceRef || '').trim() || undefined,
    poster: String(spec.poster || '').trim(),
    fanart: String(spec.fanart || '').trim(),
    logo:   String(spec.logo   || '').trim(),
    posterShape: isTeamCatalog(spec) ? 'square' : String(spec.posterShape || 'landscape').trim(),
    ...
    // 0.91.0 — RETAINED, NOT DELETED.
    //
    // Deselecting a team has to take its catalog out of the manifest without
    // throwing anything away: the user gets it back by re-picking the team, and
    // re-picking should cost no provider calls. `enabled: false` does exactly
    // that — promotions.byPrefix carries only enabled promotions, so the
    // catalog leaves the manifest by itself, while promotions.all still knows
    // the promotion, which is what keeps scripts/refresh from pruning its
    // events as orphans.
    //
    // Deleting outright is the thing to avoid: that is what left a store full
    // of unreachable `manutd:` events when the shipped promotion went away.
    enabled: spec.enabled === undefined ? true : spec.enabled !== false,
    autoTeam: spec.autoTeam === true,
    // Per-team scoping written by the Configure-page team picker: a league feed
    // narrowed to one club. Carried verbatim so the promotion registry can
    // rebuild the filter without re-consulting the provider.
    teamFilter: (spec.teamFilter && typeof spec.teamFilter === 'object')
      ? {
  ...
  if (source === 'tsdb') {
    ...
    if (spec.teamId) out.teamId = String(spec.teamId).trim();
    else out.competitionId = String(spec.competitionId || '').trim();
  } else if (source === 'espn') {
    out.league = String(spec.league || '').trim().toLowerCase();
  } else if (source === 'api-football') {
    out.leagueId = String(spec.leagueId || '').trim();
  } else if (source === 'uefa') {
    out.competitionId = String(spec.competitionId || '').trim();
  } else if (source === 'tmdb') {
    if (Array.isArray(spec.tvIds) && spec.tvIds.length > 1) out.tvIds = spec.tvIds.map(String);
    else out.tvId = String((spec.tvIds && spec.tvIds[0]) || spec.tvId || '').trim();
  }
  return out;
}
...
// Materialize a provider roster in one write. Existing hand-tuned team rules
// are preserved; only the old shared off switch is retired for these catalogs.
function ensureTeamRoster(specs) {
  const state = load();
  const byId = new Map(state.promotions.map((spec) => [spec.id, spec]));
  let changed = false;
  for (const spec of specs) {
    const existing = byId.get(spec.id);
    if (existing) {
      if (!isTeamCatalog(existing)) continue;
      if (existing.autoTeam !== true || existing.enabled === false) {
        existing.autoTeam = true;
        existing.enabled = true;
        changed = true;
      }
      continue;
    }
    const validation = validateSpec(spec, new Set(byId.keys()));
    if (!validation.ok) throw new Error(validation.error);
    const normalized = normaliseSpec({ ...spec, autoTeam: true, enabled: true });
    state.promotions.push(normalized);
    byId.set(spec.id, normalized);
    changed = true;
  }
